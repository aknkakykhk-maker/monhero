...
const SKIN_RULES = {
  ...
  // みゅあと横並びにしたとき顔の大きさ・高さが揃って見えるよう、切り出しを
  // 引いた(headRatio/headTopSkipを調整した)ときの実測値。前髪で額が隠れる
  // ぶん肌色の重心はみゅあより下寄りに出るが、頭シルエットの重心(headCy)は
  // みゅあの53%とほぼ同じ53〜54%まで揃っている
  kiki: { centerMin: 0.35, x: [0.50, 0.60], y: [0.55, 0.66] },
  // ももすけは元絵が正方形のバストアップで、額も出ているため肌がよく見える。
  // 切り出しを合わせたときの実測は 肌中心81〜92% / 重心x48〜49% / y52〜54% で、
  // 頭シルエットの重心(y53%)はみゅあ・ききと揃っている
  momosuke: { centerMin: 0.70, x: [0.44, 0.56], y: [0.46, 0.60] },
  // ドラは元絵が頭だけの絵で、太い輪郭線と大きいメガネが顔の面積をかなり取る。
  // 切り出しをみゅあ・きき・ももすけと同じ「引き」にそろえたときの実測は
  // 肌中心59〜65% / 重心x49〜51% / y58〜61%、頭シルエットの占有66〜70%(他の3人は61〜71%)
  dra: { centerMin: 0.55, x: [0.44, 0.56], y: [0.52, 0.66] },
};
const SKIN_FALLBACK = { centerMin: 0.10, x: [0.35, 0.65], y: [0.40, 0.80] };
// 肌色とみなす色。アニメ調の肌は赤みが強く、青が少し低い。
// ★r-b の上限は 90 → 105 へ広げた(2026-09-17・ドラを足したとき)。
//   ドラの肌は他の3人より彩度が高いオレンジ(実測 R≈255 G≈192 B≈160 ＝ r-b≈95)で、
//   90 のままだと肌が1画素も拾えず「顔が真ん中に無い」と誤検出していた(肌中心1〜2%)。
//   広げてもみゅあ・きき・ももすけの実測値は変わらない(肌中心 75〜84% / 44〜53% / 77〜91%)。
const isSkin = (r, g, b, a) => a > 128 && r > 200 && g > 160 && b > 140 && r > b + 10 && r - b < 105;
