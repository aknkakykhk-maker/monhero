...
for(const width of [1,2,3,4]){
  ...
  assert.strictEqual(span.subLaneWidth,width,`幅${width}の始点投影`);
  assert.deepEqual(match([hold(3,width)],[input(3+width/2)]),[0],`幅${width}の開始入力`);
}
assert.deepEqual(match([hold(4,1)],[input(3.85)]),[0],'幅1の最小タッチ許容');
...
assert(/widthHoldTestNotes[\s\S]*\[1800,3200,0,1\][\s\S]*\[8400,10000,6,4\][\s\S]*\[10800,12200,0,1\][\s\S]*\[13000,14400,9,1\][\s\S]*\[15200,17000,4,1\],\[15200,17000,5,1\]/.test(source),'WIDTH TEST NORMALに幅1〜4・左右端・隣接を収録');
...
console.log('OK: 可変幅HOLDの投影・帯・ENDバー・入力・互換・性能回帰');
