...
// 「先頭6000バイトの中にあるか」で見ていたが、更新履歴は新しい項目を先頭へ足していくので、
// 項目が2つ増えるだけで窓から押し出されて落ちる(実際に 5899 → 6511 で落ちた)。
// 見たいのは「その項目が更新履歴にあるか」なので、見出しそのものを探す。
// 窓を広げるのではなく探し方を変えているので、確かめていることは緩めていない(むしろ狭めた)。
check('更新履歴に書いてある',/title:'[^']*回数ボーナス[^']*'/.test(changelog));
