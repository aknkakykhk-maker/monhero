...
// HOLDに途中幅変化(holdPoints)を足したので、終端バーには「終端の時刻の幅」を渡す。
// 渡さないと、終わりが細いHOLDで横棒だけ始点の太さのまま残る。
check('終端バーの横位置と幅はHOLDサブレーンまたは共通lane projectionを再利用',source.includes("rhythmNoteHasVariableSpan(note)&&note.type==='HOLD'?rhythmNoteVisualSpan(note,lane,endY,rhythmReleaseTargetMs(note)):rhythmNoteIsSlide(note)?rhythmProjectSlideSpan(rhythmReleaseLane(note),note,endY,rhythmReleaseTargetMs(note)):rhythmProjectLane(rhythmReleaseLane(note),endY)")&&source.includes('rect.width*end.width*RHYTHM_NOTE_WIDTH_RATIO'));
