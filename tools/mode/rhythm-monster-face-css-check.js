...
// span:last-child で装飾を書くと、絵が入るノーツでは飾りが絵のほうへ付く。粒を名指しする。
check('通常ノーツの装飾は粒(data-rhythm-note-head)に掛ける。span:last-child で書かない',
  compact.includes('[data-rhythm-note] > [data-rhythm-note-head] {')
  &&!/\[data-rhythm-note\][^\n]{0,80}span:last-child/.test(compact.replace(/\/\*[\s\S]*?\*\//g,'')));
...
check('モンスターノーツ本体だけに白金の輪郭と金色発光を足す',
  compact.includes('[data-rhythm-note][data-rhythm-monster-note] > [data-rhythm-note-head]::before')
...
check('モンスターノーツ本体だけに紫・シアンの薄い外周オーラを足す',
  compact.includes('[data-rhythm-note][data-rhythm-monster-note] > [data-rhythm-note-head]::after')
...
check('動きを減らす設定では専用オーラを常時アニメーションしない',
  ...
  &&compact.includes('[data-rhythm-note][data-rhythm-monster-note] > [data-rhythm-note-head]::after { animation:none; opacity:.66; }'));
console.log(failed?`\n${failed}件のNGがあります`:'\nすべてOK');
