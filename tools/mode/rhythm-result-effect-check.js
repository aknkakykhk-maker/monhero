...
// ---- 実ブラウザ: 段ごとの計算済みの値を測る ----
// リザルトの左に大きく出すマスモン(2026-09-26)。はじめは「能力がいちばん多く出た子」だったが、
// モンスターノーツは1体1回なので同点ばかりになり、ユーザーが「毎回ランダム」を選んだ。
// ノーツを取れた子からランダム → 誰も取れなければ絵のある子からランダム。乱数は演奏の始めに1回だけ決める
{
  ...
  const f=ctx.f;
  const picks=(arts,counts)=>[...new Set(Array.from({length:40},(_,i)=>f(arts,counts,i/40)))].sort();
  ok('ノーツを取れた子の中からランダムに選ぶ(取れなかった子・絵の無い子は選ばない)',
    JSON.stringify(picks(['a','b','c','d'],[1,0,1,1]))==='[0,2,3]'&&JSON.stringify(picks(['a','','c','d'],[1,1,0,0]))==='[0]');
  ok('誰も取れなければ絵のある子からランダム・誰もいなければジャケット(-1)',
    JSON.stringify(picks(['','b','c'],[]))==='[1,2]'&&f(['',''],[3],.5)===-1);
  ok('乱数が変な値でも落ちない',f(['a','b'],[1,1],NaN)===0&&f(['a','b'],[1,1],1)===0&&f(['a','b'],[1,1],.99)===1);
  ok('乱数は演奏の始めに1回だけ決め、リザルトはその数で選ぶ(描き直しで入れ替わらない)',
    game.includes('run.abilityCounts[slot-1]=(Number(run.abilityCounts[slot-1])||0)+1;')
    &&game.includes('runRef.current={audio,heroSeed:Math.random(),')
    &&game.includes('const index=rhythmResultHeroIndex(sideArtUrls,runRef.current?.abilityCounts,runRef.current?.heroSeed);'));
}
