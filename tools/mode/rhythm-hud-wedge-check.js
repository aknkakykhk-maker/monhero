...
//     「せっかく空いているスペースを活かせていない」「レーンが上まで見えるようにしてほしい」と
//     指摘された(2026-09-03)。
//   ・そこで、HUD(<header data-rhythm-hud>)をプレイエリアに重ねる絶対配置から、
//     プレイエリアの**外側・上**に独立した帯として置く形へ変えた。これで台形の上端の幅を
//     HUDの置き場所のために犠牲にする必要がなくなり、レーンをより上まで広く見せられる。
//
// この検査では、
...
//       いないこと
//   (4) HUD自身が全幅の不透明な背景パネルを持たない(文字に影を付けるだけ)こと
// を実測する。
//
...
// 以前の「台形の外側ウェッジへHUD本文を収める」方式でもHUDの実測下端は高さの約30%まで
// 達しており(rhythm-mode-note-perspective-check.js等の実測値)、この帯方式の実測(小さい
// 端末で最大24.8%)はそれより小さい。かつ以前は同じ高さ範囲で台形の幅も狭く犠牲にしていた
// (HUDの置き場所を作るため)のに対し、この方式ではHUDの帯より下は台形の幅を一切削らない。
const HUD_HEIGHT_LIMIT_RATIO=.26;
...
(async()=>{
  ...
  for(const size of SIZES){
    ...
    check(`  HUDの帯はmain全体の高さの${Math.round(HUD_HEIGHT_LIMIT_RATIO*100)}%以内に収まる(レーンの実質的な高さを削りすぎない)`,
