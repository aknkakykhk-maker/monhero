...
// コンボ数は段(rhythmComboTier)が上がるほど倍率で大きくなる(2026-09-12)。
// 倍率は transform なので、実際の描画範囲(getBoundingClientRect)がそのぶん広がる。
// ★ここを写さないと「500コンボまで伸びたときに台形へかぶる」を検査が見逃す。
//   いちばん大きい段の倍率を実装から取り出し、その最大の形で測る。
...
const headerHtml=headerJsx
  ...
  // 残したままだと本文として描かれ、HUDの高さ・幅が実際よりずっと大きく出る
  .replace(/\{\/\*[\s\S]*?\*\/\}/g,'')
  ...
  // ライフは段(rhythmLifeState)で見た目が変わる(2026-09-12)。位置と大きさを見る検査なので、
  // 幅を多く取るほう(DOWNの「DOWN」・「💔」)へ写して、いちばん厳しい形で測る
  ...
  //   丸ごと中へ抱え込み、測る箱の位置が実際とずれていた(2026-09-12に発見)。
  .replace(/<([a-z]+) ([^>]*?)\/>/g,'<$1 $2></$1>')
