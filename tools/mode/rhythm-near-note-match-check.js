...
// --- 過ぎているノーツが2つ以上あるときは、前のノーツから順に取る ---
// 2つめの時刻ちょうどに叩いても、1つめがまだ残っているならそちらへ渡す。
// ここで2つめを取ると、1つめは必ず取り逃しになる(＝1回の遅れで2つ崩れる)。
// 2026-09-05のユーザー指摘「1個目が過ぎて押したときに次のノーツのBad範囲内に
// 入っているとそっちを拾う」は、まさにこれが起きていた
for(const [label,gap] of GAPS){
  const got=hit([note(1000,0),note(1000+gap,1)],1000+gap);
  // 間隔が受付幅より広い場合、2つめの時刻には1つめはもう受け付けない(取り逃し確定)。
  // その場合だけは2つめが正しい
  const expected=gap<=WINDOW?0:1;
  ok(`${label}: 2つめの時刻に叩いたとき${expected===0?'、残っている1つめから先に取る':'は2つめを取る(1つめは受付幅の外)'}`,
    got===expected,got===null?'どれにも当たらない':`${got+1}つめ`);
}
