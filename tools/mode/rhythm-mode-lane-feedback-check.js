...
const must=["const setPressedLanes=lanes=>","data-rhythm-lane={lane}","liveLanes=new Set()","setPressedLanes(liveLanes)","backgroundColor=pressed?'rgba(34,211,238,0.30)'","boxShadow=pressed?'inset 0 0 26px","setPressedLanes([]);run.notes.forEach","run.audio?.stop();}runRef.current=null;setPressedLanes([])"];
