...
// 2026-09-07: 発熱対策で drop-shadow(全面SVGを毎フレームぼかし直す)をやめ、帯の外周に半透明の太い線(data-rhythm-slide-glow)を敷いて同じ発光に見せる
check('SLIDE帯は薄い塗りと控えめな発光でも縁を維持',source.includes('fill:rgba(168,85,247,.48)')&&source.includes('stroke:rgba(233,213,255,.56)')&&source.includes('[data-rhythm-slide-glow]{fill:none;stroke:rgba(168,85,247,.12)')&&source.includes('[data-rhythm-slide-glow][data-rhythm-slide-glow-inner]{stroke:rgba(168,85,247,.22);stroke-width:6}')&&!source.includes('drop-shadow(0 0 5px rgba(168,85,247,.38)'));
check('発光の線は帯の外周1本(区切りごとに引くと帯の中に横線が出る)',source.includes("const glow=document.createElementNS('http://www.w3.org/2000/svg','path');glow.dataset.rhythmSlideGlow='';")&&source.includes("const outline=`M${rightEdge.join('L')}L${leftEdge.reverse().join('L')}Z`;")&&source.includes('const segmentOffset=glows.length;'));
