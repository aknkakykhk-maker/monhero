...
assert(source.includes("applyNightmareSignedModifier(aptGradeToPct(apt[i] || 'C'), nightmare,waveNumber)"), 'monster aptitude must use the signed rule separately');
...
assert(source.includes('const recoveryDelta=applyNightmareSignedModifier(baseRecoveryDelta,specialRuleDifficulty,wave);'), 'recovery rule must apply after the base calculation');
