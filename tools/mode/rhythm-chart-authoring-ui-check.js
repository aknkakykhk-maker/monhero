...
ok('iPhone向け操作サイズを確保',ui.includes('min-h-[44px]')&&ui.includes('min-h-[56px]')&&ui.includes('playsinline')&&offsetUi.includes('min-h-[44px]')&&visualUi.includes('min-h-[40px]')&&visualUi.includes('max-h-[62vh]')&&holdResizeUi.includes("h-[18px]"));
...
ok('デバッグ編集自体はセーブデータへ直接書き込まない',!ui.includes('localStorage.setItem')&&!ui.includes('mh_rhythm_best')&&!offsetUi.includes('localStorage.setItem')&&!visualUi.includes('localStorage.setItem')&&!holdResizeUi.includes('localStorage.setItem'));
...
console.log('OK: 音ゲー譜面制作基盤（視覚エディタSTEP2-A HOLD伸縮・実機補正・描画/監視軽量化・EASYドラフト実プレイ）');