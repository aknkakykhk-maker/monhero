...
try{
  ...
  // ただし**譜面の形に乗って決まる段**があるので、候補ごとに少しちがう。
  //   ・同時押さえ / 同時スライド / 分岐・合流 … 置ける場所の数が形で変わる
  //   ・指の本数と叩き直しの間隔 … 落ちる打点の数が形で変わる
  // 実測は 620/622(2026-09-12) → 叩き直しの下限を入れたあと 624/617。
  // 「候補ちがいで密度そのものが変わる」のは行きすぎなので、2%までに抑える。
  const countGap=Math.abs(a.length-b.length);
  const allowed=Math.max(6,Math.round(a.length*.02));
  ok('ノーツの数はほぼ変わらない(音源で決まるので。見せ場と指の都合のぶんだけ動く)',
    countGap<=allowed,`${a.length} と ${b.length}（差 ${countGap} / 許容 ${allowed}）`);
  ...
  if(totals.length>=2){
    ...
    // ★版6(2026-09-26)からは、フリック・同時押しの場所を音の性格で決めるので、候補の違いはレーンの並びだけになり、
    //   どの候補も高い点に寄る(実測 monster_hero_theme 0.6点・dullahan 1.4点。版5では1点以上)。
    //   「種だけ」の0.2点とは見分けられる 0.5点を下限にする
    ok('候補どうしに点数の差が付く(選ぶ意味がある)',spread>=.5,`いちばん上と下で ${spread.toFixed(1)}点`);
  }
