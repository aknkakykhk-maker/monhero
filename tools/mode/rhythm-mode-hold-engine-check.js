...
check('入力ID別Mapとpointer captureで複数入力を独立管理',game.includes('activePointers:new Map()')&&game.includes('run.activePointers.set(input.inputKey,target.index)')&&game.includes('run.activePointers.get(input.inputKey)')&&game.includes('input.captureTarget.setPointerCapture(input.pointerId)'));
...
check('ポーズ・リスタート・中断でpointer管理を残さない',game.includes('run.activePointers.clear();run.activeTouchInputs?.clear();run.notes.forEach')&&game.includes('run.activePointers=new Map();run.activeTouchInputs=new Set();run.combo=0')&&game.includes('run.finished=true;run.paused=true;run.activePointers.clear();run.activeTouchInputs?.clear();run.audio?.stop()'));
