...
if(helper){
  ...
  // 収束率は .18 から .55 へ上げた(2026-09-03)。.18のときは台形の上端の左右に
  // 台形へ入らない黒い余白が大きく残り、「せっかく空いているスペースを活かせていない」
  // 「レーンが上まで見えるようにしてほしい」と指摘された。この余白は元々、HUD本文を
  // 台形の外側ウェッジへ押し込むために意図して確保していたが、HUDをプレイエリアの外側
  // (上の帯、<header data-rhythm-hud>)へ出したことで不要になった。
  // 上端が細く収束すること・判定線側が十分広いことは変わらず担保する(暫定値)。
