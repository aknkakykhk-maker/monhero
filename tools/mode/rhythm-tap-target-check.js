...
//
// いまの決め方:
//   ・過ぎている側は「いちばん後ろ」、まだ来ていない側は「いちばん前」まで絞る
//   ・その2つのうち、叩いた時刻に**近いほう**を取る(＝2つのちょうど中間で切り替わる)
...
for(const [now,want,why] of [
  ...
  [1060,1,'1つめは56ms=EXCELLENTへ落ちた。2つめは28ms前でMARVELOUS。段が良い2つめ'],
  [1088,1,'2つめちょうど'],
...
console.log('\n--- 巻き込みの再発防止 (実機の指摘そのもの) ---');
// ノーツA=1000ms / B=1100ms。Bを狙って1099ms(ほぼジャスト)に叩く。
// 以前はここで99msも前のAが取られ、狙ったBが巻き込まれてMISSになっていた
check('次のノーツをほぼジャストで叩いたら、そちらが取れる',
  hit([note(0,1000,4),note(1,1100,4)],5,1099)===1,
  '1.099秒に叩く→1.100秒のノーツ(‑1ms)。0.099秒前のノーツを取らない');
