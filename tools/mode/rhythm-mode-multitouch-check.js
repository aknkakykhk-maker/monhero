...
check('同時入力は単発処理ではなくbatch matcherを使う',data.includes('const rhythmMatchInputBatch=')&&game.includes('rhythmMatchInputBatch(run.notes,inputs,now,settings.judgmentTimingOffsetMs)')&&game.includes('inputStarts(starts)'));
check('iPhone向けnative Touch Eventsをpassive:falseで直接登録',game.includes("addEventListener('touchstart',syncTouches,{passive:false})")&&game.includes("addEventListener('touchmove',syncTouches,{passive:false})")&&game.includes("addEventListener('touchend',syncTouches,{passive:false})")&&game.includes("addEventListener('touchcancel',syncTouches,{passive:false})"));
