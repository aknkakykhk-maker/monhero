...
check('画面の大きさが変わったときも静的形状を組み直す',/const relayout=\(\)=>\{[\s\S]*?layoutFor\(next\);/.test(source)&&/\['resize','orientationchange'\]\.forEach\(type=>window\.addEventListener\(type,relayout/.test(source));
...
check('可変幅はTAP/HOLD描画だけに限定',source.includes("note?.type==='TAP'||note?.type==='HOLD'")&&source.includes('rhythmNoteVisualSpan(note,lane,yRatio,slideTravel?.chartNowMs)'));
// HOLD帯の幅は、途中幅変化(holdPoints)があるときだけ「その高さに当たる時刻の幅」を使い、
// 無いときは従来どおり始点の幅をそのまま投影する。
check('可変幅HOLDとSLIDEの帯・ENDバーprojection',source.includes('rhythmProjectSlideSpan(Number(point.lane),note,yRatio,point.timeMs)')&&source.includes('const bodyTopY=centerY-height;')&&source.includes('const yRatioAt=rhythmClamp01((bodyTopY+height*ratio)/rect.height);')&&source.includes('holdSpan?rhythmProjectSubLaneRange(holdSpan.subLane,holdSpan.subLaneWidth,yRatioAt)')&&source.includes(':variableHold?rhythmNoteVisualSpan(note,lane,yRatioAt)')&&source.includes("rhythmNoteIsSlide(note)?rhythmProjectSlideSpan(rhythmReleaseLane(note),note,endY,rhythmReleaseTargetMs(note)):rhythmProjectLane(rhythmReleaseLane(note),endY)"));
