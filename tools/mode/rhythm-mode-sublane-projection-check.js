...
check('薄い5本のサブレーン境界を共通boundaryで配置',game.includes('Array.from({length:5},(_,index)=><i key={index} data-rhythm-sublane-boundary="" />)')&&source.includes('const coordinate=index+.5')&&source.includes('rhythmProjectBoundary(coordinate,0)')&&source.includes('rhythmProjectBoundary(coordinate,1)'));
...
check('可変幅HOLDとSLIDEの帯・ENDバーprojection',source.includes('rhythmProjectSlideSpan(Number(point.lane),note,yRatio,point.timeMs)')&&source.includes('variableHold?rhythmNoteVisualSpan(note,lane,topY/rect.height):rhythmProjectLane(lane,topY/rect.height)')&&source.includes("rhythmNoteIsSlide(note)?rhythmProjectSlideSpan(rhythmReleaseLane(note),note,endY,rhythmReleaseTargetMs(note)):rhythmProjectLane(rhythmReleaseLane(note),endY)"));
