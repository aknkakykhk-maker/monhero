...
//   ④ 一覧では染色しない(重いため。実測は tools/ranking-dye-cost-check.js)
...
check('一覧では染色しない(一覧はふつうの画像のまま)',
  ...
  '一覧で染めると再着色が重すぎる(実測 tools/ranking-dye-cost-check.js)');
check('重さを実測する道具がある', fs.existsSync(path.join(root, 'tools/ranking-dye-cost-check.js')));
