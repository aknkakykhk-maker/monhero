// ミーアの本番染色マスクを、リポジトリに保存した正解見本と画素単位で比較する。
...
const LABELS = ['染色1（髪）', '染色2（茶色い上着・スカート・靴）', '染色3（緑の衣装・靴下）', '染色対象外'];
const ALPHA_THRESHOLD = 128;
const MIN_REGION_MATCH_RATE = 0.985;
...
(async () => {
  ...
  const imageUrl = imageForBaseId('Mia', images);
  const [source, reference, maskUrls] = await Promise.all([
    decodeDataUrl(imageUrl),
    decodeDataUrl(artSourcePath('dye-masks', 'mia-dye-mask.PNG')),
    dye.getDyeRegionMasks('Mia', imageUrl),
  ]);
  if (!maskUrls || maskUrls.length !== 3) throw new Error('ミーアの3色マスクを生成できませんでした');
