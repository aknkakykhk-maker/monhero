...
const FACE_BOXES = {
  ...
  // 髪の先端〜口ばしの下。左右は髪の広がりの実測(0.275〜0.726)にそろえる。
  // 背中の2本の剣は範囲へ入れない(丸くトリミングすると刃先が切れるだけで、
  // 顔が小さくなる)。頭の下に体が続くので、biasYを負にして丸の上寄りへ持ち上げる
  KENSHI_MOCCHI: [0.275, 0.045, 0.726, 0.40, -0.02],
};
