...
// 2026-09-22: 勇者特性の持ち主を traitOwnerOf が決めるようになった(ザンの連斬だけが見る)。
...
const makeActual = (rng) => Function('d', 'card', 'activeMon', 'mainHero', 'getPermaBuff', 'getTurnBuff', 'localGlobalComboAdd', 'slotIdx', 'Math', 'buildAttackHits', 'soulTraitAttackProfile', 'getMasuMon', 'tacticsSlotFlag', 'traitOwnerOf', `
  ...
  const tacticsExEffectAt = () => null; const tacticsExStyleAt = () => null; const TACTICS_EX_DUAL_HIT_REPEAT = 2; // タクティクスのEX(片手盾・二刀流)は既存5モードでは効かない
  ${act.text}
...
// ヒット列の順序(メイン → 連撃 → 全体連撃)と本数の代表例
{
  ...
  const hits = makeActual()(100, { type: 'unique', monId: 'Eiki' }, { id: 'Eiki' }, { id: 'Eiki' }, getPermaBuff, getTurnBuff, 0, 1, mathWith(() => 1), shared.buildAttackHits, soulTraitAttackProfile, getMasuMon, tacticsSlotFlag, traitOwnerOfFor({ id: 'Eiki' })).attackHits;
...
// 乱数を固定したときの合計が「予測の合計」と一致すること(確定会心のときだけ、メイン分の会心差が出る)を確かめる。
{
  ...
  const makeStun = () => Function('d0', 'card', 'stunMon0', 'mainHero', 'getPermaBuff', 'getTurnBuff', 'localGlobalComboAdd', 'slotIdx', 'Math', 'buildAttackHits', 'soulTraitAttackProfile', 'getMasuMon', 'tacticsSlotFlag', 'traitOwnerOf', `
  ...
  const perma = { comboDmgPct: 0, globalComboDmgPct: 0.1, critDmgPct: 0, critRatePct: 0 };
  const getPermaBuff = (k, def = 0) => (k in perma ? perma[k] : def);
  const zan = makeStun()(100, stunCard, { id: 'Zan' }, { id: 'Zan' }, getPermaBuff, (k, def) => def, 0, 1, mathWith(() => 1), shared.buildAttackHits, soulTraitAttackProfile, getMasuMon, tacticsSlotFlag, traitOwnerOfFor({ id: 'Zan' })).attackHits;
  ...
  const eiki = makeStun()(100, stunCard, { id: 'Eiki' }, { id: 'Eiki' }, getPermaBuff, (k, def) => def, 0, 1, mathWith(() => 1), shared.buildAttackHits, soulTraitAttackProfile, getMasuMon, tacticsSlotFlag, traitOwnerOfFor({ id: 'Eiki' })).attackHits;
