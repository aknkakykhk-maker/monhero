...
// ★swordSkill はタクティクスのEX「ソード・コンバージョン」(片手盾)だけが false、hitRepeat は二刀流だけが 2 にする。既存5モードは渡さないので常に true / 1
assert(source.includes("if (swordSkill && isUniqueOf('KenshiMocchi')) for (const rate of ATTACK_COMBO_RULES.kenshiUnique) combo(rate + comboDmgBonus);")
  && source.includes('kenshiExtraCombos = 0, comboFinalMultiplier = 1, swordSkill = true, hitRepeat = 1 }) => {'), 'ソードスキルの連撃');
