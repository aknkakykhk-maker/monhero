...
assert(source.includes("if (enemyId === 'Durahan') return quick ? bgmArrangement.quickDullahan : bgmArrangement.dullahan;"), 'Dullahan must use its dedicated BGM route');
assert(source.includes("if (enemyId === 'Moo' || currentWave === 10) return bgmArrangement.boss;"), 'debug Moo and normal WAVE 10 must use the selected boss BGM');
...
const startDebug = section('const startDebugBattle = (extreme=false) => {', '\n  };');
...
assert(/debugBattleRef\.current\s*=\s*false;[^\n]+setDebugBattle\(false\)/.test(source), 'normal battle entry must clear debug mode');
assert(/debugBattleRef\.current = false;[\s\S]{0,120}debugResultRef\.current = false;/.test(source), 'leaving a debug battle must clear synchronous flags');
