// 修行デバッグ試作版が通常データから隔離され、ノード移動・全効果・道具・デバッグ導線を備えることを静的確認する。
...
check('保存禁止バナーを常時表示',(source.match(/DEBUG・報酬や進行状況は保存されません/g)||[]).length>=5);
check('24ノードの接続マップ',/TRAINING_BEGINNER_NODES/.test(source)&&(source.match(/\['n\d+'/g)||[]).length>=24&&/trainingPaths/.test(source));
check('到達先と複数経路を選択',/chooseTrainingDestination/.test(source)&&/選択ルート確認/.test(source)&&/moveTrainingPath/.test(source));
