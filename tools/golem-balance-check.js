...
// --- ① 固有技が現実的に撃てること ---
// 看板の必殺技なので、WAVE1〜2の短い戦いでも1回は出せる回転であってほしい。
// 上限を5ターンにしているのは、実際に11ターンかかっていたゴーレムが
// 「実質撃てない」状態だったのに対し、他11種は全員1ターン目だったため。
const FIRST_UNIQUE_TURN_MAX = 5;
...
// --- ② 素の火力が1種だけ突出していないこと ---
// 差があること自体は種の個性なので止めない。止めたいのは「他が霞むほどの差」。
const ATK_TOP_RATIO_MAX = 1.6;   // ちから1位 ÷ 2位
const PLUS_ATK_RATIO_MAX = 1.5;  // 供モンのちから加算 1位 ÷ 2位
...
const NORMAL_HIT_RATIO_MAX = 1.6;
...
// --- ③ ゴーレムの調整値が意図せず戻っていないこと ---
// 今回の調整で決めた値。変えるときはこの検査も一緒に直す(=意図した変更だと分かるように)
const golem = MONS.Golem;
check('ゴーレム: ちから200', golem.baseAtk === 200, String(golem.baseAtk));
check('ゴーレム: 最大ガッツ90', golem.baseGuts === 90, String(golem.baseGuts));
...
// 変えていないもの(重量級というアイデンティティ)。調整のついでに崩れていないか見る
check('ゴーレム: ライフ650・丈夫さ160は据え置き', golem.baseHp === 650 && golem.baseDef === 160,
  `ライフ${golem.baseHp} / 丈夫さ${golem.baseDef}`);
check('ゴーレム: 合掌の倍率3.2(全種で最高)は据え置き',
  golem.unique.baseMult === 3.2 && ids.every(id => MONS[id].unique.baseMult <= 3.2),
  `${golem.unique.baseMult}`);
check('ゴーレム: 間合い適性 A/C/E/G は据え置き', golem.distAptitude.join('/') === 'A/C/E/G', golem.distAptitude.join('/'));
check('ゴーレム: 怪力の×1.2が実装に残っている', golemTrait === 1.2, `×${golemTrait}`);
check('ゴーレム: ちからは下げても全種で1位', sortedAtk[0].name === golem.name,
