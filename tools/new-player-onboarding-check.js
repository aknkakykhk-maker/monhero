...
for(const token of ["setGameState(onboarded ? 'HOME' : 'ONBOARDING')",'ゲームの目的','バトルの基本','育成・編成',"introIndex===2?'name'", "moveOnboarding('icon')", "moveOnboarding('confirm')",'!onboardingName.trim()','!onboardingIcon',"storeSet('mh_breeder_name'", "storeSet('mh_breeder_icon'", "storeSet('mh_onboarded',true", "storeGet('mh_onboarding_step'",'hasSavedName && hasSavedIcon'])assert(src.includes(token),token);
