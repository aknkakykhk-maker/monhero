...
上の「表示用に縮小した画像しか無いとき」以外に、**アンチエイリアスを保った正しい透過が原本にしか
無いとき**も置く。パンドラは、頭上の輪の内側を透過しようとした加工でアルファが2値化され、
半透明だった輪の金色が4772画素ぶん消えたまま公開されていた(コミット `ad1f3af`)。
大きさが同じでも中身は別物なので、原本を残して `art-source-fidelity-check.js` に見張らせる。
