...
for(const token of ['autoRepeatRef.current=next','setAutoRepeat(next)','if(next)setAutoBattleEnabled(true)','else autoRepeatStartingRef.current=false'])if(!repeatToggle.includes(token))fail(`∞周回切替に ${token} がありません`);
...
for(const token of ['onClick={()=>setAutoRepeatEnabled(!autoRepeatRef.current)}','aria-pressed={autoRepeat}','∞周回'])if(!source.includes(token))fail(`∞周回UIに ${token} がありません`);
