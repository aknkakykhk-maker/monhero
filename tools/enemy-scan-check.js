...
assert(src.includes('enemyActionProbabilities(scanEnemy,scanDist)'), 'SCAN shares action definitions');
assert(src.includes('setWaveScanPreview({enemy,wave:index+1,difficulty:safeDifficulty})'), 'W1-W10 cards open explicit preview');
assert(src.includes("scanBeforeBattle?'戦闘開始前'"), 'preview hides live distance');
assert(!src.slice(src.indexOf('(showEnemyInfo&&enemy||waveScanPreview)'),src.indexOf('showHeroInfo',src.indexOf('(showEnemyInfo&&enemy||waveScanPreview)'))).includes('Math.random'), 'SCAN consumes no randomness');
