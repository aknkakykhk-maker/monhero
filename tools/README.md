...
| `node battle/damage-parity-check.js` | 与ダメージの予測表示(`getAttackPredictedDmg`)と実処理(`processTurn` の攻撃ブロック)を本体から切り出し、乱数を固定して同じ入力で回し、合計が一致することを確かめる(勇者 5 種 × 攻撃者 × カード 8 種 × 補正 × 会心 = 2,560 通り)。一本化(STEP 5)の前後で等価であることの安全網。 |
