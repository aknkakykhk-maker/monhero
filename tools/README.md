...
| `node battle/eiki-debug-entry-check.js` | 正式実装前のモンスター(`debugOnly`)を「デバッグからどこを通れば触れるか」を実ブラウザで最後まで歩いて確かめる。BATTLE TEST の「4. デバッグ戦開始」は保存済みの編成をそのまま使うので勇者モン選択を通らないこと、正しい道順は **デバッグ設定 → BATTLE TEST → ⚔️ バトルモード → チャレンジ → 難易度を選ぶ → この難易度で挑戦** であること、その勇者モン選択にエイキが「DEBUG専用」の印つきで並ぶこと、通常プレイ(HOMEのバトル)の同じ画面には出ないことを見る。★横幅は測らない。このサンドボックスはTailwindを読めず `grid-cols-2` が効かないため、通常プレイの勇者モン選択でも同じだけはみ出す。 |
...
| `node mode/eiki-debug-check.js` | 新レアモンスター「エイキ」がデバッグ専用(`debugOnly`)のまま、正式実装前の確認だけができる状態かを見る。定義(能力値・成長値・間合い適性)、通常技9個と固有技9段階、血統(ザン×？？？)、勇者特性「桜花連舞」と固有効果「緋桜連華」の実装、EXTREME等の効果倍率の掛かり方、モーション(`eikiSakuraCombo`)の登録、立ち絵・顔アイコン・染色マスク・円盤石の参照先を確認する。円盤石は、円盤の外接矩形で正規化してから外周のリングを共通の土台 `DISC_STONE_BASE` と比べ、土台を別デザインへ描き直していないかを実測する(「どのくらい違えば同じ土台か」は既存の円盤石から決めるので、検査側に固定値を書かない)。あわせて**まだ表へ出ていないこと**――図鑑・種族チャレンジ・血統一覧(`dexMonsterList`)・通常ロースター・マーケット・更新履歴・ヘルプのどこにも出ず、保存データへも書き込まないこと――も見る。 |
