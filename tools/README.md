...
| `node image/clean-trapped-background.js <monster-hero/からの相対パス> [--write] [--max-area N] [--sat N] [--touch N]` | 立ち絵の**内側に閉じ込められた**白い背景を透過にする。`make-disc-icon-transparent.js` は外周からたどるので、星形の切り欠きや輪の内側のように装飾で囲まれた白は届かない(パンドラの頭上の輪・星飾りで実際に残っていた)。塊のまわり4px分を見て「囲んでいるのが彩度の高い装飾か」で肌・ハイライトの白と区別する(金色の飾り0.56〜0.61に対し髪のハイライト0.06〜0.14)。引数だけなら消す候補を並べるだけで、`--write` を付けたときだけ書き換える。実行後は `node image/compress-images.js <パス> --write`(canvasで書き直すとパレットPNGがRGBAへ膨らむため)と `node tools/build.js` を通すこと。 |
