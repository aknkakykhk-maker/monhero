...
| `node mode/species-challenge-battle-check.js` | STEP5Bの実戦接続を確認する。Base/Masu勇者、runで選んだWAVE1開始距離と勇者スロット、既存通常・極限難易度、WAVE2/4/6の事前選択供モン加入、二重加入防止、候補なし進行、保存抑止、終了後の選択導線を静的に固定する。 |
...
| `node mode/species-challenge-selection-flow-check.js` | 種族チャレンジの選択フロー(種族→難易度→勇者→開始距離→供モン→確認)を確認する。開始距離未選択の進行禁止、勇者変更時のリセット、4距離の適性表示と出撃確認、種族が主血統単位であること、同じモンスターを勇者・供モンで重複させないこと、モンスターの絵を共通枠(`MonsterArtFrame`)で切らずに収めること、選択SEに既存の`Audio_.se.tap`を使うこと、選択フローが保存キーを読み書きしないことを見る。 |
| `node mode/species-challenge-browser-check.js` | 種族チャレンジを実ブラウザ(iPhone相当の390×844)で通しで遊んでみる。デバッグ設定→BATTLE TEST→⚔️バトルモード→本番と同じBATTLE MODEカルーセルから入れること、種族選択(11血統)・難易度(14)・勇者・開始距離・供モン・出撃確認・バトルまで進めること、勇者変更で開始距離がリセットされること、選択距離のslotへ勇者が配置されること、重複制限、0体出撃、ランキング、横はみ出し、実行時エラーを確認する。★このサンドボックスはTailwind CDNへ出られないため、見た目そのものは確認できない。 |
