...
`node mode/rhythm-hud-wedge-check.js` は、プレイ画面のHUD(スコア・コンボ・ライフ等)が、レーンの台形に重ねる絶対配置ではなく画面上部の薄い帯(通常のflow)になっていることを実ブラウザで測る。以前は台形の外側ウェッジ(奥へ向かって狭くなる分だけ左右に空く三角形の余白)だけにHUD本文を収める形だったが、これは台形の上端を狭く保つ必要があり「レーンが上まで見えない」不具合(2026-09-03指摘)の一因だったため、常時表示する行(ランク+SCORE、LIFE+COMBO+ポーズ)を1行に絞りBEST・難易度・曲名を2行目の細い行へ回す薄い帯レイアウトへ変更した(過去にはHUDへ全幅の背景パネルを敷いて台形の頂点を覆ってしまい、遠近感が変わってプレイしづらくなる不具合も実機プレイで出している)。(1)HUDがプレイエリアより上にあり重ならないこと、(2)Safe Areaを二重に消費していないこと、(3)台形の最上部・中心ピクセルがHUDの背景に覆われずレーンの背景色のままであること(回帰チェック)、(4)HUDの帯がmain全体の高さの18%以内に収まる薄い帯であること、を4つの画面サイズ(320〜428px幅)で確認する。JSXのHUDからclassを取り出して手書きCSSへ写すため、知らないTailwindクラスが出てきたら失敗する。

`node mode/rhythm-landscape-hud-check.js` は、横画面(landscape)のHUD配置を実ブラウザで測る。rhythm-hud-wedge-check.jsと同じ考え方(HUDは台形に重ねない薄い帯)を横向きビューポート(844x390 / 926x428 / 667x375)で行う。Tailwindの`landscape:`バリアントは`@media (orientation: landscape)`そのものなので、縦画面用のCSSシムをそのまま流用できず(縦向きビューポートではlandscape:が発火しない)、シム自体をlandscape:プレフィックスに対応させたうえで横向きビューポートを渡す。(1)HUDがプレイエリアより上にあり重ならないこと、(2)台形の頂点がHUDに覆われないこと(回帰チェック)、(3)ポーズボタンが横画面でも44px未満に縮んでいないこと、(4)プレイ画面自身が左右のSafe Area(ノッチ)ぶんを`env(safe-area-inset-left/right)`で確保していること、(5)HUDの帯がmain全体の高さの22%以内に収まること、を固定する。isLandscapeという向き判定がrun・audio・スコア・コンボ・判定に一切触れていないことも確認する。
