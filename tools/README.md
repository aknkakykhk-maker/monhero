...
| `node battle/rpg-debug-check.js` | ダンジョンRPG戦闘テスト(デバッグ専用試作)の回帰チェック。RPGの純粋計算(1/10変換・1P＝ライフ+6/他+2・敵の自動配分・ダメージ式・ガッツ)と戦闘の進行を本体から取り出して実際に動かし、あわせて「入口はデバッグ設定だけ」「マスモンを使わない」「保存・報酬・ランキングへ触れない」「既存デバッグ戦と混ざらない」「通常バトルの式を変えていない」を確認する。 |
| `node battle/rpg-debug-layout-check.js` | ダンジョンRPG戦闘テストの3画面(編成・戦闘・結果)を、本体のJSXとCSSのまま実ブラウザの320x568・375x667・393x852で描いて測る。横スクロールが起きないこと、味方4体・敵4体でもコマンドが画面内に収まって押せる大きさがあること、編成と結果は下のボタンへ届きスクロールで全部追えることを数値で見る。確認用の画像を `tools/out/rpg-debug-layout/` へ出す。 |
