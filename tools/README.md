...
| `node monster/lineage-dex-check.js` | 血統データ(主血統×副血統)とモンスター図鑑を確認する。全プレイヤーモンスターに血統と図鑑説明があり欠損や綴り間違いが無いこと、純血/派生種/レアの判定、？？？(レア血統)を安全に扱えること、モンスターがいない血統に画像を作っていないこと、図鑑の一覧・詳細が全モンスターぶん組み立てられること(体数は実データから数える)、解放判定に既存の mh_unlocked_monsters を使い図鑑用の保存キーを増やしていないこと、技・能力を既存データから引いていること、解放済みモンスターの攻撃アクションプレビューが本番のatkMotion・既存FXを再利用し画面移動で安全に止まること、ヘルプ/更新履歴/仕様書の反映までを見る。 あわせて**図鑑が種族(主血統)順に並んでいること**も見る。同じ種族がひとまとまりで出るか、種族の並びが血統カタログの定義順か、種族の先頭がその種族を代表するモンスターか、絞り込みチップも同じ順かを、期待する並びを検査側へ書き写さずに性質として確かめる(以前は `ALL_PLAYER_MONSTERS` の定義順のままで、モンスターを足した順に並び同じ種族が離れて出ていた。2026-09-08・ユーザー指摘)。 |
