...
| `node ranking/profile-frame-sql-check.js` | `docs/sql/rankings/PROFILE_FRAME_*.sql` を、実際の PostgreSQL へ流して確かめる。本番と同じ順番でSQLを流した使い捨てのデータベースを作り(`RHYTHM_TOTAL_APPLY` → … → `RANKINGS_SCORE_BIGINT_APPLY`)、そのうえで予行演習 → 本番用 → 確認用を通す。列が増えること・ビュー4つと関数5つがフレームIDを返すこと・**集計の型と値を変えていないこと**・二重適用に強いこと・変な値を検査制約が弾くことを見る。SQLはCIで実行していないため「本番で流すまで分からない」ずれ(2026-09-15の `42P16` など)を、ここで拾う。PostgreSQL が入っていない環境では SKIP する。 |
