...
`node mode/rhythm-chart-v2-step1-check.js` は、あつ杯テーマとMonster Heroの実音源を同一アルゴリズムで再解析し、保存JSONとの完全一致（決定性）、NaN / Infinity / null、値範囲、250ms時系列、全尺終端、onset / downbeatの昇順、既存譜面・候補の非変更、ゲームruntime・STEP2以降への未接続を確認する。ffmpeg必須。
