...
`node mode/rhythm-landscape-travel-check.js` は、横画面で実機から指摘された「奥行きが短すぎて難易度が大幅アップ」への対応(`rhythmTravelMsHeightRatio`)を確かめる。judgmentタイミング・BPM・noteTime・判定窓・スコア式は変えず、ノーツが判定ラインへ着くまでの見た目の飛行時間(travelMs)だけをプレイエリアの実測高さに応じて伸ばす。縦画面の基準高さ(743px)以上では比率が1のままで既存の縦画面の挙動を変えないこと、横画面(844x390/926x428/667x375相当)では約1.86〜2.15倍まで伸びること、上限(2.2倍)で頭打ちになること、向き判定にだけ連動して同じ高さでも縦画面の小さい端末には適用しないこと、向き判定をrefで持ちプレイ中の回転でも即座に反映すること、判定条件そのものには一切関与しないことを固定する。
