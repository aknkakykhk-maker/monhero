...
| `node monster/kenshi-mocchi-debug-check.js` | 新レアモンスター「剣士モッチー」(モッチー×？？？)の**正式実装前**の確認。まだ表へ出ていないこと(`debugOnly:true`、図鑑・マーケット・更新履歴・ヘルプ・初期解放のどこにも無い、連れた周回がスコアやランキングに残らない、マスモン登録の対象外)と、染色が**5部位として成立していること**(`MASU_COLOR_REGION_HUES` が5要素・`EXACT_DYE_MASKS` が正本・マスクと立ち絵が同寸・マスクに5色ぶんの画素がある)、デバッグ画面(モンスター画像・染色確認／染色マスク編集)から確かめられることを見る。部位数の登録を間違えると例外が握りつぶされて「そのモンスターだけ染色が丸ごと効かない」状態になり、画面には何のエラーも出ないため、ここで機械的に拾う。正式実装のときは④を「表へ出ていること」へ反転させる(エイキの `mode/eiki-debug-check.js` と同じ流れ)。 |
