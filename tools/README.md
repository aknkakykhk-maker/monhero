...
| `node battle/rpg-debug-check.js` | ダンジョンRPG戦闘テスト(デバッグ専用試作)の回帰チェック。6ステータス(ライフ・ちから・丈夫さ・ガッツ・素早さ・運)の純粋計算(1/10変換・素早さ/運の初期値10・1P＝ライフ+6/他+2・敵の自動配分・ダメージ式・ガッツ・行動値・回避率・クリティカル率)と戦闘の進行(コマンド入力→行動順→実行→次ターン)を本体から取り出して実際に動かす。乱数は本体が受け取る rng を差し込んで固定するので「たまたま当たった」で通らない。あわせて「入口はデバッグ設定だけ」「マスモンを使わない」「保存・報酬・ランキングへ触れない」「既存デバッグ戦と混ざらない」「通常バトルの式を変えていない」も確認する。 |
| `node battle/rpg-debug-layout-check.js` | ダンジョンRPG戦闘テストの3画面(編成・戦闘・結果)を、本体のJSXとCSSのまま実ブラウザの320x568・375x667・393x852で描いて測る。横スクロールが起きないこと、6ステータスのステ振り行と会心・回避を含む9列の結果表がはみ出さないこと、味方4体・敵4体でも行動順8体の帯が折り返して収まること、コマンドが画面内で押せる大きさにあること、編成と結果は下のボタンへ届きスクロールで全部追えることを数値で見る。確認用の画像を `tools/out/rpg-debug-layout/` へ出す。 |
