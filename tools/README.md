...
| `node ranking/breeder-profile-check.js` | ランキングに出す「いまの見た目」(`breeder_profiles`)のしかけを見る。引き当ての規則(①ID ②名前が1人に定まるときだけ ③見つからなければ記録の値)を実装そのものを動かして確かめ、**記録(`rankings`/`bond_levels`)を書き換えていないこと**・表がまだ無い環境で壊れないこと・見た目を変えたその場で送ること・全ランキングの7つのエントリ生成が差し替えを通ることを見る。 |
| `node ranking/breeder-profile-browser-check.js` | 同じものを実ブラウザで通す。記録にはすべて**昔の見た目**を入れ、プロフィール表だけに**いまの見た目**を置いて、①IDが合えば改名していても新しい見た目で出る ②IDの無い古い記録も名前が1人に定まれば出る ③同名の人が2人いるときは当てない ④表に居ない人は記録のまま、の4つに分かれることを確かめる(`python3 -m http.server 8899` でルートを配信した状態で実行する)。 |
