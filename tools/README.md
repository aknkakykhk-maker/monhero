...
| `node battle/eiki-motion-check.js` | エイキの攻撃モーション(桜の花びら)を実ブラウザで測る。花びらのCSSと定義を本体から取り出して同じDOMへ当て、6枚が実際に動くか・攻撃モーションと同じ長さ(0.32秒)で1回だけ終わるか(常時アニメーションになっていないか)・終わったあと残らないか・枠からはみ出して横スクロールを増やさないか・動きを減らす設定の端末では流さず淡く出て消えるだけか(`eikiSakuraFade`)を確かめる。 |
