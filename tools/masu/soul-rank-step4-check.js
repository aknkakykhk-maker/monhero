...
ck('鉄壁/特殊防御/威圧/自動ガッツを実戦へ接続',app.includes('iceLockEnemyDamageMult*soulDamageRemaining')&&app.includes('rollUnifiedSpecialDefense(defenseTable,')&&app.includes("(!isTacticsMode(runMode)&&mainHero?.id==='Suezo')?40:0")&&app.includes('Math.max(0,gutsRecoveryRate)*soulBattleParty.autoGutsMultiplier'));
