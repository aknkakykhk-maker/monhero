...
vm.runInContext(`${source.slice(configStart, configEnd)}\nglobalThis.rules = { extremeSpecialRule, specialRuleDifficultyForRun, applyExtremeIntegerRule };`, context);

const { extremeSpecialRule, specialRuleDifficultyForRun, applyExtremeIntegerRule } = context.rules;
...
assert.strictEqual((source.match(/applyExtremeIntegerRule\(finalDmg,specialRuleDifficulty,'damageDealt'\)/g)||[]).length, 1, '与ダメージの共通経路へ1回だけ適用');
