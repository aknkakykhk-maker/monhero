...
> **イベントを開く手順は [`RHYTHM_EVENT_PLAYBOOK.md`](RHYTHM_EVENT_PLAYBOOK.md) にまとめてある。**
> この文書は仕組みの説明、あちらはやることの順番(2026-09-11・週末ゲリラ杯の形を雛形として残したもの)。
...
> | 4. 報酬と期間限定イベント | **一部完了**（2026-09-11。期間限定イベントと報酬の受け取りを実装。§7・§9） |
>
...
> **実例（2026-09-11）**: 公開の直後、合算だけが **18曲**（公開曲は17曲）になっていた。
> 原因は `monster_hero_theme_candidate`（体験版のいちばん最初に出していた
> 「Monster Hero 候補」）の記録が3件・1人ぶん残っていたこと。いまの `monster_hero` と
> **同じ音源で譜面だけが古いもの**で、曲を入れ替えたあとも記録は残り続けていた。
>
> 曲を入れ替える・下げるたびに同じことが起きる。**公開曲から外した曲は、そのとき
> 除外テーブルへも入れる**こと。`RHYTHM_TOTAL_VERIFY.sql` の「合算に入っている曲の一覧」で
> 気づけるようにしてある。
>
> なお「同じ曲の古い譜面」を公開曲へ戻して数を合わせるのは誤り。同じ曲が2つ並び、
> その曲を持っている人だけ合計が2曲ぶんになる。
...
#### 6.1.1 記録の「いつ」は `created_at`。送り直しでも遊んだ時刻のまま送る（2026-09-14）

週の集計は `rankings.created_at` で期間を絞る。この列の既定値は `now()` なので、
**行を入れた時刻**が記録の時刻になる。ふつうに遊んだときはそれで正しいが、
**送り直し**（送れなかった記録を次の起動でもう一度送るしくみ・2026-09-13追加）では
遊んだ時刻と入れた時刻がずれる。

実際に 2026-09-14 6:22 にアプリを開いただけで、先週以前の4曲ぶんが今週の週間へ載った
（ユーザー指摘「この時間は開いた時間なんだけど、それがスコアとして
何かしらの方法でカウントされてる？」）。

- 送り直すときだけ、端末が控えていた時刻（`entry.at`）を `created_at` に入れて送る
  （`rankingCreatedAtFromLocal` / `insertResentRankingRow`・`26-supabase.jsx`）
- ふつうの送信には付けない。これまでどおり DB の `now()` が入る
- 端末の時計が狂っている場合に備え、2024年より前・現在より1分以上先の値は**付けない**
  （付けなければ従来どおりの動きになる）
- `created_at` を指定できない環境（anon に列単位の制限がある等）では、
  **今週ぶんに限って**付けずに送り直す。先週以前は送らず端末に残す
  （付けて送ると遊んでいない週の合計へ足されるため。記録は消えない）
- 既にあるデータの `created_at` は書き換えない
- 確かめ方は `node tools/ranking/pending-resend-check.js` と
  `docs/sql/rankings/RHYTHM_RESEND_CREATED_AT_TEST.sql`

### 6.2 何を競うか — **累計スコア方式**（2026-09-13・ユーザーが決めた）
...
数え方は**累計**。その週に出した記録を**ぜんぶそのまま足す**。
同じ曲を何回遊んでも、そのつど積み上がる。

| | 数え方 | 測っているもの |
| --- | --- | --- |
| 総合（常設） | 曲ごとのベストを全曲ぶん合計 | **うまさ** |
| 週間 | その週に出した記録をぜんぶ足す | **やりこみ量** |
| イベント | 対象曲のベスト＋回数ボーナス | 対象曲の腕前＋通った回数 |

- **重み付けはしない。** 難易度ごとに満点が違う（EASY 60万／NORMAL 70万／HARD 80万／
  EXPERT 90万／MASTER 100万）ので、足すだけで難易度差は自然に付く
- **上限は付けない**（イベントの回数ボーナスと同じ考え方）
- **回数ボーナス（`playBonus`）は週間には付けない。** 累計そのものが回数を反映しているので二重になる
- 短い曲を何度も回すのが効率的になるが、これは**やりこみを測る以上どうしても付いてくる性質**。
  上限を付けない以上避けられないので、そのまま受け入れる（2026-09-13・ユーザーへ説明済み）
- 集計はサーバーの `public.rhythm_week_score_totals(from_at, to_at)`
  （`docs/sql/rankings/RHYTHM_WEEK_TOTAL_APPLY.sql`）。**対象曲の引数を持たない**
  （週間は公開曲すべてが対象なので、期間だけで足りる）
- 除外曲（`rhythm_ranking_song_exclusions`）はこれまでどおり外す

### 6.2.1 報酬（2026-09-13・ユーザーが決めた）

**1位から10位まで。** 主軸は新アイテム**勇者の証片**。
数字は10行書き写さず、**式で出す**（幅を変えるのが1か所で済む）。

    勇者の証片    = 11 − 順位      (1位=10片 … 10位=1片)
    虹のプシュケー = 片 × 50
    ダイヤ         = 片 × 3,000

**参加報酬**は「その週に**3回遊ぶ**」で成立（週間に対象曲は無いので、イベントの
「何曲遊んだか」ではなく**何回遊んだか**で見る）。中身は10位より軽く、
🎖️証片×1 ／ 💗虹のプシュケー×30 ／ 💎ダイヤ×2,000。順位報酬とは別なので両方もらえる。

**勇者の証片**（`hero_proof_shard`）

- マーケットで**20個 → 勇者の証1個**（`HERO_PROOF_SHARD_PER_PROOF`）。20個未満でも貯まる
- 入手先は週間ランキング（順位報酬・参加報酬）と、**クイックGODの1周クリアごとに1個**
  （2026-09-13・ユーザー指示。∞周回とモンヒロビート換算の周回にも周回数ぶん入る。
  詳細は `docs/spec/BATTLE_SYSTEM.md`「クイックの極限難易度」）
- 所持数は既存の `mh_owned_items` の中。**新しい保存キーは作らない**（CLAUDE.md ⑦）
- 証そのものではないので、魂格進化には直接使えない（先に証へ交換する）
- プロフィールの「アイテム」に並ぶ。マーケットの上部にも、ダイヤと同じように所持数を出す
  （2026-09-13・ユーザー指示「マーケットにプシュケーとか証片もいくつあるかダイヤみたいに表示がほしい」）

**受け取り**はイベントと同じ仕組み。週が切り替わったあと、次にゲームを開いたときに出る。
受取期限は2週間。受取済みフラグは**イベントと同じ配列**（`mh_rhythm_event_reward_v1`）へ
週ID（`weekly_YYYY_MM_DD`）で入れる。イベントのIDとは形が違うので取り違えない。

> ⚠️ **`RHYTHM_WEEKLY_REWARD_FROM_MS` より前に終わった週は対象にしない。**
> 入れておかないと、公開した瞬間に「先週・先々週ぶん」がまとめて配られる。

### 6.3 部門

> **2026-09-11・ユーザー指示で取り消した。**
> 「週間ランキングはそれのみにして、対応曲があるのはイベントのほうにして」。
> **週間に対象曲は無い。公開曲すべてが対象で、部門も分けない1本のランキング。**
> 中身は「総合」（§3）とまったく同じ計算で、違うのは数える期間だけ。
>
> | | 数える期間 | 対象曲 | 部門 |
> | --- | --- | --- | --- |
> | 総合 | ずっと | 公開曲すべて | 1つ |
> | 週間 | 今週だけ（月曜5:00 JST区切り） | 公開曲すべて | 1つ |
> | イベント | イベントの期間 | イベントが決めた曲 | 対象曲＋総合 |
>
> **対象曲・部門を持つのはイベント（§7）だけ**になった。以下は経緯として残す。
...
### 6.4 対象曲の選び方（**週間では廃止。イベントの話として読む**）
...
### 6.5 ~~対象曲はローテーションで決める~~（**廃止・2026-09-11**）

> 週間が対象曲を持たなくなったので、ローテーション（`RHYTHM_WEEKLY_ROTATION`）は消した。
> 曲を足しても週間のためにやることは無い（公開曲に足せば、そのまま週間の対象にも入る）。
> 以下は経緯。
...
- ~~同時に成立するイベントは**1つまで**にする。週間と期間限定を重ねない~~
  **→ 取り消し（2026-09-11・ユーザー指示「週間ランキングとイベントランキングは
  別々に作ったほうがいい」）。週間と期間限定は別のタブで、同時に動く。**
  報酬が付くのは期間限定だけなので、「どちらの報酬か分からなくなる」ことは起きない。
  週間は常設で止まらず、期間限定は開催しているときだけタブが増える
- ~~開催していないときは、ランキング画面のイベントタブそのものを出さない~~
  **→ 取り消し（2026-09-14・ユーザー依頼「イベントタブを常設して、前回のランキングと
  今回のランキングを見れるようにしたい。前回や今回がない場合はそのような文言をいれとく」）。**
  詳しくは §7.3
...
### 7.1 告知画像（2026-09-11・ユーザー指示「告知用画像を表示できる仕組みを作って」）

イベント定義に `banner: 'images/events/…'` を書くと、その画像を2か所へ出す。

| 出す場所 | 見え方 |
| --- | --- |
| ランキングの「イベント」タブ | 期間の帯の上に、横幅いっぱい |
| 曲えらびの開催中の案内 | みゅあの吹き出しの下に、横幅いっぱい |

- **書かなければ何も出ない。** 画像が無いイベントでも画面は壊れない
- 読めなかったときは黙って消す（壊れた画像のアイコンを残さない）。
  綴り間違いは `tools/image-asset-check.js` が先に捕まえる
- **入れる前に軽くする**（`CLAUDE.md` ⑥-2）。目安は次のとおり

| 項目 | 目安 |
| --- | --- |
| 置き場所 | `monster-hero/images/events/<イベントid>.jpg` |
| 形式 | JPEG（`sharp` の `quality:80` / `mozjpeg`） |
| 横幅 | 1080px（縦は自由。横長のバナーを想定） |
| 大きさ | 150KB以内（検査は200KBで止める） |

- キャッシュキー（`?v=`）は `node tools/build.js` が中身のハッシュから打つ。
  画像を差し替えたら必ずビルドし直す（しないと古い画像が端末に残る）
- **起動時には読まない。** `index.html` の `SIZES` へ入っていないことを検査で見張る
  （告知画像のために起動が重くなってはいけない）
...
### 7.3 イベントタブは常設し、中で「今回／前回」を切り替える（2026-09-14・ユーザー依頼）

> 「モンビーのランキングでイベントタブを常設して、前回のランキングと今回のランキングを
> 見れるようにしたい。前回や今回がない場合はそのような文言をいれとく」

これまではイベントを開催しているあいだしかタブを出していなかったので、**終わった瞬間に
ランキングの画面から結果へ辿れなくなっていた**（プロフィールの「これまでの記録」からは
見られるが、順位を見に来た人がそこへ行き着けない）。

- `{id:'event',label:'イベント'}` は公開フラグ（`rhythmWeeklyRanking`）が立っていれば**常に出す**
- タブの中に「今回」「前回」の2つを置く
  - **今回** … `rhythmLimitedEventAt(now)`。開催中のイベント
  - **前回** … `rhythmPreviousLimitedEvent(now)`（`rhythmHistoryEvents(now)[0]`）。
    いちばん最近に終わったイベント。集計は履歴（`kind:'history'`）とまったく同じで、
    渡す期間が違うだけ
- 押されるまではおまかせ。開催中なら「今回」、開催していなければ「前回」を先に見せる
- **無いときは文言を出す**
  - 今回が無い … 「いま開催しているイベントはありません」。
    `rhythmNextLimitedEvent(now)` で次の予定が分かるときは、その日時と名前も出す
  - 前回が無い … 「まだ終わったイベントがありません」
- 一覧の置き場所は `prevEvent` として**別に持つ**（`rhythmEventRanking.prevEvent`）。
  「これまでの記録」の画面（`history`）と行き来しても、お互いの順位を上書きしない
- **どちらも見るたびに数え直す**（`CLAUDE.md` ⑥-4）。開きっぱなしの端末でも、
  開始・終了の時刻をまたいだ瞬間に中身が入れ替わる
- 確かめ方は `node tools/mode/rhythm-event-window-check.js`
...
> 絆Lv・総合力ランキングが読む `bond_levels` テーブルには、この列を足していない。
> あちらは1人1個体1行の別テーブルで、フレームを出すには同じ手当てをもう一度することになる。
> 必要になったときに別の作業として足す（それまで絆Lv・総合力の行は「フレームなし」で出る）。
...
- 各曲ランキングは対応する種族の**超越の実**、**週間総合は虹の超越の実**
  （2026-09-11 の週末ゲリラ杯では、ユーザー指示で**総合を勇者の証**にした。
  イベント定義の `totalReward` に `'heroProof'` / `'rainbowFruit'` を書いて選ぶ）
- プシュケー数は暫定値。実装前に経済バランスを見直す
- 3曲側でどの種族の超越の実を使うかは**実装前に対応表を確認する**。会話だけから割り当てない

### 9.1 受け取り方（**実装済み・2026-09-11**）
...
- **受け取れるのは終了から2週間まで**（ユーザーが決めた）。順位は終了した時点で固まるので、
  遅れても内容は変わらない。期限を切ってあるのは、古いイベントの問い合わせが溜まらないようにするため
- 問い合わせるのは**各部門の上位5件だけ**。報酬は5位までなので、それより下は見なくてよい
- **入賞していなかったときも受け取り済みにする。** そうしないと、受取期限のあいだ
  毎回起動のたびに問い合わせ直すことになる
- **通信に失敗したときは受け取り済みにしない**（次の起動でやり直す）
- 受け取りは**先にフラグを保存してからアイテムを足す**。逆順にすると、途中で終了したときに
  二重に配りうる（`CLAUDE.md` ⑦）
- 画面は**重ねて出すだけ**で、新しい `gameState` は増やしていない（§10.1 と同じ理由）
- 報酬は既存の `mh_owned_items` の個数として足す。虹のプシュケーも同じ入れ物
  （`rainbow_psyche`）なので、新しい保存の形を作っていない

### 9.2 参加報酬（**一部確定・2026-09-11**）

**成立条件は「対象曲をすべて遊んだら」**（2026-09-11・ユーザーが決めた）。
付与単位は**イベントで1回**（部門ごとではない）。個数は `RHYTHM_MODE.md` §20 の候補
（ダイヤ×3,000 / プシュケー×50）をそのまま使っている。

- イベント定義の `participationReward: { songs, gold, psyche }` に書く。
  書かなければ参加報酬は無し（週間には付けていない）
- 成立の判定は**総合部門の自分の行の `song_count`**（その期間に何曲ぶん記録があるか）で行う。
  上位5件に入っていなくても成立するので、自分の行だけを別に取りにいく
- 入賞していなくても、参加報酬だけで受け取り画面が出る（見出しが
  「ご参加ありがとうございました！」に変わる）
- ダイヤは `mh_gold`、虹のプシュケーは `mh_owned_items` と入れ物が別なので分けて足す

**週間ランキングには報酬を付けない**（2026-09-11・ユーザーが決めた）。
毎週配ることになるので、インフレの影響を見てから決める。
...
| 総合 | ブリーダー別 全曲合算（§3） | 常時 |
| 週間 | その週の対象曲＋総合の部門（§6） | 常時 |
| イベント | 期間限定の対象曲＋総合の部門（§7） | 開催中だけ |

> **2026-09-11・ユーザー指示で「週間」と「イベント」を別のタブに分けた。**
> もとは1つのタブを差し替える形だったが、それだと期間限定のあいだ週間が見られない。
> 状態も `kind`（`'weekly'` / `'limited'`）ごとに分けて持つので、片方を読み込んでも
> もう片方の一覧は消えない。
...
| `tools/mode/rhythm-weekly-reward-check.js`（**実装済み**） | 週間の累計方式、順位報酬の式（1〜10位）、参加報酬（3回）、勇者の証片と交換、始める前の週へ遡らないこと、週間に回数ボーナスを付けないこと（§6.2・§6.2.1） |
...
2. ✅ ユーザーが Supabase の画面から適用した（2026-09-11。予行演習→本適用とも、件数1432のまま・RLS・ポリシー・権限に変化なし）
3. ✅ `mh_breeder_id_v1` を作り、モンビーの送信へ `breeder_id` を足した。
   列が無い環境でも通るフォールバック付き（§4.7）
4. ✅ 画面の見た目は変わらない

> このフェーズを先にやったのは、合算の正しさがIDの有無で決まるため。
> 適用が済んだので、**これ以降の記録にはIDが付く**。それより前の記録はNULLのままで、
> 合算のときに §4.4 の決めごとで引き継ぐ。

### フェーズ2 — ブリーダー別 全曲合算ランキング（常設）

1. ✅ 集計ビュー・除外テーブル・索引・権限の適用SQLを用意した（§8.2・§8.3・§8.5〜§8.8）
2. ✅ ユーザーが Supabase の画面から適用した（2026-09-11。13人・1位16,485,177点=達成率97%が並ぶことを確認）
...
6. ✅ 公開フラグ `RHYTHM_TOTAL_RANKING_PUBLIC_RELEASE` を `true` にして公開した（2026-09-11）
...
- ~~未受取報酬をいつまで受け取れるか~~ → **終了から2週間**（2026-09-11 決定・§9.1）
- ~~参加報酬の成立条件と付与単位~~ → **対象曲すべて／イベントで1回**（2026-09-11 決定・§9.2）
- ~~週間ランキングに報酬を付けるか~~ → **付けない**（2026-09-11 決定・§9.2）
- 3曲側の超越の実の種族対応表（§9）。※2026-09-11 の週末ゲリラ杯ぶんは、
  ユーザーが対象曲ごとに直接指定した（Monster Hero＝スエゾー種／風がそよぐ場所＝モッチー種／
  Close To Your Heart＝ライガー種）。**曲と種族の対応は毎回ユーザーに確認する**
- プシュケーの個数（暫定値のまま）
