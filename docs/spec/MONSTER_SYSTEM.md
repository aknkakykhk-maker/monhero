...
- **詳細**: 上半分に立ち絵（左右ボタンと横スワイプで前後へ移動）、下半分に情報カード（名前・血統・区分・図鑑説明）と
  ...
  - 攻撃アクション … 解放済みモンスターだけ、立ち絵枠から代表的な通常攻撃を手動再生できる。`atkMotion` と
    `attackMotionAnimation`、エイキの桜・剣士モッチーの二刀斬撃・パンドラの分身雷撃など既存の本番演出をそのまま使い、
    図鑑専用のモーション定義は持たない。再生は表示とSEだけで、バトル状態・報酬・保存は変更しない。
    前後移動や図鑑一覧へ戻ると再生を中止し、`prefers-reduced-motion` では大きな移動アニメーションを流さない。
...
`dexMonsterList` が決める。**主血統（種族）ごとにまとめる**。

1. 血統の並びは「`ALL_PLAYER_MONSTERS` でその血統が最初に出てくる順」
2. 各血統の先頭は、その血統を代表するモンスター（カタログの `monId`）
3. その中は `ALL_PLAYER_MONSTERS` の定義順

血統カタログ（`MONSTER_LINEAGES`）の定義順**ではない**。カタログではプラントが
`dragon` / `joker` より後ろに置かれているため、そちらを使うとプラント種だけが末尾へ動き、
図鑑の絞り込みチップを共有している**種族チャレンジの種族タブと超越の実（アイテム欄）の
並びまで巻き添えで変わる**（実測で確認）。初出順なら、それらの並びは1つも変わらないまま
図鑑だけが種族順になる。

一覧・詳細・「◯ / 20」の番号・左右の移動はすべて同じ関数を通るので、並びがずれることはない。
主血統のしぼりこみチップ（`dexMainLineages`）も図鑑の並びから作るため、自動で同じ順になる。
並び順は表示だけの話で、解放状況の保存（`mh_unlocked_monsters`）は種のidを持つので影響しない。

以前は `ALL_PLAYER_MONSTERS` の定義順そのままで、モンスターを足した順に並んでいた。そのため
同じ種族が離れて出ていた（剣士モッチーがモッチー・ミタラシと離れ、エイキの隣に出ていた。
2026-09-08・ユーザー指摘）。並びが崩れていないことは `tools/monster/lineage-dex-check.js` が
機械的に見張る（期待する並びを検査側へ書き写さず、「ひとまとまりか」「カタログの順か」
「先頭は代表モンスターか」「顔ぶれが変わっていないか」を性質として確かめる）。

### 文字組み

日本語の折り返しは共通CSSの `body { line-break: strict; }` で厳しい禁則にしている。
既定のままだと長音「ー」や小書き仮名「っ」が行頭へ出る（`<html lang="ja">` と両方そろって
初めて効く）。図鑑の「基本」タブの値は右揃えにしない。1行に収まる短い値は
`justify-between` が右端へ寄せるので見た目は変わらず、折り返す長い値だけ行頭がそろう。
「特性の効果」のように必ず長くなる値は、呼ぶ側が `{block:true}` を渡してラベルを上に置く。
`tools/text/japanese-linebreak-check.js` が実ブラウザで折り返し位置を測って見張る。
...
魂格の数値・特性・UI・合体・オーラの正式仕様は [SOUL_RANK_SYSTEM.md](./SOUL_RANK_SYSTEM.md) を正本とする。

- 超越済み魂格0のLv上限は500。魂格Ⅰ〜Ⅴで600 / 700 / 800 / 900 / 1000へ段階解放する。
- Lv500→501以降は魂格専用XP式を使い、Lv501〜1000の初到達だけ魂格Pを得る。転生後の再到達では二重取得しない。
- 魂格Pは個体固有の18種の魂格特性へ振り分ける。未使用Pは保存せず、最高初到達Lvと振り分けから導出する。
- 転生では魂格段階・最高初到達Lv・魂格特性を維持する。転生回数そのものと「転生×N」表示は残す。
- 副モンの魂格が主より高い場合だけ、合体確認で「魂格を引き継いで合体」を選べる。不足段階の通常進化コストを合算して支払い、主の魂格段階 / Lv上限を先に解放してから合体XPを加算する。主の魂格特性は維持し、副の最高到達Lv・魂格P・特性振り分けはコピーしない。
- 魂格Ⅰ〜Ⅴでは従来の「超」バッジを青 / 黄 / 緑 / 赤 / 虹の魂格バッジへ置き換える。魂格0では従来の「超」を維持する。
- 転生回数で色が変わる既存オーラは廃止予定で、正式な5色透過PNG保存後に魂格段階だけを参照する魂格オーラへ置き換える。画像が未保存の段階では存在しない参照先を追加しない。

回帰確認: `node tools/masu/soul-rank-step1-check.js` ～ `soul-rank-step5c-check.js`、`node tools/boot/soul-rank-backup-check.js`。
