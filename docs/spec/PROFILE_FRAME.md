...
#### 呼び分けはアシストカードの3段階にそろえる（2026-09-16）

名前は**その助手のアシストカードの3段階**（`BREEDER_EVO_NAMES`）から取る。
`「助手名・段階名」`の形で、カード名から「の」を抜いたもの。

| 助手 | アシストカード | 枠 |
| --- | --- | --- |
| みゅあ | みゅあの愛 / 深愛 / 慈愛 | みゅあ・愛 / 深愛 / 慈愛 |
| きき | ききの応援 / 本気 / 全力全開 | きき・応援 / 本気 / 全力全開 |

**ももすけにはまだアシストカードが無い**ので、先に枠のほうで3段階を決めた
（おねだり → だだこね → 独り占め）。カードを実装するときは、この3段階をそのまま
`BREEDER_EVO_NAMES.momosuke` へ入れる。あとから決め直すと枠とカードで呼び名がずれる。
