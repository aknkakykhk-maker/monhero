...
### KI-011: Undine(ウンディーネ)の染色マスクの塗り残しが上限を超えている

| 項目 | 内容 |
| --- | --- |
| ステータス | 未着手 |
| 優先度 / 種別 | P3 / 不具合(データ) |
| 影響 | Undine を染色すると、輪郭に元の色の縁が残り、腕と体の境目に元の色の筋が残る可能性がある(`node tools/image/dye-edge-check.js` で 輪郭 22.89%(上限 5%)/ 内側 2.18%(上限 0.2%)) |
| 再現環境 | 全環境。`monster-hero/images/monsters/undine.PNG`(1024px)と 512px の部位マスク 3 枚 |
| 再現手順 | 1. `node tools/image/dye-edge-check.js` を実行 2. Undine の行が NG になることを確認 3. ゲーム内で Undine を濃い色に染めて輪郭を見る |
| 期待結果 | 輪郭 5% 以下・内側 0.2% 以下(他のモンスターと同じ水準) |
| 実際の結果 | 上記の数値。コードではなくマスク画像(部位定義)の品質の問題 |
| 回避策 | なし(見た目だけ) |
| 完了条件 | マスクを作り直すか部位定義の色相を合わせ、`dye-edge-check` が Undine を含め全件 OK になること |
| 関連 | `docs/refactor/BASELINE_2026-09.md`(2026-09-06 の監査で検出) |
...
### KI-008: GOD難易度の助手セリフ(extremeDifficultyGuides)がみゅあ/きき混在検査に落ちる（2026-09-06 解決確認）

- 解決内容: 記載当時は `node tools/assistant-bond-check.js` の `godDifficulty` シーンが NG だったが、その後の助手セリフ整備で解消していた。2026-09-06 に main(#1113)で再実行し全件 OK を確認したため解決済みへ移す。
- 検証: `node tools/assistant-bond-check.js`（全件 OK）
- 関連: PR #899、`docs/refactor/CURRENT_ARCHITECTURE.md` §16 D-04
