...
> 2026-09-06 の調査結果: `MonsterHeroGame` の `setTimeout` 58 箇所の内訳は「参照に保存して止められる 10 / effect の後始末で止める 8 /
> 投げっぱなしで state を更新する 40」。投げっぱなしの中には「WAVE リザルトへ進む」「処理中フラグを戻す」「演出を消す」など、
> **画面を離れても実行しないと困るもの**が多く(止めるとフラグが立ったまま操作できなくなる)、一律に「画面を離れたら止める」仕組みは危険。
> 登録簿を作るなら、タイマーごとに「画面専用(止めてよい)」「進行(止めてはいけない)」を宣言する形にし、1 本ずつ判断して移す。
