# 検査ベースライン(2026-09-06 記録 / 2026-09-11 にトリアージ済み)

> **2026-09-11 追記 — STEP 1(安全網)のトリアージが終わった。**
> 検査は 336本 → **407本**に増え、NG は 51本 → **12本**になった。
> 何をどう直したかは下の「## 2026-09-11 のトリアージ結果」を見ること。
> 以下の 2026-09-06 時点の記録は、そのときの状態として残してある。
...
`node tools/run-checks.js --area all`(407本)で **56本**が落ちていた状態から、
**46本を直して 12本**にした。PR #1273 / #1275。
...
| 分類 | 本数 | 何をしたか |
| --- | ---: | --- |
| 検査の見かたが実装から取り残されていた | 44 | `harness` の `readAppSource()` / `screenSource()` へ向け直す、綴りを今の形に合わせる、「1か所であること」を「見つかったすべてが正しい形か」に変える |
| 実データの書き間違い | 2 | 週末ゲリラ杯の告知2行の略称、作業メモ12件の `status:'new'` |

**判定の厳しさは1つも下げていない。** 緩めずに直せないものは直さずに残した。
逆に、直したついでに**確認を増やした**ものがいくつかある
(`changelog/dev-entry-check` の「公開後の項目を作業メモにしていないか」、
`mode/rhythm-name-check` の「愛称の会話で正式名称へ言い直しているか」、
`assistant/momosuke-check` の顔画像を先頭64文字ではなく中身ぜんぶで見比べる、など)。

### 直したときに出てきた「隠れていたもの」

- `boot/ui-preferences-check` は**起動すらできていなかった**(公開フラグを手書きで並べていた)。
  起動できるようにしたら、同じ検査の `pick` が宣言を1行しか取れず、3行に伸びた
  `releasedForPlayers` の1行目だけを拾って**実在する項目をすべて「出さない」と判定**していた。
  件数の確認が全部0件で素通りする形だった
- `layout-consistency-check` は画面の切り出し前、`gameState` の位置から 9000文字の窓で見ていたため、
...
### 最後の1本も直した — 染色マスク(2026-09-11)

`image/dye-edge-check` の「ウンディーネの輪郭の塗り残し 22.89%(上限5%)/ 内側 2.18%(上限0.2%)」。
**手作業なしで直した。** これで NG は 56本 → 0本。

あわせて、ユーザーから「モッチーの目が染色範囲に入っていて、白などで染めると目が薄くなる」
という指摘があり、同じ回で直した。

#### 何が原因だったか(3つ疑って2つを否定した)

| 疑ったこと | 実測 | 結論 |
| --- | --- | --- |
| 目を染色対象にしていないから数えられている | 目は `UNDINE_EYE_BOXES` を `notBbox` に宣言済みで、検査も `notBbox` の範囲は数えていない | 違う |
| 部位マップが 256×384 と粗いから | 見本PNG(原寸1024×1536)をそのまま使っても輪郭 30.18%。512×768 で 30.10%、256×384 で 29.79% | 違う。解像度をいくら上げても変わらない |
| 実装側でマスクを広げれば埋まる | 本体の中で 8px 広げて 11.77%、32px で 5.79%、64px でようやく 3.07% | 筋が悪い |

残っていたのは**髪の毛先・腕のヒレ・尾びれのふち**だけで、内側の2.15%も毛先の細いところに
集まっていた。つまり**見本マスクがふちを塗っていなかった**。

#### どう直したか

`node tools/image/undine-region-rebuild.js --write` を作って、2段階でやる。

1. 見本の塗り残しを**いちばん近い部位で埋める**。埋める相手は `dye-edge-check` が
   塗り残しとして数える画素(彩度0.1以上・明度0.25以上)だけなので、
   白いハイライト・黒い輪郭線・目(`notBbox`)は触らない
2. 埋めた見本から `UNDINE_EXACT_REGION_2BIT` を作り直す。縮小はふつうの多数決

**この2つ目が効いた。** 埋めただけでは原寸で 0.01% でも、256×384 へ縮めると 17.50% へ戻る。
ところが実装側の `_buildHiResMaskUrls`(にじみへの塗り足し)と噛み合うと **0.57%** まで落ちる。
つまり「見本を埋める」と「実装の塗り足し」は片方だけでは足りず、両方そろって初めて届く。

結果: 輪郭 22.89%→**0.57%** / 内側 2.18%→**0%**。
大きさは 256×384 のままなので、**起動時に読むJSは1バイトも増えていない**。

#### モッチーの目

正式マスク(`images/monsters/mocchi-dye-mask.PNG`)で目が丸ごと染色①(体)に入っていた。
`node tools/image/mocchi-eye-unmask.js --write` で透明にした。位置は手で書き写さず、
元絵の暗い塊の上位2つ＋内側の穴(白いハイライト)＋アンチエイリアス3px で機械的に決める
(左右の高さと面積が釣り合わなければ止める)。
外した範囲は `MOCCHI_EYE_BOXES` を `notBbox` として宣言したので、
`dye-edge-check` の「内側の塗り残し」にも数えられない(0.03%)。

どちらのツールも**何度実行しても結果は同じ**(すでに直っていれば0件と出る)。

#### 測り方

同じ切り分けをやり直すときは、`tools/harness.js` の `loadDyeModule()` で
`getDyeRegionMasks(id, url)` と `MASU_COLOR_REGION_HUES` を取り出し、
`dye-edge-check` と同じ4条件(①元絵 alpha>=32 ②`notBbox` は数えない
③彩度0.1未満・明度0.25未満は数えない ④3px先が本体の外なら「輪郭」)で数え直す。
**この4つのどれかを落とすと数字が合わない。** 実際、②を入れ忘れて「目が原因」と読み違えた。
