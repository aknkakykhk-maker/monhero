...
2026-09-11 に実測して切り分けた(手順は下の「測り方」)。

| 疑ったこと | 実測 | 結論 |
| --- | --- | --- |
| 目を染色対象にしていないから数えられている | 目は `UNDINE_EYE_BOXES` を `notBbox` に宣言済みで、検査も `notBbox` の範囲は数えていない | **違う。** 目は無関係 |
| 部位マップが 256×384 と粗いから | 見本PNG(原寸1024×1536)をそのまま使っても輪郭 30.18%。512×768 で 30.10%、256×384 で 29.79% | **違う。** 解像度をいくら上げても変わらない |
| 実装側でマスクを広げれば埋まる | 本体の中で 8px 広げて 11.77%、32px で 5.79%、**64px でようやく 3.07%** | **筋が悪い。** そこまで広げるのは別の壊し方になる |

残っているのは**髪の毛先・腕のヒレ・尾びれのふち**だけで、内側の2.15%も毛先の細いところに集まっている。
つまり**見本マスクがふちを塗っていない**。ゲームが作るマスク(22.81%)のほうが見本(30.18%)より
少しマシなのは、`_buildHiResMaskUrls` のにじみへの塗り足しが効いているぶん。

直し方は1つ。デバッグ画面の「🖌️ 染色マスク編集」でウンディーネを開き、
**髪の毛先・ヒレ・尾びれのふちを、輪郭からはみ出すくらい塗って「範囲外を掃除」**を押す
(はみ出したぶんは本体の外として自動で消える)。「PNG書出」で書き出したものを
`tools/art-sources/dye-masks/undine-dye-mask.PNG` へ置き換え、
`UNDINE_EXACT_REGION_2BIT` を作り直す。検査を緩める形では直さない。

**測り方**(同じ切り分けをやり直したくなったとき)

`tools/harness.js` の `loadDyeModule()` で `getDyeRegionMasks(id, url)` と
`MASU_COLOR_REGION_HUES` を取り出し、`dye-edge-check` と同じ4つの条件
(①元絵 alpha>=32 ②`notBbox` は数えない ③彩度0.1未満・明度0.25未満は数えない
④3px先が本体の外なら「輪郭」)で数え直す。**この4つのどれかを落とすと数字が合わない。**
実際、②を入れ忘れて「目が原因」と読み違えた。
