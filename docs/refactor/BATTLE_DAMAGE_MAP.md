...
**あつの挑発(`stun_atsu`)も同じ関数へ(2026-09-06 3本目)**: `buildAttackHits` に `mainCanCrit`(既定 true)を足し、
`processTurn` のバフ・デバフ側ブロックは `mainCanCrit:false` で呼ぶだけにした。連撃(ザン 30%×1 / エイキ 10%×2)と全体連撃は
通常攻撃と同じ規則で、固有技ぶんの追加は付かない(`card.type` が `debuff` なので `isUniqueOf` が偽になる)。旧ブロックと新ブロックを
同じ乱数列で 72,000 通り走らせ、合計・ヒット列・会心の並びまで一致することを確かめてから置き換えた。
`battle/damage-parity-check.js` にもあつの挑発の照合(320 通り + ザン・エイキの代表例)を足してある。

**残っている差(表示だけ・今回は変えていない)**: 予測表示 `getAttackPredictedDmg` は `mainCanCrit` を渡さないので、
確定会心(タイガーの固有技の次ターン)のときだけ、あつの挑発の予測がメイン 1 発ぶん(`floor(d×会心倍率) − d`)多く出る。
実処理は一本化前からメインに会心を乗せていないので、実ダメージは変わっていない。直すなら予測側へ
`mainCanCrit: card.subType!=='stun_atsu'` を 1 行足すだけだが、見た目の数値が変わるためユーザーの判断を待つ(検査は
この差を `knownGap` として明示的に許容している)。
