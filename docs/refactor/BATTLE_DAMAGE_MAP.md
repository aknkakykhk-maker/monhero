...
**予測表示の差も解消(2026-09-06、ユーザー指示)**: 予測 `getAttackPredictedDmg` も `mainCanCrit: card.subType!=='stun_atsu'` を渡すようにした。
それまでは確定会心(タイガーの固有技の次ターン)のときだけ、あつの挑発の予測がメイン 1 発ぶん(`floor(d×会心倍率) − d`)多く出ていた
(実ダメージは一本化前からメインに会心が乗っておらず、変わっていない)。`battle/damage-parity-check.js` は既知差の許容を外し、
予測と実処理の完全一致を見る。
