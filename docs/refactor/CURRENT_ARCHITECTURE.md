...
> 部品は共有層 21 個(`10-core` 〜 `30-rhythm-play`。2026-09-06 に旧 `10-shared` を節ごとに分けた。役割は `parts.json`)と、`40-screen-effects.jsx`(画面ライフサイクル。2026-09-10 の STEP 6-1 で追加)、`50-error-boundary.jsx`、`51-screen-settings.jsx` と `52-screen-missions.jsx`(切り出した画面。STEP 6-2 から1つずつ増える)、`60-app.jsx`(MonsterHeroGame)、`70-bootstrap.jsx`(CSS 注入と createRoot)。
>
> | 部品 | 役割 |
> | --- | --- |
