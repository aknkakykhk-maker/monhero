...
2. 重複候補を目視確認する。3列一致だけを理由に自動削除すると正当な同点を失うため、事故由来と確認できた行だけを既存の削除手順で削除する。
...
- 3列一致の既存重複削除と `clear_id` 導入は独立している。3列重複が残っていても `clear_id` の追加は可能だが、依頼どおり既存の事故データを削除してから適用する。
