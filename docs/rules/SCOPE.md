...
**版8からは、版が遊んだ感想で自動に増える**(2026-09-26)。`rhythm-chart-learn.js --write` が音ゲーの作法の
重みを学び直すと、`tools/mode/authoring/chart-knowledge-weights.json` へ新しい版を書き足し、最新版の番号がひとつ上がる。
これも「次に解析する曲から効く」だけで、公開中の曲は変わらない。作り直すかはユーザーが数字を見て決める。
重みの版は消さない(前の版の曲をいつでも同じに作り直せるように)。詳細: 同 3.1.21。
