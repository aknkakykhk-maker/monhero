-- 安全試験。DELETE結果を表示するが、最後に必ず全変更を取り消す。
begin;
...
begin
  ...
  raise notice '成功: 11件、11件、5件（合計27件）を試験削除し、各1件が残りました';
end $$;

-- 削除候補27件（試験中に実際に削除できた行）。
select id, user_name, difficulty, score, clear_id, created_at, '削除候補（ROLLBACK予定）' as status
from duplicate_deleted_rows
order by user_name desc, score desc, created_at asc, id asc;
