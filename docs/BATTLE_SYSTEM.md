...
| ULTIMATE | 実装済み（CHAOSクリアで解放） | ×25 | ×20 | ×40 | ×20 | 60 | 累計ターン圧、覚醒低下、50/100/150TのDISTANCE BREAK（弱体距離の与ダメージ50%） |
| INFINITY | 未実装（`available:false`） | － | － | － | － | － | － |

ULTIMATEでは累計ターン×0.5%を次WAVEの敵HP・攻撃倍率へ加算し、供モン加入時のステータス加算倍率を同率で低下させる。そのWAVEのターン数×0.5%で次の能力覚醒を低下させる（距離強化は対象外）。累計50・100・150ターン到達時は、次WAVE開始時に未弱体距離をランダムで1つ追加し、その距離からの与ダメージを50%にする。INFINITYは未実装段階のため数値を持たせず、画面には「？？？」とだけ出す。
...
```text
max(30, (敵攻撃値 - 実効丈夫さ×0.75) × (1 - min(0.5, 実効丈夫さ×0.00015)))
× モッチー／ミタラシ勇者補正0.8
× アーク／イブリース勇者の残回数あり補正0.5
× max(0.01, 1 - 永続被ダメ軽減率)
```

丈夫さの基本防御部分には最低30を適用し、その後に勇者特性、永続被ダメ軽減、絶氷の楔、次ターン被ダメ倍率の順で既存補正を適用して最後に切り捨て、最低1とする。敵攻撃値には WAVE 限定の敵攻撃低下を先に適用する。ガード値はカードごとの `実効丈夫さ×段階倍率` の合計で、倍率は順に2.40、2.60、2.80、3.00、3.40、3.70、4.45、5.35、6.75。段階はバフを除く基礎丈夫さ100ごとに0～8へ上昇する一方、基本防御とガード値には丈夫さバフ込みの実効丈夫さを使う。万象拒絶も丈夫さ800で固定しない。デッキに入るガードカードの枚数は最初2枚で、ガードが2段階進化するごとに1枚増え、最大4枚（`guardCardCount`）。反射、吸収、回避、無効化、スタンはガードより先に判定される。
