...
```text
max(30, (敵攻撃値 - 実効丈夫さ×1.5) × (1 - min(0.5, 実効丈夫さ×0.0002)))
× モッチー／ミタラシ勇者補正0.8
× アーク／イブリース勇者の残回数あり補正0.5
× max(0.01, 1 - 永続被ダメ軽減率)
```

丈夫さの基本防御部分には最低30を適用し、その後に勇者特性、永続被ダメ軽減、絶氷の楔、次ターン被ダメ倍率の順で既存補正を適用して最後に切り捨て、最低1とする。敵攻撃値には WAVE 限定の敵攻撃低下を先に適用する。ガード値はカードごとの `実効丈夫さ×段階倍率` の合計で、倍率は順に2.10、2.20、2.25、2.30、2.65、2.85、3.60、4.40、5.70。段階はバフを除く基礎丈夫さ100ごとに0～8へ上昇する一方、基本防御とガード値には丈夫さバフ込みの実効丈夫さを使う。万象拒絶も丈夫さ800で固定しない。デッキに入るガードカードの枚数は最初2枚で、ガードが2段階進化するごとに1枚増え、最大4枚（`guardCardCount`）。反射、吸収、回避、無効化、スタンはガードより先に判定される。
