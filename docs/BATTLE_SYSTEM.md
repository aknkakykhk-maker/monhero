...
| NIGHTMARE | 実装済み（EXTREMEクリアで解放） | ×15 | ×20 | ×30 | ×10 | 100 | WAVE後強化50%、自動回復率・距離適性の＋50% / －200% |
| CHAOS / ULTIMATE / INFINITY | 未実装（`available:false`） | － | － | － | － | － | － |

CHAOS以降の未実装段階は数値を持たせず、画面には「？？？」とだけ出す（仮の倍率を置かない）。
...
- **記録**: 自己ベストは `mh_extreme_hs_<極限難易度>`、クリア回数は `mh_extreme_clears_<極限難易度>`。NIGHTMAREクリアは同じ構造の専用キーへ保存し、将来の上位難易度解放に再利用できる。
...
- **報酬**: 選択中の極限難易度設定を共通経路で使う。EXTREMEはスコア×20・経験値×25・ダイヤ×7.5・虹75個、NIGHTMAREはスコア×20・経験値×30・ダイヤ×10・虹100個。
