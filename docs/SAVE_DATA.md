...
- `inheritedUniques[]`: 副の固有技スナップショットと `monId` / `lineageId` / `sourceMasuName` / 恒久Lv。ゲーム内では `monId` に対応する現在の固有技定義を正本として解決し、継承したLvと履歴情報を重ねる。`monId` が無い、または現在の定義が無い旧セーブは保存スナップショットをそのまま使う。
