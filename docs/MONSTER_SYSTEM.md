...
| 5 | 赤 | 21〜25 |
| 6 | 金 | 26〜30 |
| 虹1〜5 | 虹（金から1個ずつ置換） | 31〜35 |

例: `1凸=青★1` / `5凸=青★5` / `6凸=黄色★1+青★4` / `11凸=ピンク★1+黄色★4` /
`26凸=金★1+赤★4` / `30凸=金★5` / `31凸=虹★1+金★4` / `35凸=虹★5`。
1段階目（青）だけは前の色が無いので、5個に満たないまま凸数ぶんだけ出す。

- 「黄色」と「金」を見分けられるよう、黄色は光沢無しの素の黄色（`#fde047`）、
  金は一段濃い金色（`#f5c04a`）に上が明るく下が暗い金属的な縁取りを付ける。
- 虹だけは `images/ui/breakthrough-rainbow-star.PNG` を使い、文字で描く既存色と同じ固定スロットへ
  `object-fit: contain` で収める。画像の縦横比と外周グローを保ち、5個の並び幅と行高を変えない。
- 31〜35凸は金★を左から虹★へ1個ずつ置き換える。既存の31凸・Lv.200個体は保存値を書き換えず、
  虹★1+金★4として表示する。
