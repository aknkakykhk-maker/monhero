...
レベルは1から始まり、コードはLv.400まで反復して算出する。通常は1レベル上昇ごとに強化ポイントを1得る。虹★4（34凸）で上がるLv.270→330は1レベルにつき2、虹★5（35凸）で上がるLv.330→400は1レベルにつき3を得る。倍率は共通の絆XP加算処理で上昇レベル数に掛け、獲得XPや必要XPには掛けない。読み込み時には「絆Lv-1」と使用済み＋未使用ポイントを比較し、不足分を補填する。
...
- 技を継承しない合体は0ダイヤ。副が絆Lv30以上で継承を選択した場合は3000ダイヤを消費し、副の種の固有技を `inheritedUniques` へ追加する。主の絆Lvに条件はない。継承技の性能・名称・画像・説明・効果は `monId` から現在の `ALL_PLAYER_MONSTERS[monId].unique` を解決し、継承したLvと継承元個体の履歴は保存値を維持する。継承元が存在しない旧セーブだけは保存済み技へフォールバックする。
...
- 継承した固有技そのものは履歴に持っていない。継承したのは必ず相手の種の固有技なので、
  主の `inheritedUniques` から同じ `monId` のものを探し、現在の継承元定義で解決した名前を出す。見つからなければ種の固有技名を出す。
- 0件なら「まだ合体履歴はありません」。
