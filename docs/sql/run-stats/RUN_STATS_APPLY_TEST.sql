-- public.rankings へ turns / reached_wave の2列を追加する「予行演習」。
-- 読み取り専用: いいえ(トランザクション内で足すが、最後に必ず取り消す)。
-- 本番変更が残るか: いいえ。末尾が rollback; なので、この実行では何も保存されない。
--
-- 目的: 実適用と同じSQLを一度通し、エラーが出ないことと、既存の記録に
--       いっさい影響しないことを確かめる。結果表を確認してから実適用へ進む。
...
-- 予行演習なので必ず取り消す。実適用は RUN_STATS_APPLY.sql を使う。
rollback;
