-- public.rankings へ profile_frame 列を1つ追加し、モンビーのビュー・関数へその列を通すSQL。
-- 読み取り専用: いいえ。
...
-- 先に PROFILE_FRAME_APPLY_TEST.sql(末尾 rollback;)をエラー無く通してから実行する。
--
-- なぜ足すのか:
...
-- これを忘れると、しばらくの間アプリから profile_frame が見えないことがある。
