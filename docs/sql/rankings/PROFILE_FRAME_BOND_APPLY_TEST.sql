-- public.bond_levels へ profile_frame 列を1つ追加するSQLの【予行演習】。
-- 読み取り専用: いいえ(ただし末尾でrollbackするため、本番には何も残らない)。
-- 本番変更が残るか: いいえ。実適用(PROFILE_FRAME_BOND_APPLY.sql)の前にこちらを通す。
--
-- 既存の行・列・RLS・ポリシー・権限は変更しない(DROP・DELETE・UPDATEをしない)。
-- 足すのはNULL許容の列1つだけなので、既存の記録はすべてNULLのまま残る(NULL = フレームなし)。
-- これをエラー無く通してから PROFILE_FRAME_BOND_APPLY.sql(末尾 commit;)を実行する。
--
-- なぜ足すのか:
