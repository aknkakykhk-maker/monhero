-- 「いまの見た目をランキングに出す」ために残っている作業を、1回の実行でまとめて行うSQL。
-- 読み取り専用: いいえ。
-- 本番変更が残るか: はい。安全確認にすべて成功した場合だけ、最後に1回だけcommitする。
--
...
--
-- 先に PROFILE_LOOK_ALL_APPLY_TEST.sql(末尾 rollback;)をエラー無く通してから実行する。
--
...
-- 安全確認済みの変更を本番へ保存する。①②③とも、ここで初めて確定する。
commit;

-- Data API(PostgREST)へ新しい列とテーブルを認識させる。
