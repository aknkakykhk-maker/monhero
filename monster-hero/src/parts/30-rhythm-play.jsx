...
const score=run.lifeDepleted?run.lockedScore:run.score;setView(v=>({...v,score,combo:run.combo,maxCombo:run.maxCombo,last:judgment,fastSlow:side||'',counts:{...run.counts},fast:run.fast,slow:run.slow,life:run.life,...(abilityFlash?{ability:abilityFlash}:{})}));scheduleJudgmentClear();if(abilityFlash)scheduleAbilityClear();if(_judgeT0)RHYTHM_PERF.judge(performance.now()-_judgeT0,!!monster);},[chart.totalNotes,difficulty.maxScore,scheduleAbilityClear,scheduleJudgmentClear,settings.vibrationEnabled,tutorial]);
  ...
  const scheduleTick=useCallback(()=>{stopFrame();const tick=(frameNowMs)=>{RHYTHM_PERF.frame(frameNowMs);RHYTHM_GESTURE_RUNTIME.invalidateAreaRect();const run=runRef.current;if(!run||run.finished||run.paused)return;const perfTickStart=RHYTHM_PERF.enabled?performance.now():0;const songTimeMs=run.audio.songTimeMs();RHYTHM_PERF.songTime(songTimeMs);/* ★装飾の巡回(デバッグ限定)。曲の位置で段を決め、段が変わったらプレイエリアの目印を書き替える。 切り替えた直後はスタイルの計算が入るので、落ち着くまでは段へ数えない(lane(-1))。 時計ではなく曲の位置で決めているので、ポーズしても同じところで同じ条件になる。 */if(RHYTHM_STRIP.sweep&&RHYTHM_PERF.enabled){const stripStep=RHYTHM_STRIP.stepAt(songTimeMs);if(run.stripStep!==stripStep){run.stripStep=stripStep;run.stripSettled=false;RHYTHM_PERF.lane(-1);const stripEl=playAreaRef.current;if(stripEl){const next=RHYTHM_STRIP_SWEEP_STEPS[stripStep].strip;if(next)stripEl.dataset.rhythmStrip=next;else delete stripEl.dataset.rhythmStrip;}}if(!run.stripSettled&&RHYTHM_STRIP.stepElapsedMs(songTimeMs)>=RHYTHM_STRIP_SWEEP_SETTLE_MS){run.stripSettled=true;RHYTHM_PERF.lane(stripStep);}}const travel=measureTravel(),visualTime=songTimeMs-settings.judgmentTimingOffsetMs,travelMs=rhythmTravelMsForSpeed(settings.noteSpeed);let perfScanned=0,perfDrawn=0;updateJudgmentBand(travel,travelMs);
