...
const MARKET_PROFILE_ICON_STYLES = {
  ...
  // 「同じウンディーネ種なんだからそれをもとにしろよ」。お手本は同じ種のスネグーラチカ。
  // 以前は「顔の高さが丸の約4割」でそろえていたが、そろえる先がずれていたうえ、
  // 顔の中心が丸の中心より30pxほど上にあり、丸の下半分が服で埋まって引いて見えていた。
  //
  // 合わせ方は「顔が丸の何％か」ではなく、**立ち絵の縦のどこからどこまでが丸に映るか**で見る。
  // 顔の高さだけをそろえると、髪の広がりや頭身が違うぶん見た目が合わない(実際に合わなかった)。
  // スネグーラチカは立ち絵(正方形)の 12〜36% が映っていて、頭の上から胸元までが入る。
  // 人魚2体は立ち絵が縦長(1024x1536)なので、同じ見え方になるのは「上から4%の位置から21%ぶん」。
  // 倍率は 1/0.21 ≒ 4.76、位置はその範囲が丸の上端から始まるように決めた。
  // 横は顔の中心(絵の左から約47%)が丸の中心へ来る値。
  undine_icon: { scale: 4.76, x: 9.8, y: 169 },
  yaobikuni_icon: { scale: 4.76, x: 8.9, y: 169 },
