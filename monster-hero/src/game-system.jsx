...
const buildMasuRebirth = ({ masu, skillKey, gold }) => {
  ...
  return { ok:true, cost, skillKey, skillLevel:currentSkillLevel + 1, nextGold:donationDiamondValue(gold) - cost, nextMasu:{ ...normalized, bondXp:totalBondXpForLevel(1), rebirthCount:normalized.rebirthCount + 1, levelCap:normalized.levelCap + REBIRTH_LEVEL_CAP_GAIN, distAptPoints:donationDiamondValue(normalized.distAptPoints) + 5, uniqueSkillLevels:{ ...normalized.uniqueSkillLevels, [skillKey]:currentSkillLevel + 1 } } };
};
...
function MonsterHeroGame() {
  ...
  const executeMasuFusion = () => {
    ...
    const inheritedUnique = canInherit ? { ...uniqueSkillAtLevel(subBase.unique, inheritedLevel), monId: subBase.id, sourceMasuName: sub.name } : null;
