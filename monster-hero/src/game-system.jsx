...
const MASU_PATTERN_DEFAULTS = Object.freeze({ pattern:'stripe', target:'all', color:'#38bdf8', opacity:55, size:0.045, spacing:0.14, rotation:0, x:0.5, y:0.5 });
...
const MasuPatternLayer = ({ baseId, src, settings }) => {
  ...
  useEffect(() => {
    ...
    (async()=>{
      ...
      ctx.translate(settings.x*w,settings.y*h);ctx.rotate(settings.rotation*Math.PI/180);
      ...
      if(settings.pattern==='stripe'){
        ...
        for(let y=-extent;y<=extent;y+=unit)for(let x=-extent;x<=extent;x+=unit){const ox=x+((Math.round(y/unit)&1)*unit*.5);ctx.beginPath();for(let i=0;i<10;i++){const a=-Math.PI/2+i*Math.PI/5,r=i%2?motif*.22:motif*.5,px=ox+Math.cos(a)*r,py=y+Math.sin(a)*r;i?ctx.lineTo(px,py):ctx.moveTo(px,py);}ctx.closePath();ctx.fill();}
        ...
        {gameState==='MASU_PATTERN_DEBUG'&&(()=>{
          ...
          return <main className="flex-1 min-h-0 flex flex-col bg-slate-950" style={{paddingTop:'env(safe-area-inset-top)',paddingBottom:'env(safe-area-inset-bottom)'}}>
            ...
            {eligible.length===0?<section className="flex-1 flex flex-col items-center justify-center p-6 text-center"><div className="text-5xl mb-4">🐾</div><p className="font-black text-slate-300">カスタマイズできるマスモンがいません</p><button onClick={()=>setGameState('DEBUG_SETTINGS')} className="mt-6 min-h-[48px] px-8 rounded-xl bg-slate-800 font-black">デバッグ設定へ戻る</button></section>:!selected?<section className="flex-1 min-h-0 overflow-y-auto mh-scroll p-4"><p className="mb-3 text-[11px] font-bold text-slate-400">所持マスモンを1体選択してください。</p><div className="grid grid-cols-3 gap-2">{eligible.map(m=>{const base=ALL_PLAYER_MONSTERS[m.baseId];return <button key={m.id} onClick={()=>{setPatternMasuId(m.id);setPatternSettings({...MASU_PATTERN_DEFAULTS});setPatternCustomColor({open:false,h:198,s:0.77,v:0.97});}} className="min-h-[116px] rounded-2xl border border-fuchsia-500/30 bg-slate-900 p-2 active:scale-[.97]"><DyedMonsterImage baseId={m.baseId} src={masuDisplayImageUrl(base)} alt={m.name} masuColors={getMasuColors(m)} className="w-16 h-16 mx-auto object-contain"/><b className="block truncate text-[10px]">{m.name}</b><small className="text-[8px] text-amber-300">{Number(m.rebirthCount)>0?`転生 ${m.rebirthCount}回`:'未転生'}</small></button>})}</div></section>:(()=>{const base=ALL_PLAYER_MONSTERS[selected.baseId],regions=dyeRegionCount(selected.baseId),colors=getMasuColors(selected);return <div className="flex-1 min-h-0 overflow-y-auto mh-scroll p-3 space-y-3 scroll-smooth">
              ...
              <aside className="sticky top-0 z-20 min-h-[76px] rounded-2xl border border-fuchsia-400/50 bg-slate-950/95 px-3 py-2 shadow-xl backdrop-blur flex items-center gap-3"><PatternedMasuImage masu={selected} base={base} colors={colors} settings={patternSettings} className="w-14 h-14 shrink-0"/><div className="min-w-0 flex-1"><b className="block truncate text-[10px] text-fuchsia-200">追従プレビュー</b><small className="block text-[8px] text-slate-400">全サイズ共通の画像座標</small></div><button onClick={()=>document.getElementById('masu-pattern-large-preview')?.scrollIntoView({behavior:'smooth',block:'start'})} className="min-h-[40px] px-3 rounded-xl bg-fuchsia-800 text-[9px] font-black">プレビューを拡大</button></aside>
              <section className="space-y-3 rounded-2xl border border-white/10 bg-slate-900/50 p-3"><div><div className="mb-2 text-[10px] font-black text-slate-300">模様</div><div className="grid grid-cols-3 gap-2">{MASU_PATTERN_OPTIONS.map(([id,label])=><button key={id} onClick={()=>updatePattern('pattern',id)} className={`min-h-[44px] rounded-xl text-[10px] font-black ${patternSettings.pattern===id?'bg-fuchsia-700 ring-2 ring-fuchsia-300':'bg-slate-800 text-slate-300'}`}>{label}</button>)}</div></div>
                ...
                <div className="rounded-xl bg-slate-900 px-3 py-3"><div className="flex items-center justify-between mb-2 text-[10px] font-black"><span>模様の色</span><span className="w-8 h-8 rounded-full border-2 border-white/30" style={{backgroundColor:patternSettings.color}}/></div><div className="grid grid-cols-9 gap-1">{MASU_PATTERN_PRESET_COLORS.map(color=><button key={color} aria-label={`模様色 ${color}`} onClick={()=>updatePattern('color',color)} className={`aspect-square rounded-full border ${patternSettings.color===color?'ring-2 ring-fuchsia-300 border-white':'border-white/20'}`} style={{backgroundColor:color}}/>)}<button aria-label="カスタム色" onClick={()=>setPatternCustomColor(prev=>({...prev,open:!prev.open}))} className={`aspect-square rounded-full border border-white/30 ${patternCustomColor.open?'ring-2 ring-fuchsia-300':''}`} style={{background:'conic-gradient(#ef4444,#eab308,#22c55e,#06b6d4,#3b82f6,#d946ef,#ef4444)'}}/></div>{patternCustomColor.open&&<div className="mt-3"><CustomColorPicker h={patternCustomColor.h} s={patternCustomColor.s} v={patternCustomColor.v} onChange={(h,s,v)=>{const [r,g,b]=_hsvToRgb(h,s,v),hex=`#${[r,g,b].map(n=>n.toString(16).padStart(2,'0')).join('')}`;setPatternCustomColor({open:true,h,s,v});updatePattern('color',hex);}}/><div className="mt-2 grid grid-cols-3 text-center text-[8px] font-bold text-slate-400"><span>色相 {Math.round(patternCustomColor.h)}°</span><span>彩度 {Math.round(patternCustomColor.s*100)}%</span><span>明度 {Math.round(patternCustomColor.v*100)}%</span></div></div>}</div>
                {slider('opacity','透明度',0,100,1,v=>`${v}%`)}{slider('size','模様サイズ',0.01,0.12,0.005,v=>`${Math.round(v*100)}%`)}{slider('spacing','模様の間隔',0.04,0.3,0.005,v=>`${Math.round(v*100)}%`)}{slider('rotation','回転',-180,180,1,v=>`${v}°`)}{slider('x','横位置',0,1,0.01,v=>`${Math.round(v*100)}%`)}{slider('y','縦位置',0,1,0.01,v=>`${Math.round(v*100)}%`)}
