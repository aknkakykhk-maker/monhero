...
// generated-sha256: 6045572459bef5a7
// ============================================================
// ---- part: 10-core.jsx ----
...
const DyeMaskTouchEditor=({onClose,onTryInGame,onReleaseTemporary,onReleaseAllTemporary,temporaryMasks,active=true})=>{
 ...
 const [previewMaskUrl,setPreviewMaskUrl]=useState(null),[previewRevision,setPreviewRevision]=useState(0),[previewColors,setPreviewColors]=useState(['red','green','blue','yellow','magenta']);
 ...
 const colors={red:[255,0,0,255],green:[0,255,0,255],blue:[0,0,255,255],yellow:[255,255,0,255],magenta:[255,0,255,255],eraser:[0,0,0,0]},colorCss={red:'#ff0000',green:'#00ff00',blue:'#0000ff',yellow:'#ffff00',magenta:'#ff00ff',eraser:'#ffffff'};
 ...
 const MASK_REGION_RGB=[[255,0,0],[0,255,0],[0,0,255],[255,255,0],[255,0,255]],MASK_REGION_CSS=['#f00','#0f0','#00f','#ff0','#f0f'];
 ...
 const normalizeMask=(image,outside,regionCount=3)=>{const data=image.data,palette=MASK_REGION_RGB.slice(0,Math.max(3,Math.min(MASK_REGION_RGB.length,regionCount)));for(let i=0,p=0;i<data.length;i+=4,p++){if(outside[p]||!data[i+3]){data[i]=data[i+1]=data[i+2]=data[i+3]=0;continue;}const r=data[i],g=data[i+1],b=data[i+2];let best=0,bestD=Infinity;for(let k=0;k<palette.length;k++){const c=palette[k],d=(r-c[0])*(r-c[0])+(g-c[1])*(g-c[1])+(b-c[2])*(b-c[2]);if(d<bestD){bestD=d;best=k;}}data[i]=palette[best][0];data[i+1]=palette[best][1];data[i+2]=palette[best][2];data[i+3]=255;}return image;};
 ...
 useEffect(()=>{if(!ready||view!=='composite')return;const c=maskRef.current,ctx=context();if(!c||!ctx)return;let cancelled=false;const source=ctx.getImageData(0,0,c.width,c.height),image=ctx.createImageData(c.width,c.height);image.data.set(source.data);normalizeMask(image,outsideRef.current,dyeRegionCount(target.baseId));const temp=document.createElement('canvas');temp.width=c.width;temp.height=c.height;temp.getContext('2d').putImageData(image,0,0);temp.toBlob(blob=>{if(cancelled||!blob)return;const nextUrl=URL.createObjectURL(blob);setPreviewMaskUrl(previous=>{if(previous)URL.revokeObjectURL(previous);return nextUrl;});},'image/png');return()=>{cancelled=true;};},[ready,view,historyTick,previewRevision,target.baseId]);
 ...
 const tryInGame=()=>{const c=maskRef.current;if(!c||!ready)return;const image=normalizeMask(context().getImageData(0,0,c.width,c.height),outsideRef.current,dyeRegionCount(target.baseId));context().putImageData(image,0,0);c.toBlob(blob=>{if(blob)onTryInGame(target,blob,previewColors);},'image/png');};
