...
const TRAINING_TOOLS = Object.freeze({
 feather:{name:'加速の羽',emoji:'🪽',timing:'サイコロを振る前',desc:'次の出目に＋2'}, gale:{name:'疾風の札',emoji:'🌪️',timing:'サイコロを振る前',desc:'2回振り、高い出目を採用'}, reroll:{name:'振り直しの石',emoji:'🪨',timing:'出目確定後、移動先選択前',desc:'現在の出目を破棄して振り直す'}, noReturn:{name:'戻らずのお守り',emoji:'🧿',timing:'取得後は自動待機',desc:'次に受ける後退効果を1回無効化して消滅'}, sand:{name:'時の砂',emoji:'⏳',timing:'自分の手番中',desc:'残りターン＋1'}, fixed:{name:'確定サイコロ',emoji:'🎲',timing:'サイコロを振る前',desc:'次の出目を1・2・3から選択'}, returnCharm:{name:'帰還のお守り',emoji:'🏮',timing:'取得後は自動待機',desc:'ゴール失敗時、仮獲得した通常アイテムから1個選んで保護'},
});
const TRAINING_SPACE_TYPES = Object.freeze({
 start:{kind:'start',label:'スタート',emoji:'🚩',color:'#475569',desc:'修行の開始地点'}, xp30:{kind:'xp',value:30,label:'絆EXP',emoji:'💗',color:'#db2777',desc:'仮獲得絆経験値＋30'}, xp60:{kind:'xp',value:60,label:'大EXP',emoji:'💖',color:'#be185d',desc:'仮獲得絆経験値＋60'}, gem50:{kind:'diamond',value:50,label:'ダイヤ',emoji:'💎',color:'#0891b2',desc:'仮獲得ダイヤ＋50'}, gem100:{kind:'diamond',value:100,label:'大ダイヤ',emoji:'💠',color:'#2563eb',desc:'仮獲得ダイヤ＋100'}, item:{kind:'item',value:'training_ticket',label:'アイテム',emoji:'🎁',color:'#7c3aed',desc:'仮獲得通常アイテムを1個追加'}, tool:{kind:'tool',label:'修行道具',emoji:'🎒',color:'#9333ea',desc:'修行専用アイテムをランダム取得'}, forward:{kind:'move',value:1,label:'前進',emoji:'⏩',color:'#16a34a',desc:'1～3マス追加移動（停止マスだけ発動）'}, back:{kind:'move',value:-1,label:'後退',emoji:'⏪',color:'#dc2626',desc:'1～3マス戻る（停止マスだけ発動）'}, turnPlus:{kind:'turn',value:1,label:'ターン＋',emoji:'⏱️',color:'#059669',desc:'残りターン＋1'}, turnMinus:{kind:'turn',value:-1,label:'ターン－',emoji:'⚡',color:'#b91c1c',desc:'残りターン－1'}, boost:{kind:'effect',value:'boost',label:'強化',emoji:'✨',color:'#ca8a04',desc:'次回のサイコロ出目＋1'}, again:{kind:'effect',value:'again',label:'もう一度',emoji:'🔁',color:'#0d9488',desc:'ターンを消費せず再度サイコロ'}, happening:{kind:'happening',label:'ハプニング',emoji:'⁉️',color:'#ea580c',desc:'良い効果または悪い効果が発動'}, goal:{kind:'goal',label:'ゴール',emoji:'🏁',color:'#eab308',desc:'到達または通過で修行成功'},
});
...
const createTrainingSession=(masuId,difficulty='BEGINNER')=>({status:'playing',masuId:String(masuId),difficulty,mapId:TRAINING_MAP_ID,seed:trainingSeed(),position:'n0',previous:null,remainingTurns:10,rewards:trainingEmptyRewards(),tools:[],effects:{},lastRoll:null,paths:[],selectedPath:null,forcedMoves:0,eventLog:['修行テスト開始'],message:'サイコロを振ってください'});
...
const trainingPaths=(start,steps)=>{const out=[];const walk=(id,left,path)=>{if(id==='n23'){out.push(path);return;}if(left===0){out.push(path);return;}for(const next of TRAINING_NODE_BY_ID[id].next)if(path.length<2||next!==path[path.length-2])walk(next,left-1,[...path,next]);};walk(start,steps,[start]);return out.filter(p=>p.length===steps+1||p.at(-1)==='n23');};
...
function MonsterHeroGame() {
  ...
  const [changelogSeen, setChangelogSeen] = useState({ update: '', issue: '' }); // タブごとの最新既読日時(端末に保存)
  ...
  const markChangelogTabSeen = (type) => {
    ...
    setChangelogSeen(prev => ({ ...prev, [type]: latest }));
  ...
  const loadRankings = useCallback(async (targetDiff=null, includeLevels=false, force=false) => {
    ...
    const loadOne = async (requestedDiff) => {
      ...
      try {
        ...
        setRankingErrorByDiff(prev => ({ ...prev, [d]: message }));
  ...
  const startTraining=()=>{if(!trainingSelectedId||trainingDifficulty!=='BEGINNER')return;setTrainingSession(createTrainingSession(trainingSelectedId));setTrainingDebugRoll(null);setGameState('TRAINING_BOARD');};
  ...
  const finishTraining=(success,base=trainingSession)=>{if(!base||base.status==='result')return;const result={...base,status:'result',success,finalRewards:settleTrainingRewards(base,success),paths:[],selectedPath:null,message:success?'修行成功！':'修行失敗…',eventLog:logTraining(base,success?'強制/通常成功':'ターン切れ/強制失敗')};setTrainingSession(result);setGameState('TRAINING_RESULT');success?Audio_.se.trainingGoal():Audio_.se.trainingFail();};
  ...
  const applyTrainingSpace=async base=>{const node=TRAINING_NODE_BY_ID[base.position],space=TRAINING_SPACE_TYPES[node.type];let next={...base,rewards:{...base.rewards,items:[...base.rewards.items]},tools:[...base.tools],effects:{...base.effects},paths:[],selectedPath:null,message:space.desc,eventLog:logTraining(base,`${space.label}: ${space.desc}`)};setTrainingModal({type:'effect',space});
  ...
  const rollTrainingDice=(fixedValue=null)=>{if(!trainingSession||trainingSession.status!=='playing'||trainingMovingRef.current||trainingSession.paths.length)return;Audio_.se.trainingDice();const roll=()=>1+Math.floor(Math.random()*3);let value=fixedValue||trainingDebugRoll||roll();if(trainingSession.effects.gale)value=Math.max(value,roll());if(trainingSession.effects.boost)value++;if(trainingSession.effects.feather)value+=2;const effects={...trainingSession.effects};delete effects.gale;delete effects.boost;delete effects.feather;delete effects.fixed;const paths=trainingPaths(trainingSession.position,value);patchTraining({lastRoll:value,paths,selectedPath:null,effects,remainingTurns:trainingSession.remainingTurns-1,forcedMoves:0,message:`${value}が出ました。発光する移動先を選択してください`,eventLog:logTraining(trainingSession,`サイコロ ${value}`)});};
  const chooseTrainingDestination=id=>{const paths=trainingSession.paths.filter(p=>p.at(-1)===id);if(!paths.length)return;if(paths.length===1)setTrainingModal({type:'route',paths});else setTrainingModal({type:'route',paths});};
  const moveTrainingPath=async path=>{if(trainingMovingRef.current)return;setTrainingModal(null);trainingMovingRef.current=true;Audio_.se.trainingDecide();let prev=trainingSession.position;for(const id of path.slice(1)){Audio_.se.trainingMove();setTrainingSession(v=>({...v,position:id,previous:prev,selectedPath:path,message:`${id}へ移動中…`}));prev=id;await new Promise(r=>setTimeout(r,240));if(id==='n23')break;}trainingMovingRef.current=false;const next={...trainingSession,position:path.at(-1),previous:path.at(-2),paths:[],selectedPath:null};setTrainingSession(next);await applyTrainingSpace(next);};
  const useTrainingTool=id=>{const t=TRAINING_TOOLS[id];setTrainingModal({type:'tool',id});if(!t||['noReturn','returnCharm'].includes(id))return;const beforeRoll=!trainingSession.lastRoll||!trainingSession.paths.length;if(['feather','gale','fixed'].includes(id)&&!beforeRoll)return;if(id==='reroll'&&!trainingSession.paths.length)return;const tools=[...trainingSession.tools];tools.splice(tools.indexOf(id),1);let effects={...trainingSession.effects},remainingTurns=trainingSession.remainingTurns,paths=trainingSession.paths,lastRoll=trainingSession.lastRoll;if(id==='sand')remainingTurns++;else if(id==='reroll'){paths=[];lastRoll=null;setTimeout(rollTrainingDice,0);}else effects[id]=true;Audio_.se.trainingTool();patchTraining({tools,effects,remainingTurns,paths,lastRoll,eventLog:logTraining(trainingSession,`${t.name}使用`)});};
  const restartTraining=()=>{if(trainingSelectedId){setTrainingSession(createTrainingSession(trainingSelectedId));setGameState('TRAINING_BOARD');}};
  ...
  return (
    <div onPointerDown={(e)=>{const rect=e.currentTarget.getBoundingClientRect(); spawnRipple(e.clientX-rect.left, e.clientY-rect.top);}} className="h-full w-full bg-slate-950 text-white overflow-hidden relative select-none font-sans" style={{height:'100%'}}>
      ...
      <div className="relative z-10 h-full flex flex-col" style={screenShake?{animation:bigShake?'mooQuake 750ms ease-in-out':'screenShake 450ms ease-in-out'}:undefined}>
        ...
        {gameState==='HOME'&&(
          <main className="mh-home-scene" aria-label="村の広場">
            ...
            <nav className="mh-home-facilities" aria-label="拠点施設">
              ...
              <button className="mh-home-facility training" onClick={openTrainingInfo} aria-label="修行（実装予定）"><span>🎲 修行<small>実装予定</small></span></button>
        ...
        {gameState==='TRAINING_CONFIRM'&&<main className="mh-training-screen"><div className="mh-debug-banner">DEBUG・報酬や進行状況は保存されません</div><header className="mh-training-head"><button onClick={()=>setGameState('TRAINING_DIFFICULTY')}><ArrowLeft/></button><div><small>BEGINNER</small><h2>マップとルール確認</h2></div><i/></header><section className="mh-training-confirm"><h3>BEGINNER 試作マップ</h3><p>24ノード / 10ターン / サイコロ1～3。3か所の分岐で短い安全ルートと遠回りの報酬ルートを選び、再合流してゴールを目指します。</p><h4>移動ルール</h4><p>出目と同じ歩数で到達できるマスが発光します。目的地を選び、同じ目的地への経路が複数ある場合は経路も選びます。ゴールは到達または通過で成功。前進・後退は最終停止マスだけ発動し、強制移動は1ターン3回までです。</p><button className="mh-rule-button" onClick={()=>setTrainingModal({type:'rules'})}>マス一覧／ルール・全道具を見る</button><div className="mh-training-ticket"><b>デバッグ専用</b><small>チケット・通常データ・報酬・ミッション・ランキング・実績には一切反映しません。</small></div></section><footer className="mh-training-footer"><button onClick={startTraining}>BEGINNER 修行開始</button></footer></main>}

        {gameState==='TRAINING_BOARD'&&trainingSession&&(()=>{const m=masuMons.find(x=>String(x.id)===String(trainingSession.masuId));const destinations=new Set(trainingSession.paths.map(p=>p.at(-1)));const current=TRAINING_NODE_BY_ID[trainingSession.position];return <main className="mh-training-board"><div className="mh-debug-banner">DEBUG・報酬や進行状況は保存されません</div><header><div><b>{m?.name||'マスモン'} / 残り {trainingSession.remainingTurns}ターン</b><span>現在 {current.id}　🎲 {trainingSession.lastRoll??'-'}　ゴール最短 {trainingDistanceToGoal(current.id)}マス</span></div><button onClick={()=>setTrainingModal({type:'rules'})}>マス一覧／ルール</button></header><section className="mh-training-hud"><span>💗 {trainingSession.rewards.bondXp} XP</span><span>💎 {trainingSession.rewards.diamonds}</span><span>🎁 {trainingSession.rewards.items.length}</span><span>補助 {Object.keys(trainingSession.effects).join('・')||'なし'}</span></section><div className={`mh-node-map ${trainingMapOverview?'overview':''}`} style={{'--focus-x':current.x,'--focus-y':current.y}}>{TRAINING_BEGINNER_NODES.map(n=><React.Fragment key={n.id}>{n.next.filter(id=>TRAINING_BEGINNER_NODES.findIndex(x=>x.id===id)>TRAINING_BEGINNER_NODES.findIndex(x=>x.id===n.id)).map(id=>{const to=TRAINING_NODE_BY_ID[id],dx=to.x-n.x,dy=to.y-n.y;return <i key={id} style={{left:`${n.x}%`,top:`${n.y}%`,width:`${Math.hypot(dx,dy)}%`,transform:`rotate(${Math.atan2(dy,dx)*180/Math.PI}deg)`}}/>})}<button style={{left:`${n.x}%`,top:`${n.y}%`,background:TRAINING_SPACE_TYPES[n.type].color}} className={`${n.id===current.id?'current':''} ${destinations.has(n.id)?'destination':''}`} onClick={()=>destinations.has(n.id)?chooseTrainingDestination(n.id):setTrainingModal({type:'space',space:TRAINING_SPACE_TYPES[n.type]})}><span>{TRAINING_SPACE_TYPES[n.type].emoji}</span><small>{TRAINING_SPACE_TYPES[n.type].label}</small>{n.id===current.id&&m&&<DyedMonsterImage baseId={m.baseId} src={ALL_PLAYER_MONSTERS[m.baseId]?.iconUrl} alt={m.name} masuColors={getMasuColors(m)}/>}</button></React.Fragment>)}</div><div className="mh-board-buttons"><button onClick={()=>setTrainingMapOverview(v=>!v)}>{trainingMapOverview?'現在地へ':'全体マップ'}</button><button onClick={()=>setTrainingModal({type:'rewards'})}>仮報酬</button><button onClick={()=>setTrainingDebugOpen(v=>!v)}>DEBUG</button></div><p className="mh-training-message">{trainingSession.message}</p><section className="mh-training-tools">{trainingSession.tools.length?trainingSession.tools.map((id,i)=><button key={`${id}-${i}`} onClick={()=>setTrainingModal({type:'tool',id})}><span>{TRAINING_TOOLS[id].emoji}</span><small>{TRAINING_TOOLS[id].name}</small></button>):<p>修行道具 0 / 3（アイコンを押すと説明）</p>}</section><footer>{trainingSession.effects.fixed?<div className="mh-fixed-dice"><span>確定サイコロ</span>{[1,2,3].map(n=><button key={n} onClick={()=>rollTrainingDice(n)}>{n}</button>)}</div>:<button disabled={trainingSession.paths.length>0||trainingMovingRef.current} onClick={rollTrainingDice} className="mh-roll-button">🎲 サイコロを振る</button>}</footer>{trainingDebugOpen&&<aside className="mh-training-debug"><b>DEBUG PANEL</b><div>次の出目 {[1,2,3].map(n=><button onClick={()=>setTrainingDebugRoll(n)} className={trainingDebugRoll===n?'active':''}>{n}</button>)}<button onClick={()=>setTrainingDebugRoll(null)}>解除</button></div><div><button onClick={()=>patchTraining({remainingTurns:trainingSession.remainingTurns+1})}>ターン＋1</button><button onClick={()=>patchTraining({remainingTurns:Math.max(0,trainingSession.remainingTurns-1)})}>－1</button></div><select onChange={e=>{if(e.target.value)addTrainingTool(e.target.value)}} defaultValue=""><option value="">道具を追加</option>{Object.entries(TRAINING_TOOLS).map(([id,t])=><option value={id}>{t.name}</option>)}</select><button onClick={()=>setTrainingModal({type:'rewards'})}>仮報酬を確認</button><button onClick={()=>finishTraining(true)}>強制成功</button><button onClick={()=>finishTraining(false)}>強制失敗</button><button onClick={restartTraining}>最初から</button><pre>map: {trainingSession.mapId}{'\n'}seed: {trainingSession.seed}{'\n'}{trainingSession.eventLog.join('\n')}</pre></aside>}</main>})()}
        ...
        {trainingModal&&<div className="mh-training-modal" onClick={()=>setTrainingModal(null)}><div onClick={e=>e.stopPropagation()}>{trainingModal.type==='rules'?<><h3>マス一覧／ルール</h3><div className="mh-rules-list">{Object.values(TRAINING_SPACE_TYPES).map(s=><p><b>{s.emoji} {s.label}</b><span>{s.desc}</span></p>)}</div><h3>修行道具</h3><div className="mh-rules-list">{Object.entries(TRAINING_TOOLS).map(([id,t])=><p><b>{t.emoji} {t.name}</b><span>使用：{t.timing}<br/>効果：{t.desc}</span></p>)}</div></>:trainingModal.type==='route'?<><h3>選択ルート確認</h3>{trainingModal.paths.map((p,i)=><button className="mh-route-choice" onClick={()=>moveTrainingPath(p)}>経路 {i+1}: {p.join(' → ')}</button>)}</>:trainingModal.type==='tool'?<><h3>{TRAINING_TOOLS[trainingModal.id].emoji} {TRAINING_TOOLS[trainingModal.id].name}</h3><p>使用可能なタイミング：{TRAINING_TOOLS[trainingModal.id].timing}</p><p>正確な効果：{TRAINING_TOOLS[trainingModal.id].desc}</p>{!['noReturn','returnCharm'].includes(trainingModal.id)&&<button className="mh-route-choice" onClick={()=>{useTrainingTool(trainingModal.id);setTrainingModal(null)}}>使用する</button>}</>:trainingModal.type==='discard'?<><h3>道具の所持上限（3個）</h3><p>捨てる道具を選ぶか、新しい道具を諦めてください。</p>{trainingSession.tools.map((id,i)=><button className="mh-route-choice" onClick={()=>{const tools=[...trainingSession.tools];tools.splice(i,1,trainingModal.newTool);patchTraining({tools});setTrainingModal(null)}}>{TRAINING_TOOLS[id].name}を捨てる</button>)}<button className="mh-route-choice" onClick={()=>setTrainingModal(null)}>新しい道具を諦める</button></>:trainingModal.type==='rewards'?<><h3>仮報酬</h3><p>絆経験値：{trainingSession?.rewards.bondXp||0}<br/>ダイヤ：{trainingSession?.rewards.diamonds||0}<br/>通常アイテム：{trainingSession?.rewards.items.length||0}個</p></>:<><h3>{trainingModal.space?.emoji} {trainingModal.space?.label}</h3><p>{trainingModal.space?.desc}</p></>}<button className="mh-modal-close" onClick={()=>setTrainingModal(null)}>閉じる</button></div></div>}
        ...
        {gameState==='BATTLE_MENU'&&(
          <div className="flex-1 flex flex-col h-full min-h-0 px-4" style={{paddingTop:'calc(clamp(1.5rem, 4vh, 2.5rem) + env(safe-area-inset-top))',paddingBottom:'calc(1rem + env(safe-area-inset-bottom))'}}>
            ...
            <div className="grid grid-cols-2 gap-1.5 mb-2 shrink-0 rounded-xl bg-slate-900/60 p-1 border border-white/5">
      ...
      {showEnemyInfo&&enemy&&(<div className="fixed inset-0 p-6 flex flex-col" style={{position:'fixed',inset:0,backgroundColor:'#020617',zIndex:40000,paddingTop:'calc(1.5rem + env(safe-area-inset-top))',paddingBottom:'calc(1.5rem + env(safe-area-inset-bottom))'}}><div className="flex justify-between items-center mb-6 border-b border-white/10 pb-4 shrink-0"><h3 className="font-black italic uppercase text-red-500 text-lg">Enemy Scan</h3><button onClick={()=>setShowEnemyInfo(false)} className="px-6 py-2 bg-white/10 rounded-full text-[11px] text-white active:scale-90">戻る</button></div><div className="flex-1 min-h-0 flex flex-col items-center justify-center text-center overflow-y-auto mh-scroll">{enemy.imgUrl?(<img src={enemy.imgUrl} alt={enemy.name} style={enemy.id==='Moo'?{width:'clamp(252px,70vw,280px)',height:'clamp(252px,70vw,280px)'}:{width:'140px',height:'140px'}} className={`mx-auto object-contain drop-shadow-[0_0_50px_rgba(239,68,68,0.4)] ${enemy.id==='Moo'?'mb-2':'mb-6'}`}/>):(<div style={{fontSize:'112px'}} className="mb-6 drop-shadow-[0_0_50px_rgba(239,68,68,0.4)]">{enemy.emoji}</div>)}<h4 className="text-2xl font-black italic mb-6 uppercase shrink-0">{enemy.name}</h4><div className="w-full max-w-sm space-y-4 bg-slate-900/50 p-6 rounded-3xl border border-white/5 shrink-0"><div className="grid grid-cols-2 gap-6 text-left"><div><div className="text-[9px] text-pink-400 font-black uppercase">ライフ</div><div className="text-xl font-mono font-black">{enemy.hp.toLocaleString()}</div></div><div><div className="text-[9px] text-red-400 font-black uppercase">攻撃力</div><div className="text-xl font-mono font-black">{enemy.atk}</div></div></div></div></div></div>)}
