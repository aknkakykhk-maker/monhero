      ...
      <div className="relative z-10 h-full flex flex-col" style={screenShake?{animation:bigShake?'mooQuake 750ms ease-in-out':'screenShake 450ms ease-in-out'}:undefined}>
        ...
        {gameState==='PASTURE_SETTINGS'&&(
          <div className="flex-1 flex flex-col h-full min-h-0 p-4">
            ...
            <div className="flex-1 min-h-0 overflow-y-auto mh-scroll">
              <div className="grid grid-cols-3 gap-2.5 pb-4">
                {sortMonsterEntries(buildUnifiedMonsterEntries([],masuMons,monsterRosterIds)).filter(e=>e.type==='masu').map(({masu,base})=>{
                  ...
                  return <button key={id} disabled={disabled} onClick={()=>toggleDraftPasture(id)} aria-pressed={selected} className={`relative rounded-2xl border-2 p-2 flex flex-col items-center gap-1 active:scale-95 ${selected?'border-emerald-300 bg-emerald-950/80 ring-2 ring-emerald-400/30':'border-slate-800 bg-slate-900'} disabled:opacity-35`}>
                    ...
                    <div className="relative w-12 h-12"><div className="w-12 h-12 rounded-full overflow-hidden border border-pink-400/40"><DyedMonsterImage baseId={masu.baseId} src={base.iconUrl} alt={masu.name} masuColors={getMasuColors(masu)} className="w-full h-full object-cover"/></div><RebirthStars count={masu.rebirthCount} className="absolute bottom-0 inset-x-0"/></div>
