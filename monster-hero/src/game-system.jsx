...
const HomeWalkingMasumon = ({ masu, base, masuColors }) => {
  const [motion, setMotion] = useState({ x: 50, y: 55, facing: 1, walking: false, duration: 0 });
  ...
  return <div className={`mh-home-masumon ${motion.walking ? 'is-walking' : ''}`} style={{left:`${motion.x}%`,top:`${motion.y}%`,transitionDuration:`${motion.duration}ms`}}>
...
function MonsterHeroGame() {
  ...
  useEffect(() => {
    if (gameState !== 'HOME') { wasHomeRef.current = false; return; }
      ...
      <div className="relative z-10 h-full flex flex-col" style={screenShake?{animation:bigShake?'mooQuake 750ms ease-in-out':'screenShake 450ms ease-in-out'}:undefined}>
        ...
        {gameState==='HOME'&&(
          <main className="mh-home-scene" aria-label="村の広場">
            ...
            <div className="mh-home-masumon-layer" aria-hidden="true">{homeMasumon&&homeMasumonBase&&<HomeWalkingMasumon masu={homeMasumon} base={homeMasumonBase} masuColors={getMasuColors(homeMasumon)}/>}</div>
