          ...
          <main className="mh-home-scene" aria-label="村の広場">
            ...
            <nav className="mh-home-menu-layer" aria-label="ホームメニュー">
              {[
                ...
                {label:'バトル', icon:<Sword size={29}/>, featured:true, action:()=>{setBattleMenuTab('difficulty');setGameState('BATTLE_MENU');}},
              ...
              ].map(item=><button key={item.label} onClick={item.action} className={item.featured?'is-battle':''}><span>{item.icon}</span><b>{item.label}</b></button>)}
            </nav>
