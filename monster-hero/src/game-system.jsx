...
const createTrainingSession=(masuId,difficulty='BEGINNER')=>({status:'playing',masuId:String(masuId),difficulty,mapId:TRAINING_MAP_ID,seed:trainingSeed(),position:'n0',previous:null,remainingTurns:10,rewards:trainingEmptyRewards(),tools:[],effects:{},lastRoll:null,branchOptions:[],movementRemaining:0,forcedMoves:0,eventLog:['修行テスト開始'],message:'サイコロを振ってください'});
...
function MonsterHeroGame() {
  ...
  useEffect(()=>{if(gameState==='TRAINING_BOARD'&&!trainingMapOverview)requestAnimationFrame(()=>trainingPieceRef.current?.scrollIntoView({behavior:'smooth',block:'center',inline:'center'}));},[gameState,trainingSession?.position,trainingMapOverview]);
  ...
  const applyTrainingSpace=async base=>{const node=TRAINING_NODE_BY_ID[base.position],space=TRAINING_SPACE_TYPES[node.type];let next={...base,rewards:{...base.rewards,items:[...base.rewards.items]},tools:[...base.tools],effects:{...base.effects},branchOptions:[],movementRemaining:0,message:space.desc,eventLog:logTraining(base,`${space.label}: ${space.desc}`)};setTrainingModal({type:'effect',space});
  ...
  const advanceTraining=async(base,steps,chosenFirst=null)=>{if(trainingMovingRef.current)return;trainingMovingRef.current=true;let next={...base,branchOptions:[],movementRemaining:steps},chosen=chosenFirst;while(next.movementRemaining>0){const options=trainingForwardOptions(next.position);if(!options.length)break;if(options.length>1&&!chosen){trainingMovingRef.current=false;setTrainingSession({...next,branchOptions:options,message:`分岐です。進む方向を選んでください（あと${next.movementRemaining}マス）`});return;}const id=chosen&&options.includes(chosen)?chosen:options[0];chosen=null;const previous=next.position;next={...next,position:id,previous,movementRemaining:next.movementRemaining-1,branchOptions:[],message:`あと ${next.movementRemaining-1} マス`};Audio_.se.trainingMove();setTrainingSession(next);await new Promise(r=>setTimeout(r,320));if(id==='n23'){trainingMovingRef.current=false;return finishTraining(true,next);}}trainingMovingRef.current=false;setTrainingSession(next);await applyTrainingSpace(next);};
  const rollTrainingDice=(fixedValue=null)=>{if(!trainingSession||trainingSession.status!=='playing'||trainingMovingRef.current||trainingSession.branchOptions.length)return;Audio_.se.trainingDice();const roll=()=>1+Math.floor(Math.random()*3);let value=fixedValue||trainingDebugRoll||roll();if(trainingSession.effects.gale)value=Math.max(value,roll());if(trainingSession.effects.boost)value++;if(trainingSession.effects.feather)value+=2;const effects={...trainingSession.effects};delete effects.gale;delete effects.boost;delete effects.feather;delete effects.fixed;const next={...trainingSession,lastRoll:value,effects,remainingTurns:trainingSession.remainingTurns-1,forcedMoves:0,message:`${value}が出ました。自動で進みます`,eventLog:logTraining(trainingSession,`サイコロ ${value}`)};setTrainingSession(next);advanceTraining(next,value);};
  ...
  const useTrainingTool=id=>{const t=TRAINING_TOOLS[id];setTrainingModal({type:'tool',id});if(!t||['noReturn','returnCharm'].includes(id))return;const beforeRoll=!trainingSession.movementRemaining&&!trainingSession.branchOptions.length;if(['feather','gale','fixed'].includes(id)&&!beforeRoll)return;if(id==='reroll'&&!trainingSession.branchOptions.length)return;const tools=[...trainingSession.tools];tools.splice(tools.indexOf(id),1);let effects={...trainingSession.effects},remainingTurns=trainingSession.remainingTurns,branchOptions=trainingSession.branchOptions,lastRoll=trainingSession.lastRoll;if(id==='sand')remainingTurns++;else if(id==='reroll'){const value=1+Math.floor(Math.random()*3);const next={...trainingSession,tools,branchOptions:[],movementRemaining:value,lastRoll:value,message:`残り歩数を${value}に振り直しました`};Audio_.se.trainingTool();setTrainingSession(next);advanceTraining(next,value);return;}else effects[id]=true;Audio_.se.trainingTool();patchTraining({tools,effects,remainingTurns,branchOptions,lastRoll,eventLog:logTraining(trainingSession,`${t.name}使用`)});};
  ...
  return (
    <div onPointerDown={(e)=>{const rect=e.currentTarget.getBoundingClientRect(); spawnRipple(e.clientX-rect.left, e.clientY-rect.top);}} className="h-full w-full bg-slate-950 text-white overflow-hidden relative select-none font-sans" style={{height:'100%'}}>
      ...
      <div className="relative z-10 h-full flex flex-col" style={screenShake?{animation:bigShake?'mooQuake 750ms ease-in-out':'screenShake 450ms ease-in-out'}:undefined}>
        ...
        {gameState==='TRAINING_BOARD'&&trainingSession&&(()=>{const m=masuMons.find(x=>String(x.id)===String(trainingSession.masuId));const current=TRAINING_NODE_BY_ID[trainingSession.position];return <main className="mh-training-board"><header><div><b>{m?.name||'マスモン'} <small>BEGINNER・DEBUG保存なし</small></b><span>残り {trainingSession.remainingTurns}ターン　・　ゴールまであと {trainingDistanceToGoal(current.id)}マス</span></div><button className="mh-debug-toggle" onClick={()=>setTrainingDebugOpen(v=>!v)}>DEBUG</button></header><section className="mh-training-hud"><span>💗 仮XP <b>{trainingSession.rewards.bondXp}</b></span><span>💎 仮ダイヤ <b>{trainingSession.rewards.diamonds}</b></span><span>🎁 仮アイテム <b>{trainingSession.rewards.items.length}</b></span></section><div ref={trainingMapRef} className={`mh-tile-viewport ${trainingMapOverview?'overview':''}`}><div className="mh-tile-board">{TRAINING_BEGINNER_NODES.map(n=><React.Fragment key={n.id}>{n.next.filter(id=>TRAINING_BEGINNER_NODES.findIndex(x=>x.id===id)>TRAINING_BEGINNER_NODES.findIndex(x=>x.id===n.id)).map(id=>{const to=TRAINING_NODE_BY_ID[id],dx=to.x-n.x,dy=to.y-n.y;return <i key={id} style={{left:`${n.x}%`,top:`${n.y}%`,width:`${Math.hypot(dx,dy)}%`,transform:`rotate(${Math.atan2(dy,dx)*180/Math.PI}deg)`}}/>})}<button ref={n.id===current.id?trainingPieceRef:null} style={{left:`${n.x}%`,top:`${n.y}%`,'--tile-color':TRAINING_SPACE_TYPES[n.type].color}} className={`mh-training-tile ${n.id===current.id?'current':''} ${trainingSession.branchOptions.includes(n.id)?'branch-choice':''} ${trainingForwardOptions(n.id).length>1?'branch':''} ${n.type==='goal'?'goal':''} ${n.type==='start'?'start':''}`} onClick={()=>trainingSession.branchOptions.includes(n.id)?chooseTrainingBranch(n.id):setTrainingModal({type:'space',space:TRAINING_SPACE_TYPES[n.type]})}><small>{n.type==='start'?'START':n.type==='goal'?'GOAL':TRAINING_SPACE_TYPES[n.type].label}</small><span>{TRAINING_SPACE_TYPES[n.type].emoji}</span>{n.id===current.id&&m&&<div className="mh-training-piece"><DyedMonsterImage baseId={m.baseId} src={ALL_PLAYER_MONSTERS[m.baseId]?.iconUrl} alt={m.name} masuColors={getMasuColors(m)}/><b>{m.name}</b></div>}</button></React.Fragment>)}</div></div><div className="mh-board-buttons"><button onClick={()=>setTrainingMapOverview(v=>!v)}>{trainingMapOverview?'📍 現在地へ':'🗺️ 全体マップ'}</button><button onClick={()=>setTrainingModal({type:'rules'})}>マス詳細</button></div><p className="mh-training-message">{trainingSession.message}</p><section className="mh-training-tools"><strong>修行道具</strong>{trainingSession.tools.length?trainingSession.tools.map((id,i)=><button key={`${id}-${i}`} onClick={()=>setTrainingModal({type:'tool',id})}><span>{TRAINING_TOOLS[id].emoji}</span><small>{TRAINING_TOOLS[id].name}<br/>{TRAINING_TOOLS[id].timing}</small></button>):<p>所持なし（最大3個）</p>}</section><footer>{trainingSession.effects.fixed?<div className="mh-fixed-dice"><span>確定サイコロ</span>{[1,2,3].map(n=><button key={n} onClick={()=>rollTrainingDice(n)}>{n}</button>)}</div>:<button disabled={trainingSession.movementRemaining>0||trainingSession.branchOptions.length>0||trainingMovingRef.current} onClick={()=>rollTrainingDice()} className="mh-roll-button">🎲 サイコロを振る<small>{trainingSession.branchOptions.length?'分岐方向を選んでください':'出目ぶん自動で進みます'}</small></button>}</footer>{trainingDebugOpen&&<aside className="mh-training-debug"><b>DEBUG PANEL</b><button className="mh-debug-close" onClick={()=>setTrainingDebugOpen(false)}>閉じる</button><div>次の出目 {[1,2,3].map(n=><button key={n} onClick={()=>setTrainingDebugRoll(n)} className={trainingDebugRoll===n?'active':''}>{n}</button>)}<button onClick={()=>setTrainingDebugRoll(null)}>解除</button></div><div><button onClick={()=>patchTraining({remainingTurns:trainingSession.remainingTurns+1})}>ターン＋1</button><button onClick={()=>patchTraining({remainingTurns:Math.max(0,trainingSession.remainingTurns-1)})}>－1</button></div><select onChange={e=>{if(e.target.value)addTrainingTool(e.target.value)}} defaultValue=""><option value="">道具を追加</option>{Object.entries(TRAINING_TOOLS).map(([id,t])=><option key={id} value={id}>{t.name}</option>)}</select><button onClick={()=>finishTraining(true)}>強制成功</button><button onClick={()=>finishTraining(false)}>強制失敗</button><button onClick={restartTraining}>最初から</button><pre>map: {trainingSession.mapId}{'\n'}seed: {trainingSession.seed}{'\n'}{trainingSession.eventLog.join('\n')}</pre></aside>}</main>})()}
