...
const MASU_COLOR_REGION_HUES = {
  ...
  Mocchi: [{ hue: 350, sMin: 0.08 }, { hue: 92, sMin: 0.2 }, { hue: 45, sMin: 0.12, bbox: [0.30, 0.23, 0.70, 0.35] }],
...
const buildGiftClaim = (gift, balances, now=Date.now()) => {
  ...
  const itemIds = { dyeMock:'dye_mock', bondPointReset:'bond_reset_scroll', trainingTicket:'training_ticket', trainingTicketLarge:'training_ticket_l', skipTicketJo:'skip_ticket_jo', skipTicketHa:'skip_ticket_ha', skipTicketKyu:'skip_ticket_kyu' };
...
const MISSION_DEFS = {
  daily: [
    ...
    {id:'daily_complete',name:'デイリーコンプリート',condition:'通常デイリー4個をすべて達成する',key:'complete',target:4,rewards:[{type:'diamond',amount:500},{type:'skipTicketHa',amount:1}],complete:true},
  ],
  weekly: [
...
function MonsterHeroGame() {
  ...
  const [patternCustomColor, setPatternCustomColor] = useState({open:false,h:198,s:0.77,v:0.97});
  ...
  const monsterCardName = (node, className='text-white') => <div className={`text-[10px] font-black truncate w-full text-center leading-tight ${className}`} style={{height:'14px'}}>{node}</div>;
  const monsterCardInfo = (node) => <div className="w-full" style={{height:'22px'}}>{node||null}</div>;
  const monsterCardSub = (node) => <div className="w-full flex items-center justify-center" style={{height:'13px'}}>{node||null}</div>;
  const monsterCardStatus = (node) => <div className="w-full flex items-center justify-center" style={{height:'18px'}}>{node||null}</div>;
  // マスモンの絆Lvと進捗バー(カードの情報行に入れる小さい版)
  const monsterCardBond = (lvl) => (<>
    <div className="text-[8px] text-pink-300 font-black flex items-center gap-0.5 leading-tight"><Heart size={7}/>絆Lv.{lvl.level}</div>
    <div className="w-full h-1 bg-slate-800 rounded-full overflow-hidden border border-pink-500/20 mt-0.5"><div className="h-full bg-gradient-to-r from-pink-500 to-rose-400" style={{width:`${Math.max(0,Math.min(100,(lvl.xpIntoLevel/Math.max(1,lvl.xpForNext))*100))}%`}}></div></div>
  </>);
  ...
  const executeBattleSkip = async () => {
    ...
    try {
      ...
      if (gainedBreederLevels > 0) {
        setBreederPoints(prev => { const next = prev + gainedBreederLevels; storeSet('mh_breeder_points', next, false); return next; });
        storeSet('mh_breeder_points_granted', Math.max(0, breederLevelAfter.level - 1), false);
      }
      ...
      setSkipResult({
        ...
        breederXpGain, breederLevelBefore, breederLevelAfter, goldGain, goldBefore, goldAfter,
        heroBondGain, allyBondGains,
      });
  ...
  const skipPickMon = (mon) => {
    setSkipFlow(prev => {
      ...
      if (!prev.hero) return { ...prev, hero: mon };
      if ((prev.allies || []).length >= 3) return prev;
      return { ...prev, allies: [...(prev.allies || []), mon] };
    });
  };
  const skipClearSlot = (index) => {
    setSkipFlow(prev => {
      if (!prev) return prev;
      if (index === 0) return { ...prev, hero: null, allies: [] }; // 勇者モンを外すと供モンも外す
  ...
  useEffect(() => {
    if (tutorialStep == null || !tutorialSpot) { setTutorialPanelAtTop(false); return; }
    // 光らせた要素が描かれてから測る
    const id = requestAnimationFrame(() => {
      ...
      } catch { setTutorialPanelAtTop(false); }
    });
    return () => cancelAnimationFrame(id);
  }, [tutorialStep, tutorialSpot]);
  const startTutorial = (kind = 'tour') => { tutorialShownRef.current = true; setShowTitleSettings(false); setTutorialKind(kind); setTutorialStep(0); };
  ...
  const renderLoginBonusList = (todayDay) => (
    <div className="text-left">
      <div className="text-[10px] font-black text-slate-400 mb-1 tracking-wider">7日間のログインボーナス</div>
      <div className="max-h-[34vh] overflow-y-auto mh-scroll space-y-1 pr-0.5">
        {LOGIN_BONUS_REWARDS.map((rewards, idx) => {
          const day = idx + 1;
          const phase = day < todayDay ? 'done' : day === todayDay ? 'today' : 'next';
          return (
            <div key={day} className={`rounded-xl border px-2.5 py-1.5 flex items-start gap-2 ${phase==='today'?'bg-amber-500/15 border-amber-300':phase==='done'?'bg-black/30 border-white/10 opacity-60':'bg-black/25 border-white/10'}`}>
              <div className="shrink-0 w-12 text-center">
                <div className={`text-[11px] font-black leading-tight ${phase==='today'?'text-amber-200':'text-slate-300'}`}>{day}日目</div>
                <div className={`text-[7px] font-black leading-tight ${phase==='today'?'text-amber-300':phase==='done'?'text-emerald-400':'text-slate-500'}`}>{phase==='today'?'今日':phase==='done'?'受取済み':'これから'}</div>
              </div>
              <div className="flex-1 min-w-0 text-[10px] font-bold text-cyan-200 leading-tight break-words">{rewards.map(r=>giftRewardText(r)).join('　/　')}</div>
            </div>
          );
        })}
      </div>
  ...
  const getNextEnemyAction = useCallback((ent, currentDist) => {
    ...
    if (scenario && typeof battleScenarioIntent === 'function') {
      ...
      if (scripted) { battleScenarioIntentIndexRef.current += 1; return scripted; }
    }
  ...
  const processTurn = async () => {
    ...
    await handleEnemyTurn(finalActionType,{invincible:immediateInvincible,stun:immediateStun,guardFlat:currentTurnGuardFlat,guardMult:currentTurnGuardMult,distLocked:forcedMoveTarget!=null},executedIntent);
  ...
  const spawnEnemy = useCallback((w, forcedEnemyKey=null, initialDistance=null) => {
    ...
    const dist=Number.isInteger(scriptedDist)?scriptedDist
      :(Number.isInteger(initialDistance)&&initialDistance>=0&&initialDistance<RANGE_LABELS.length
        ? initialDistance
        : Math.floor(Math.random()*4));
    if (scenario) {
      if (Number.isFinite(Number(scenario.enemyHp))) { newEnemy.hp=Math.max(1,Math.floor(scenario.enemyHp)); newEnemy.maxHp=newEnemy.hp; }
      if (Number.isFinite(Number(scenario.enemyAtk))) { newEnemy.atk=Math.max(0,Math.floor(scenario.enemyAtk)); }
  ...
  const battleTutorialNeed = (battleTutorial && (battleTutorial.wait === 'do' || battleTutorial.wait === 'act'))
    ? (battleTutorial.need || null) : null;
  ...
  useEffect(() => {
    ...
    const next = (typeof findBattleTutorialStep === 'function')
  ...
  useEffect(() => {
    if (skillPicker) { skillPickerOpenRef.current = true; return; }
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      if (!Number.isFinite(top) || !Number.isFinite(bottom)) { setBattleTutorialAtBottom(false); return; }
      ...
      <div className="relative z-10 h-full flex flex-col" style={screenShake?{animation:bigShake?'mooQuake 750ms ease-in-out':'screenShake 450ms ease-in-out'}:undefined}>
          ...
          <main className="mh-home-scene" aria-label="村の広場">
            ...
            <header className={`mh-home-status${spotClass('settings')}`}>
              ...
              <section className="mh-home-wallet">
                ...
                <button onClick={()=>setGameState('SETTINGS')} className={`mh-home-settings${spotClass('settings')}`} aria-label="設定"><Settings size={20}/><span>設定</span></button>
              </section>
            </header>
        ...
        {gameState==='MASU_DONATION'&&(()=>{
          ...
          return <div className="flex-1 flex flex-col h-full min-h-0 p-3" style={{paddingTop:'calc(.75rem + env(safe-area-inset-top))',paddingBottom:'calc(.75rem + env(safe-area-inset-bottom))'}}>
            ...
            <p className="text-[10px] text-slate-300 leading-relaxed bg-violet-950/40 border border-violet-500/30 rounded-xl px-3 py-2 mb-2 shrink-0">累計絆経験値と同じ数のダイヤを受け取れます</p><div className="shrink-0 w-full max-w-md mx-auto mb-3"><AssistantBubble scene="donation" compact/></div>
            ...
            <div className="flex-1 min-h-0 overflow-y-auto mh-scroll">
              {masuMons.length===0?<div className="flex flex-col items-center justify-center h-full text-center text-slate-500"><Gem size={42}/><p className="text-[11px] mt-3 font-bold">寄付できるマスモンがいません</p></div>:<div className="grid grid-cols-3 gap-1.5 pb-4">{sorted.map(masu=>{const base=ALL_PLAYER_MONSTERS[masu.baseId];if(!base)return null;const diamonds=donationDiamondValue(masu.bondXp);const lvl=bondLevelInfo(diamonds);const active=monsterRosterIds.includes(`masu:${masu.id}`);return <button key={masu.id} disabled={donationProcessing} onClick={()=>{setDonationError('');setDonationSelectedId(masu.id);}} className="min-w-0 overflow-hidden bg-slate-900 border border-violet-500/30 rounded-xl p-1.5 flex flex-col items-center text-center active:scale-[.97] disabled:opacity-50">
                <div className="relative w-16 h-16 rounded-lg overflow-hidden bg-black/30"><DyedMonsterImage baseId={masu.baseId} src={masuDisplayImageUrl(base)} alt={masu.name} masuColors={getMasuColors(masu)} className="w-full h-full object-contain"/><RebirthStars count={masu.rebirthCount} className="mh-rebirth-stars-overlay"/>{active&&<span className="absolute top-1 left-1 right-1 text-[7px] leading-4 bg-pink-600/95 text-white rounded-full font-black">編成中</span>}</div>
        ...
        {gameState==='BATTLE_MENU'&&(
          <div className="flex-1 flex flex-col h-full min-h-0 px-4" style={{paddingTop:'calc(.35rem + env(safe-area-inset-top))',paddingBottom:'calc(.35rem + env(safe-area-inset-bottom))'}}>
            ...
            {/* ランキングへの導線。モードのタブのすぐ下に、タブ2つを合わせたのと同じ幅で置く。
            ...
            {battleMenuTab==='difficulty'&&(()=>{
              ...
              const recordBox=(key)=>quick
                ? { label:'最高到達WAVE', value:`WAVE ${waveOf(key)}`, valueColor:'text-amber-300', sub:`クリア ${quickClearCounts[key]||0}回` }
        ...
        {gameState==='MASU_PATTERN_DEBUG'&&(()=>{
          ...
          return <main className="flex-1 min-h-0 flex flex-col bg-slate-950" style={{paddingTop:'env(safe-area-inset-top)',paddingBottom:'env(safe-area-inset-bottom)'}}>
            ...
            {eligible.length===0?<section className="flex-1 flex items-center justify-center p-6 text-center font-black text-slate-300">カスタマイズできる所持マスモンがいません</section>:!selected?<section className="flex-1 min-h-0 overflow-y-auto mh-scroll p-4"><p className="mb-3 text-[11px] font-bold text-slate-400">所持マスモンを1体選択してください。</p><div className="grid grid-cols-3 gap-2">{eligible.map(m=>{const base=ALL_PLAYER_MONSTERS[m.baseId];return <button key={m.id} onClick={()=>{setPatternMasuId(m.id);resetPattern();}} className="min-h-[116px] rounded-2xl border border-fuchsia-500/30 bg-slate-900 p-2"><DyedMonsterImage baseId={m.baseId} src={masuDisplayImageUrl(base)} alt={m.name} masuColors={getMasuColors(m)} className="w-16 h-16 mx-auto object-contain"/><b className="block truncate text-[10px]">{m.name}</b></button>})}</div></section>:(()=>{
              ...
              const patchActive=patch=>setPatternSettings(prev=>{if(prev.mode==='all')return {...prev,fullPattern:{...prev.fullPattern,...patch}};if(prev.mode==='region'){const i=prev.selectedLayer.startsWith('region:')?Number(prev.selectedLayer.split(':')[1]):0;return {...prev,regionPatterns:{...prev.regionPatterns,[i]:{...(prev.regionPatterns[i]||makePatternLayer({target:String(i+1)})),...patch}}};}const id=prev.selectedLayer.split(':')[1];return {...prev,decals:prev.decals.map(d=>String(d.id)===id?{...d,...patch}:d)};});
              const selectMode=next=>setPatternSettings(prev=>({...prev,mode:next,selectedLayer:next==='all'?'full':next==='region'?`region:${Math.min(regionIndex,Math.max(0,regions-1))}`:(prev.decals.length?`decal:${prev.decals[prev.decals.length-1].id}`:'')}));
              const addDecal=(position={x:.5,y:.5})=>setPatternSettings(prev=>{const id=`point-${Date.now()}-${prev.decals.length+1}`,decal=makePatternLayer({id,pattern:'star',target:'all',x:position.x,y:position.y,size:.08,opacity:100});return {...prev,mode:'point',decals:[...prev.decals,decal],selectedLayer:`decal:${id}`};});
              const patchDecal=(id,patch)=>setPatternSettings(prev=>({...prev,decals:prev.decals.map(d=>d.id===id?{...d,...patch}:d)}));
              const deleteDecal=id=>setPatternSettings(prev=>{const decals=prev.decals.filter(d=>d.id!==id),fallback=decals[decals.length-1];return {...prev,decals,selectedLayer:fallback?`decal:${fallback.id}`:''};});
              const moveDecal=(id,delta)=>setPatternSettings(prev=>{const decals=[...prev.decals],from=decals.findIndex(d=>d.id===id),to=Math.max(0,Math.min(decals.length-1,from+delta));if(from<0||from===to)return prev;const [item]=decals.splice(from,1);decals.splice(to,0,item);return {...prev,decals};});
              const duplicateDecal=d=>setPatternSettings(prev=>{const copy={...d,id:`point-${Date.now()}-${prev.decals.length+1}`,x:Math.min(1,d.x+.05),y:Math.min(1,d.y+.05)};return {...prev,decals:[...prev.decals,copy],selectedLayer:`decal:${copy.id}`};});
              ...
              return <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
                ...
                <section className="flex-1 min-h-0 mt-2 rounded-t-3xl border-t border-fuchsia-500/30 bg-slate-900/95 flex flex-col overflow-hidden">
                  ...
                  <nav className="grid grid-cols-4 gap-1 px-2 pt-1 border-t border-white/10 shrink-0" style={{paddingBottom:'max(.4rem,env(safe-area-inset-bottom))'}}>{[['attach','付け方'],['pattern','模様'],['color','色'],['layers','レイヤー']].map(([id,label])=><button key={id} onClick={()=>setPatternStep(id)} className={`min-h-[48px] rounded-xl text-[9px] font-black ${patternStep===id?'bg-fuchsia-700 text-white':'bg-slate-800 text-slate-400'}`}>{label}</button>)}</nav>
                </section>
                {patternSizePreview&&<div className="fixed inset-0 z-[100] bg-black/80 flex items-end justify-center" onClick={()=>setPatternSizePreview(false)}><section onClick={e=>e.stopPropagation()} className="w-full max-w-md rounded-t-3xl bg-slate-900 border-t border-fuchsia-400 p-4 pb-[max(1rem,env(safe-area-inset-bottom))]"><div className="flex justify-between"><h3 className="font-black">サイズ確認</h3><button onClick={()=>setPatternSizePreview(false)} className="px-4 py-2 rounded-xl bg-slate-700">閉じる</button></div><div className="flex items-end justify-around text-center mt-3"><div><PatternedMasuImage masu={selected} base={base} colors={colors} settings={patternSettings} className="w-24 h-24"/><small>通常カード</small></div><div><PatternedMasuImage masu={selected} base={base} colors={colors} settings={patternSettings} className="w-11 h-11 mx-auto"/><small>小型アイコン</small></div></div></section></div>}
              </div>;
            })()}
          </main>;
        })()}
        ...
        {gameState==='MONSTER_IMAGE_DEBUG'&&(()=>{
          ...
          const sources={imgUrl:tigerTest?testUrl:base.imgUrl,iconUrl:tigerTest?testUrl:base.iconUrl,faceIconUrl:tigerTest?testUrl:base.faceIconUrl};
          ...
          const recordSize=(key)=>(e)=>{const next=`${e.currentTarget.naturalWidth} × ${e.currentTarget.naturalHeight}px`;setMonsterImageDebugSizes(old=>old[key]===next?old:{...old,[key]:next});};
          const frame=(label,key,content,frameClass='w-20 h-20 rounded-full',fit='object-cover')=><div className="text-center"><div className={`${frameClass} mx-auto overflow-hidden border-2 border-white/30 flex items-center justify-center`} style={bgStyle}>{content(fit)}</div><b className="block mt-1 text-[9px] text-slate-200">{label}</b></div>;
          return <main className="flex-1 flex flex-col h-full min-h-0 p-3" style={{paddingTop:'calc(.75rem + env(safe-area-inset-top))',paddingBottom:'calc(.75rem + env(safe-area-inset-bottom))'}}>
            <header className="flex items-center gap-2 mb-2 shrink-0"><button onClick={()=>setGameState('DEBUG_SETTINGS')} className="p-3 text-slate-400"><ArrowLeft size={20}/></button><div><small className="text-[8px] font-black text-cyan-400">DEBUG・保存されません</small><h2 className="text-sm font-black">モンスター画像・染色確認</h2></div></header>
            <div className="flex-1 min-h-0 overflow-y-auto mh-scroll space-y-3 pb-3">
              <select value={base.id} onChange={e=>{setMonsterImageDebugId(e.target.value);setMonsterImageDebugTigerNew(false);}} className="w-full min-h-[46px] rounded-xl bg-slate-900 border border-white/10 px-3 text-xs font-black">{Object.values(ALL_PLAYER_MONSTERS).map(mon=><option key={mon.id} value={mon.id}>{mon.name}（{mon.id}）</option>)}</select>
              {base.id==='Tiger'&&<div className="grid grid-cols-2 gap-2">{[[false,'現在の本番画像'],[true,'新画像で全差し替えテスト']].map(([value,label])=><button key={label} onClick={()=>setMonsterImageDebugTigerNew(value)} className={`min-h-[48px] rounded-xl px-2 text-[9px] font-black border ${monsterImageDebugTigerNew===value?'bg-amber-700 border-amber-300':'bg-slate-900 border-white/10'}`}>{label}</button>)}</div>}
              <div className="grid grid-cols-3 gap-2">{[['checker','市松模様'],['white','白'],['black','黒']].map(([id,label])=><button key={id} onClick={()=>setMonsterImageDebugBg(id)} className={`min-h-[42px] rounded-xl text-[10px] font-black border ${monsterImageDebugBg===id?'border-cyan-300 ring-2 ring-cyan-500':'border-white/10'}`} style={id==='white'?{background:'#fff',color:'#000'}:id==='black'?{background:'#000',color:'#fff'}:{background:'#64748b'}}>{label}</button>)}</div>
              <section className="rounded-2xl border border-cyan-500/30 bg-slate-900/70 p-3"><h3 className="mb-3 text-[10px] font-black text-cyan-300">本番相当の表示形式</h3><div className="grid grid-cols-3 gap-3 items-end">
                {frame('バトル／立ち絵','imgUrl',(fit)=><DyedMonsterImage baseId={base.id} src={sources.imgUrl} alt={base.name} masuColors={colors} className={`w-full h-full ${fit} drop-shadow-md`}/>,'w-[70px] h-[90px] rounded-xl','object-contain')}
                {frame('一覧・全身アイコン','iconUrl',(fit)=><DyedMonsterImage baseId={base.id} src={sources.iconUrl} alt={base.name} masuColors={colors} className={`w-full h-full ${fit}`}/>)}
                {frame('詳細・大きな全身','imgUrl',(fit)=><DyedMonsterImage baseId={base.id} src={sources.imgUrl} alt={base.name} masuColors={colors} className={`w-full h-full ${fit} drop-shadow-lg`}/>,'w-28 h-28 rounded-2xl','object-contain')}
                {frame('顔アイコン','faceIconUrl',(fit)=><DyedMonsterImage baseId={base.id} src={sources.faceIconUrl} alt={base.name} masuColors={colors} className={`w-full h-full ${fit}`}/>,'w-16 h-16 rounded-full','object-cover')}
                {frame('プロフィール・選択','faceIconUrl',(fit)=><DyedMonsterImage baseId={base.id} src={sources.faceIconUrl} alt={base.name} masuColors={colors} className={`w-full h-full ${fit}`}/>,'w-12 h-12 rounded-full','object-cover')}
                {frame('小型・編成枠','imgUrl',(fit)=><DyedMonsterImage baseId={base.id} src={sources.imgUrl} alt={base.name} masuColors={colors} className={`w-full h-full ${fit}`}/>,'w-10 h-10 rounded-full','object-contain')}
              </div></section>
              <section className="rounded-2xl border border-fuchsia-500/30 bg-fuchsia-950/20 p-3"><h3 className="text-[10px] font-black text-fuchsia-300">本番の染色処理</h3><div className="grid grid-cols-2 gap-3 my-3 text-center"><div><div className="h-36 rounded-xl border border-white/20 flex items-center justify-center overflow-hidden" style={bgStyle}><img src={sources.imgUrl} alt="元画像" className="w-full h-full object-contain"/></div><b className="text-[9px]">元画像</b></div><div><div className="h-36 rounded-xl border border-fuchsia-400/50 flex items-center justify-center overflow-hidden" style={bgStyle}><DyedMonsterImage baseId={base.id} src={sources.imgUrl} alt="染色後画像" masuColors={colors} className="w-full h-full object-contain"/></div><b className="text-[9px]">染色後画像</b></div></div>
                <div className="grid grid-cols-2 gap-2 mb-2"><button onClick={()=>{setMonsterImageDebugColors(['red','green','blue']);setMonsterImageDebugRegions([true,true,true]);}} className="min-h-[42px] rounded-xl bg-slate-800 text-[9px] font-black">赤／緑／青</button><button onClick={()=>{setMonsterImageDebugColors(['black','white','yellow']);setMonsterImageDebugRegions([true,true,true]);}} className="min-h-[42px] rounded-xl bg-slate-800 text-[9px] font-black">黒／白／黄</button></div>
                {[0,1,2].map(index=><div key={index} className="grid grid-cols-[auto_1fr] gap-2 items-center mb-2 rounded-xl bg-black/30 p-2"><button onClick={()=>setMonsterImageDebugRegions(old=>old.map((v,i)=>i===index?!v:v))} className={`min-h-[40px] min-w-[86px] rounded-lg text-[9px] font-black ${monsterImageDebugRegions[index]?'bg-fuchsia-700':'bg-slate-700'}`}>染色部位{index+1} {monsterImageDebugRegions[index]?'ON':'OFF'}</button><select value={monsterImageDebugColors[index]} onChange={e=>setMonsterImageDebugColors(old=>old.map((v,i)=>i===index?e.target.value:v))} className="min-h-[40px] rounded-lg bg-slate-900 px-2 text-[10px]">{Object.keys(MASU_COLOR_TARGET).map(id=><option key={id} value={id}>{MASU_COLOR_LABELS[id]}</option>)}</select></div>)}
                <button onClick={()=>{setMonsterImageDebugColors(['red','green','blue']);setMonsterImageDebugRegions([true,true,true]);setMonsterImageDebugBg('checker');setMonsterImageDebugTigerNew(false);}} className="w-full min-h-[44px] rounded-xl bg-fuchsia-800 text-[10px] font-black">初期状態へ戻す</button>
              </section>
              <section className="rounded-2xl bg-black/40 p-3 text-[9px] text-slate-300 space-y-2"><h3 className="font-black text-cyan-300">画像パス・元画像サイズ</h3>{Object.entries(sources).map(([key,url])=><div key={key} className="break-all"><b>{key}</b>: {url}<br/><span className="text-slate-500">{monsterImageDebugSizes[url]||'読み込み中…'}</span><img src={url} alt="" className="hidden" onLoad={recordSize(url)}/></div>)}</section>
            </div>
          </main>;
        })()}
                        ...
                        return (
                          <div key={e.key} className="relative">
                            <button onClick={()=>toggleDraftMonster(e.entryId)} style={MONSTER_CARD_STYLE} className={`${MONSTER_CARD_CLASS} ${selected?'bg-indigo-900/40 border-indigo-400 ring-2 ring-indigo-400':'bg-slate-900 border-slate-800'}`}>
                              ...
                              {monsterCardInfo(<div className="text-[8px] text-slate-500 font-bold text-center leading-tight pt-1">ベースモン</div>)}
                              {monsterCardSub(null)}
                  ...
                  return (
                    <div key={e.key} className="relative">
                      <button onClick={()=>setRosterDetailMon(m)} style={MONSTER_CARD_STYLE} className={`${MONSTER_CARD_CLASS} border-slate-800 bg-slate-900`}>
                        <div className={`${MONSTER_CARD_ICON_CLASS} border border-white/10`}><img src={m.iconUrl} alt={m.name} draggable={false} style={{WebkitTouchCallout:'none',WebkitUserSelect:'none',userSelect:'none',pointerEvents:'none'}} className="w-full h-full object-cover"/></div>
                        {monsterCardName(m.name)}
                        {monsterCardInfo(<div className="text-[8px] text-pink-400 font-bold text-center leading-tight pt-1">{masuCount>0?`マスモン${masuCount}体`:'マスモン未登録'}</div>)}
                        {monsterCardSub(null)}
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4" style={{position:'fixed',inset:0,backgroundColor:'rgba(0,0,0,0.92)',zIndex:41000}} role="dialog" aria-modal="true" aria-label={`${marketItemDetail.name}の効果`}>
          <div className="bg-slate-900 border-2 border-teal-500 rounded-3xl p-5 w-full max-w-sm shadow-2xl">
            ...
            <div className="mt-1 flex items-center justify-between text-[11px] font-black">
              ...
              <span className="text-amber-300 font-mono">{Number(marketItemDetail.cost||0).toLocaleString()} ダイヤ</span>
            </div>
        ...
        <div className="fixed inset-0 z-[99999] flex flex-col" style={{position:'fixed',inset:0,backgroundColor:'#000000',zIndex:99999}}>
          ...
          {helpAssistantOpen&&(
            <div className="shrink-0 px-3 py-3 border-b border-white/5" style={{backgroundColor:'rgba(15,23,42,0.75)'}}>
              <AssistantBubble key={`${helpCatId||''}/${helpTopicId||''}`} scene={assistantScene} line={assistantLine} detail={assistantDetail} helpRef={assistantHelpRef} expression={assistantExpression} accent={accent}/>
            </div>
          )}
          <div className="flex-1 min-h-0 overflow-y-auto mh-scroll p-4 bg-black" style={{backgroundColor:'#000000'}}>
            {!cat&&(
              <div className="space-y-2.5">
                ...
                {HELP_GUIDE.map(c=>(
                  <button key={c.id} onClick={()=>{setHelpCatId(c.id);setHelpTopicId(null);}} className="w-full rounded-2xl border-2 px-4 py-3.5 flex items-center gap-3 text-left active:scale-95" style={{borderColor:c.color,backgroundColor:'rgba(15,23,42,0.85)'}}>
                    <span className="shrink-0 text-base leading-none">{c.emoji}{c.subEmoji}</span>
                    <span className="flex-1 min-w-0">
                      <span className="block text-[13px] font-black leading-tight" style={{color:c.color}}>{c.title}</span>
                      <span className="block text-[10px] text-slate-400 leading-tight mt-0.5">{c.summary}</span>
                    </span>
          ...
          <footer className="shrink-0 p-4 bg-slate-900 border-t border-white/10 text-center" style={{backgroundColor:'#0f172a',paddingBottom:'calc(1rem + env(safe-area-inset-bottom))'}}>
            ...
            <button aria-label="" onClick={()=>{const options=getDebugEnemyOptions(difficulty);setDebugEnemyKey(options[0]?.key||null);debugBattleRef.current=false;setDebugBattle(false);setDebugOutcome(null);setShowHelp(false);setGameState('DEBUG_SETTINGS');}} className="mt-5 mx-auto block text-[10px] opacity-25 hover:opacity-40 active:opacity-60">💊</button>
          </footer>
        </div>);
      })()}
