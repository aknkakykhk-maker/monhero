...
const MASU_PATTERN_DEFAULTS = Object.freeze({ mode:'all', pattern:'stripe', target:'all', color:'#38bdf8', opacity:55, size:0.08, spacing:0.16, rotation:0, x:0.5, y:0.5, placed:false });
...
const PatternPlacementPreview = ({ masu, base, colors, settings, onChange, className='' }) => {
  ...
  const down=e=>{if(settings.mode!=='point')return;e.currentTarget.setPointerCapture(e.pointerId);pointers.current.set(e.pointerId,point(e));const ps=[...pointers.current.values()];if(ps.length===1){gesture.current={x:settings.x,y:settings.y,start:ps[0]};onChange({placed:true,x:ps[0].x,y:ps[0].y});}else if(ps.length===2){gesture.current={size:settings.size,rotation:settings.rotation,distance:Math.hypot(ps[1].x-ps[0].x,ps[1].y-ps[0].y),angle:Math.atan2(ps[1].y-ps[0].y,ps[1].x-ps[0].x)};}};
  const move=e=>{if(!pointers.current.has(e.pointerId))return;pointers.current.set(e.pointerId,point(e));const ps=[...pointers.current.values()],g=gesture.current;if(!g)return;if(ps.length===1&&g.start)onChange({x:Math.max(0,Math.min(1,g.x+ps[0].x-g.start.x)),y:Math.max(0,Math.min(1,g.y+ps[0].y-g.start.y))});else if(ps.length===2&&g.distance){const distance=Math.hypot(ps[1].x-ps[0].x,ps[1].y-ps[0].y),angle=Math.atan2(ps[1].y-ps[0].y,ps[1].x-ps[0].x);onChange({size:Math.max(.02,Math.min(.25,g.size*distance/g.distance)),rotation:g.rotation+(angle-g.angle)*180/Math.PI});}};
  const up=e=>{pointers.current.delete(e.pointerId);gesture.current=null;};
  return <div className={className} onPointerDown={down} onPointerMove={move} onPointerUp={up} onPointerCancel={up} style={{position:'relative',overflow:'hidden',touchAction:settings.mode==='point'?'none':'auto',cursor:settings.mode==='point'?'crosshair':'default'}}><DyedMonsterImage baseId={masu.baseId} src={masuDisplayImageUrl(base)} alt={masu.name} masuColors={colors} className="w-full h-full object-contain"/><MasuPatternLayer baseId={masu.baseId} src={masuDisplayImageUrl(base)} settings={settings}/></div>;
};
    ...
    <div onPointerDown={(e)=>{const rect=e.currentTarget.getBoundingClientRect(); spawnRipple(e.clientX-rect.left, e.clientY-rect.top);}} className="h-full w-full bg-slate-950 text-white overflow-hidden relative select-none font-sans" style={{height:'100%'}}>
      ...
      <div className="relative z-10 h-full flex flex-col" style={screenShake?{animation:bigShake?'mooQuake 750ms ease-in-out':'screenShake 450ms ease-in-out'}:undefined}>
        ...
        {gameState==='MASU_PATTERN_DEBUG'&&(()=>{
          ...
          const patchPattern=patch=>setPatternSettings(prev=>({...prev,...patch}));
          const resetPattern=()=>{setPatternStep(1);setPatternSettings({...MASU_PATTERN_DEFAULTS});setPatternCustomColor({open:false,h:198,s:.77,v:.97});};
          const slider=(key,label,min,max,step=1,format=v=>v)=><label className="block rounded-xl bg-slate-900/80 px-3 py-2"><span className="flex justify-between text-[10px] font-black text-slate-300"><b>{label}</b><output>{format(patternSettings[key])}</output></span><input aria-label={label} type="range" min={min} max={max} step={step} value={patternSettings[key]} onChange={e=>updatePattern(key,Number(e.target.value))} className="w-full min-h-[42px] accent-fuchsia-500 touch-none"/></label>;
          return <main className="flex-1 min-h-0 flex flex-col bg-slate-950" style={{paddingTop:'env(safe-area-inset-top)',paddingBottom:'env(safe-area-inset-bottom)'}}>
            ...
            {eligible.length===0?<section className="flex-1 flex flex-col items-center justify-center p-6 text-center"><p className="font-black text-slate-300">カスタマイズできるマスモンがいません</p></section>:!selected?<section className="flex-1 min-h-0 overflow-y-auto mh-scroll p-4"><p className="mb-3 text-[11px] font-bold text-slate-400">所持マスモンを1体選択してください。</p><div className="grid grid-cols-3 gap-2">{eligible.map(m=>{const base=ALL_PLAYER_MONSTERS[m.baseId];return <button key={m.id} onClick={()=>{setPatternMasuId(m.id);resetPattern();}} className="min-h-[116px] rounded-2xl border border-fuchsia-500/30 bg-slate-900 p-2"><DyedMonsterImage baseId={m.baseId} src={masuDisplayImageUrl(base)} alt={m.name} masuColors={getMasuColors(m)} className="w-16 h-16 mx-auto object-contain"/><b className="block truncate text-[10px]">{m.name}</b></button>})}</div></section>:(()=>{const base=ALL_PLAYER_MONSTERS[selected.baseId],regions=dyeRegionCount(selected.baseId),colors=getMasuColors(selected),options=patternSettings.mode==='point'?MASU_PATTERN_POINT_OPTIONS:MASU_PATTERN_REPEAT_OPTIONS;return <div className="flex-1 min-h-0 overflow-y-auto mh-scroll p-3 space-y-3 scroll-smooth">
              <nav className="grid grid-cols-3 gap-1 sticky top-0 z-30 bg-slate-950/95 py-2 backdrop-blur">{[1,2,3].map(n=><button key={n} onClick={()=>setPatternStep(n)} className={`min-h-[38px] rounded-lg text-[9px] font-black ${patternStep===n?'bg-fuchsia-700 ring-2 ring-fuchsia-300':'bg-slate-800 text-slate-400'}`}>STEP{n}<small className="block text-[7px]">{n===1?'付け方':n===2?'模様・配置':'色設定'}</small></button>)}</nav>
              <section id="masu-pattern-large-preview" className="rounded-2xl border border-fuchsia-500/30 bg-slate-900 p-2 text-center"><PatternPlacementPreview masu={selected} base={base} colors={colors} settings={patternSettings} onChange={patchPattern} className="w-[min(68vw,280px)] aspect-square mx-auto"/><b className="text-xs">{selected.name}</b>{patternSettings.mode==='point'&&<small className="block text-[8px] text-cyan-300">タップで配置・ドラッグで移動・2本指で拡大縮小／回転</small>}</section>
              <aside className="sticky top-[54px] z-20 rounded-xl border border-fuchsia-400/50 bg-slate-950/95 p-2 flex items-center gap-3 shadow-xl"><PatternedMasuImage masu={selected} base={base} colors={colors} settings={patternSettings} className="w-14 h-14 shrink-0"/><div className="text-[9px] font-black text-fuchsia-200">追従プレビュー<small className="block text-[7px] text-slate-400">全サイズ共通の正規化座標</small></div></aside>
              {patternStep===1&&<section className="space-y-3 rounded-2xl bg-slate-900/60 p-3"><h3 className="font-black text-fuchsia-300">STEP1：模様の付け方</h3>{[['all','全身'],['region','部位ごと'],['point','ワンポイントで貼る']].map(([id,label])=><button key={id} onClick={()=>patchPattern({mode:id,target:id==='region'?'1':'all',pattern:id==='point'?'star':'stripe',placed:false})} className={`w-full min-h-[48px] rounded-xl font-black ${patternSettings.mode===id?'bg-fuchsia-700 ring-2 ring-fuchsia-300':'bg-slate-800'}`}>{label}</button>)}{patternSettings.mode==='region'&&<div className="grid grid-cols-3 gap-2">{Array.from({length:regions},(_,i)=>String(i+1)).map(id=><button key={id} onClick={()=>updatePattern('target',id)} className={`min-h-[44px] rounded-xl font-black ${patternSettings.target===id?'bg-cyan-700 ring-2 ring-cyan-300':'bg-slate-800'}`}>染色{'①②③'[Number(id)-1]}</button>)}</div>}</section>}
              {patternStep===2&&<section className="space-y-3 rounded-2xl bg-slate-900/60 p-3"><h3 className="font-black text-fuchsia-300">STEP2：模様選択・配置調整</h3><div className="grid grid-cols-3 gap-2">{options.map(([id,label])=><button key={id} onClick={()=>patchPattern({pattern:id,placed:patternSettings.mode==='point'?patternSettings.placed:true})} className={`min-h-[44px] rounded-xl text-[9px] font-black ${patternSettings.pattern===id?'bg-fuchsia-700 ring-2 ring-fuchsia-300':'bg-slate-800'}`}>{label}</button>)}</div>{slider('size','模様の大きさ',.02,.25,.005,v=>`${Math.round(v*100)}%`)}{slider('rotation','回転',-180,180,1,v=>`${Math.round(v)}°`)}{slider('x','横位置',0,1,.01,v=>`${Math.round(v*100)}%`)}{slider('y','縦位置',0,1,.01,v=>`${Math.round(v*100)}%`)}{slider('opacity','透明度',0,100,1,v=>`${v}%`)}{patternSettings.mode==='point'&&<div className="grid grid-cols-2 gap-2"><button onClick={()=>patchPattern({placed:false})} className="min-h-[46px] rounded-xl bg-red-900 font-black">削除</button><button onClick={()=>patchPattern({placed:false,x:.5,y:.5})} className="min-h-[46px] rounded-xl bg-cyan-900 font-black">別の位置へ貼り直す</button></div>}</section>}
              {patternStep===3&&<section className="space-y-3 rounded-2xl bg-slate-900/60 p-3"><h3 className="font-black text-fuchsia-300">STEP3：色設定</h3><div className="flex justify-between items-center"><b className="text-[10px]">プリセット色・カスタム色</b><span className="w-9 h-9 rounded-full border-2 border-white/30" style={{backgroundColor:patternSettings.color}}/></div><div className="grid grid-cols-9 gap-1">{MASU_PATTERN_PRESET_COLORS.map(color=><button key={color} aria-label={`模様色 ${color}`} onClick={()=>updatePattern('color',color)} className="aspect-square rounded-full border border-white/30" style={{backgroundColor:color}}/>)}<button aria-label="カスタム色" onClick={()=>setPatternCustomColor(prev=>({...prev,open:!prev.open}))} className="aspect-square rounded-full" style={{background:'conic-gradient(#ef4444,#eab308,#22c55e,#06b6d4,#3b82f6,#d946ef,#ef4444)'}}/></div>{patternCustomColor.open&&<><CustomColorPicker h={patternCustomColor.h} s={patternCustomColor.s} v={patternCustomColor.v} onChange={(h,s,v)=>{const [r,g,b]=_hsvToRgb(h,s,v);setPatternCustomColor({open:true,h,s,v});updatePattern('color',`#${[r,g,b].map(n=>n.toString(16).padStart(2,'0')).join('')}`);}}/><div className="grid grid-cols-3 text-center text-[8px] text-slate-400"><span>色相 {Math.round(patternCustomColor.h)}°</span><span>彩度 {Math.round(patternCustomColor.s*100)}%</span><span>明度 {Math.round(patternCustomColor.v*100)}%</span></div></>}</section>}
              <section className="rounded-2xl border border-white/10 bg-slate-900/60 p-3"><div className="text-[10px] font-black text-fuchsia-300">3サイズ比較</div><div className="flex items-end justify-around"><div><PatternedMasuImage masu={selected} base={base} colors={colors} settings={patternSettings} className="w-36 h-36"/><small>編集用大表示</small></div><div><PatternedMasuImage masu={selected} base={base} colors={colors} settings={patternSettings} className="w-20 h-20"/><small>通常カード</small></div><div><PatternedMasuImage masu={selected} base={base} colors={colors} settings={patternSettings} className="w-10 h-10"/><small>小型</small></div></div></section>
              <footer className="grid grid-cols-3 gap-2 pb-2"><button disabled={patternStep===1} onClick={()=>setPatternStep(s=>Math.max(1,s-1))} className="min-h-[48px] rounded-xl bg-slate-800 disabled:opacity-30 font-black">前へ</button><button onClick={()=>setPatternStep(s=>Math.min(3,s+1))} disabled={patternStep===3} className="min-h-[48px] rounded-xl bg-fuchsia-700 disabled:opacity-30 font-black">次へ</button><button onClick={resetPattern} className="min-h-[48px] rounded-xl bg-red-950 text-[9px] font-black">最初からやり直す</button></footer><button onClick={()=>setPatternMasuId(null)} className="min-h-[48px] rounded-xl border border-fuchsia-500/50 font-black">別のマスモンを選ぶ</button>
            </div>})()}
          </main>;
        })()}
