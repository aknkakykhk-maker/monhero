...
const rankingMasuDetail = (masu) => {
  ...
  const inherited = (Array.isArray(masu.inheritedUniques) ? masu.inheritedUniques : []).map((unique, index) => {
    ...
    return { monId, level: Math.max(num(unique.evoLevel), num(masu.uniqueSkillLevels?.[`inh:${index}`])) };
  }).filter(Boolean);
  return {
...
// lastIntent は「たった今どの行動を終えたか」。
  ...
  const getNextEnemyAction = useCallback((ent, currentDist, lastIntent=null, extraState=null) => {
    ...
    return chooseEnemyAction(ent,currentDist,Math.random,{...enemyActionStateFrom(lastIntent), ...(extraState||{})});
  }, []);
  ...
  const processTurn = async () => {
    ...
    for (const entry of usedCardEntries) {
      ...
      if (card.type==='buff'||card.type==='debuff') {
        ...
        else if (card.subType==='buff_myaru') { setNextTurnBuff('atkMult',card.baseValue); const selfDmgAmt=Math.floor(hpBeforeEnemyAttack*card.selfDmg); addPopup(`自傷-${selfDmgAmt}`,'hero','text-red-600 text-2xl font-black'); hpBeforeEnemyAttack=Math.max(1,hpBeforeEnemyAttack-selfDmgAmt); setHp(hpBeforeEnemyAttack); }
      }
        ...
        if (card.type==='unique') {
          ...
          if(card.monId==='Mocchi'||card.monId==='Mitarashi'){addPermaBuff('dmgCutPct',0.03*effMul); addWaveBuff('enemyTakenDmgBonus',0.1*effMul); localDmgModAdd+=0.1*effMul; addPopup('被ダメ軽減UP!','hero','text-emerald-400 text-lg font-bold');}
        ...
        if (card.type==='unique') {
          ...
          else if(card.monId==='Monol'){addDefPct(0.03*effMul); addWaveBuff('enemyAtkDebuffPct',0.10*effMul); setNextTurnBuff('reflect',true); addPopup('丈夫さUP!','hero','text-emerald-400 text-lg font-bold'); addPopup('次ターン反射！','hero','text-purple-400 text-lg font-bold');}
          else if(card.monId==='Oboro'){const hRec=Math.floor(finalD*0.5); const gRec=Math.floor(finalD*0.05); hpBeforeEnemyAttack=Math.min(effectiveMaxHp,hpBeforeEnemyAttack+hRec); setHp(hpBeforeEnemyAttack); setGuts(p=>Math.min(effectiveMaxGuts,p+gRec)); addPopup(`💚 ドレイン +${hRec}`,'life','text-emerald-400 text-xl font-black drop-shadow-md'); addPopup(`⚡ ガッツ +${gRec}`,'guts','text-amber-400 text-base font-bold drop-shadow-md');}
    ...
    if (totalDmg>0||totalHeal>0) {
      if(totalHeal>0){addPopup(`💚 回復 +${totalHeal}`,'life','text-emerald-400 text-4xl font-black drop-shadow-lg'); await battleWait(600); hpBeforeEnemyAttack=Math.min(effectiveMaxHp,hpBeforeEnemyAttack+totalHeal); setHp(hpBeforeEnemyAttack); await battleWait(400);}
      if(totalDmg>0){
    ...
    await handleEnemyTurn(finalActionType,{invincible:immediateInvincible,stun:immediateStun,guardFlat:currentTurnGuardFlat,guardMult:currentTurnGuardMult,distLocked:forcedMoveTarget!=null},executedIntent,hpBeforeEnemyAttack);
  ...
  const guardCardCount = (guardLv) => Math.min(
    MAX_GUARD_CARD_COUNT,
    2 + Math.floor(Math.max(0, guardLv || 0) / GUARD_LEVELS_PER_EXTRA_CARD),
  );
    ...
    if (isHero) {
      ...
      if (isQuickMode(runMode)) {
        ...
        setQuickJoin({
          ...
          nextStats:{hp:nMaxHp,atk:nAtk,def:nDef,guts:nMaxGuts},
        });
        quickAdvanceRef.current=null;
        Audio_.se.levelUp();
  ...
  return (
    ...
    <div onPointerDown={(e)=>{const rect=e.currentTarget.getBoundingClientRect(); spawnRipple(e.clientX-rect.left, e.clientY-rect.top);}} className="h-full w-full bg-slate-950 text-white overflow-hidden relative select-none font-sans" style={{height:'100%'}}>
                ...
                {members.map((m,i)=>{
                  ...
                  return(
                    <div key={`${m?.masuId||m?.id||'m'}-${i}`} className={`flex items-center gap-3 rounded-2xl border p-2 ${hero?'border-amber-400/60 bg-amber-500/10':'border-white/10 bg-black/30'}`}>
                      ...
                      {/* 育て方まで見せられるのは、詳細を送るようになってからの記録だけ。
                          それ以前の記録では押せない状態にして、理由をその場に出す */}
                      <button
                        onClick={()=>{ if (m?.detail) setRankingMonsterDetail(m); }}
                        disabled={!m?.detail}
      ...
      {/* ランキングの編成から開く、1体ぶんの詳細。表示はマスモン詳細と同じ共通実装を使い、
          こちらは見るだけ(改名・強化・放牧の操作は出さない)。 */}
      {rankingMonsterDetail&&(()=>{
        ...
        return (<div onClick={close} className="fixed inset-0 flex items-center justify-center p-4" style={{position:'fixed',inset:0,backgroundColor:'rgba(2,6,23,0.94)',zIndex:41900}} role="dialog" aria-modal="true" aria-label={`${masu.name}の詳細`}>
          <div onClick={e=>e.stopPropagation()} className="bg-slate-900 border-2 border-indigo-500 rounded-3xl p-5 w-full max-w-sm flex flex-col gap-2 shadow-2xl h-auto max-h-full overflow-hidden">
            <div className="flex items-center gap-4 border-b border-white/10 pb-4 shrink-0">
              <div className="relative w-20 h-20 shrink-0">
              ...
              <button onClick={close} className="p-2 bg-white/5 rounded-full active:scale-90 shrink-0"><X size={16}/></button>
            </div>
            <div className="flex-1 overflow-y-auto mh-scroll min-h-0 space-y-2">
              {renderMonsterDetailInfo(mon, {
                statTitle: '現在のステータス(強化分込み)',
                statValues: [
                  statRow('ライフ', base.baseHp+(sp.hp||0), sp.hp||0, 'text-pink-400'),
                  statRow('ちから', base.baseAtk+(sp.atk||0), sp.atk||0, 'text-red-400'),
                  statRow('丈夫さ', base.baseDef+(sp.def||0), sp.def||0, 'text-emerald-400'),
                  statRow('ガッツ', base.baseGuts+(sp.guts||0), sp.guts||0, 'text-amber-400'),
                ],
                aptPointsLabel: <div className="text-[8px] text-amber-300 font-black flex items-center gap-1"><Sparkles size={9}/>強化P: {masu.distAptPoints}</div>,
              })}
            ...
            <button onClick={close} className="w-full min-h-[48px] rounded-2xl bg-white text-black font-black text-sm active:scale-[.98] shrink-0">とじる</button>
          </div>
        </div>);
      })()}
