...
const Audio_ = (() => {
  ...
  const getBgmEl = (key) => {
    ...
    if (!bgmEls[key]) {
      ...
      try { el.volume = _bgmGain(bgmVolumePct); } catch (e) {}
  ...
  const getBgmCtx = () => {
    ...
    if (!AC) return null;
    try { bgmCtx = new AC(); } catch (e) { bgmCtx = null; }
    return bgmCtx;
  };
  ...
  const prepareBGM = (key, timeoutMs = 2000) => new Promise((resolve) => {
    ...
    if (el.readyState >= 3) { resolve(true); return; } // HAVE_FUTURE_DATA 以上なら再生できる
    let done = false;
    const finish = (ok) => {
      if (done) return;
      done = true;
  ...
  const connectJingleToGain = (key) => {
    ...
    try {
      if (!bgmGain) { bgmGain = ctx.createGain(); bgmGain.gain.value = _bgmGain(bgmVolumePct); bgmGain.connect(ctx.destination); }
      const srcNode = ctx.createMediaElementSource(el);
      srcNode.connect(bgmGain);
  ...
  const applyBgmVolume = () => {
    ...
    if (bgmGain) {
      try {
        const ctx = bgmCtx;
        if (ctx && bgmGain.gain.setTargetAtTime) bgmGain.gain.setTargetAtTime(v, ctx.currentTime, 0.03);
        else bgmGain.gain.value = v;
      } catch (e) { try { bgmGain.gain.value = v; } catch (e2) {} }
    }
    // Web Audioへ繋げた要素は素のvolumeを1のままにする(繋げていない要素だけvolumeで絞る)
    Object.entries(bgmEls).forEach(([k, el]) => { if (!bgmSources[k]) { try { el.volume = v; } catch (e) {} } });
  };
  const setBgmVolume = async (pct) => {
    bgmVolumePct = pct;
    applyBgmVolume();
    // 音量0はミュートとして扱い、再生自体を止める(ゲインが効かない環境でも確実に無音にする)
    if (pct <= 0) { stopOthers(null); return; }
...
const sbFetchRankings = async (diff, limit=RANKING_DIAGNOSTIC_LIMIT, order='score.desc', offset=0, requestId='untracked') => {
  ...
  rankingLog(requestId, 'request-start', {
    difficulty: diff, category: 'ranking', rankingType: order, table: 'rankings',
  ...
  try {
    ...
    const normalized = error?.name === 'AbortError'
      ...
      : error;
    rankingLog(requestId, 'supabase-error', { difficulty: diff, endedAt: new Date().toISOString(), elapsedMs: Date.now() - startedAt, timeout: error?.name === 'AbortError', networkError: error instanceof TypeError, name: normalized?.name, message: normalized?.message, stack: normalized?.stack });
    throw normalized;
  } finally {
    clearTimeout(timer);
  }
...
function MonsterHeroGame() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!cancelled) setBootProgress({ done: 0, total, label: 'サウンドシステム 起動中' });
            ...
            {showRanking&&(
              <div className="fixed inset-0 z-[8000] flex flex-col p-6" style={{position:'fixed',inset:0,backgroundColor:'rgba(0,0,0,0.97)',zIndex:80000,paddingTop:'calc(1.5rem + env(safe-area-inset-top))'}}>
                ...
                <div className="flex-1 overflow-y-auto mh-scroll space-y-3 min-h-0">
                  {(localRankings[rankingViewDiff]||[]).length===0?(<div className="h-full flex items-center justify-center text-slate-600 font-black uppercase text-xs italic">{rankingLoadingByDiff[rankingViewDiff]?'Loading...':'No records yet'}</div>):(
