...
const createTrainingSession=(masuId,difficulty='BEGINNER')=>({status:'playing',masuId:String(masuId),difficulty,mapId:TRAINING_MAP_ID,seed:trainingSeed(),position:'n0',previous:null,remainingTurns:10,rewards:trainingEmptyRewards(),tools:[],effects:{},lastRoll:null,rollPending:false,branchOptions:[],movementRemaining:0,forcedMoves:0,eventLog:['修行テスト開始'],message:'サイコロを振ってください'});
...
function MonsterHeroGame() {
  ...
  const rollTrainingDice=(fixedValue=null,isReroll=false)=>{if(!trainingSession||trainingSession.status!=='playing'||trainingMovingRef.current||trainingSession.branchOptions.length||(!isReroll&&trainingSession.rollPending))return;Audio_.se.trainingDice();const roll=()=>1+Math.floor(Math.random()*3);let value=fixedValue||trainingDebugRoll||roll();if(!isReroll&&trainingSession.effects.gale)value=Math.max(value,roll());if(!isReroll&&trainingSession.effects.boost)value++;if(!isReroll&&trainingSession.effects.feather)value+=2;const effects={...trainingSession.effects};delete effects.gale;delete effects.boost;delete effects.feather;delete effects.fixed;const canReroll=!isReroll&&trainingSession.tools.includes('reroll');const next={...trainingSession,lastRoll:value,rollPending:canReroll,effects,remainingTurns:trainingSession.remainingTurns-(isReroll?0:1),forcedMoves:0,message:canReroll?`${value}が出ました。振り直すか、この出目で進んでください`:`${value}が出ました。自動で進みます`,eventLog:logTraining(trainingSession,`${isReroll?'振り直し':'サイコロ'} ${value}`)};setTrainingSession(next);if(!canReroll)advanceTraining({...next,rollPending:false},value);};
  const acceptTrainingRoll=()=>{if(!trainingSession?.rollPending)return;const next={...trainingSession,rollPending:false,message:`${trainingSession.lastRoll}で自動前進します`};setTrainingSession(next);advanceTraining(next,next.lastRoll);};
  ...
  const useTrainingTool=id=>{const t=TRAINING_TOOLS[id],availability=trainingToolAvailability(id);if(!t||!availability.ok)return;const tools=[...trainingSession.tools];tools.splice(tools.indexOf(id),1);let effects={...trainingSession.effects},remainingTurns=trainingSession.remainingTurns;if(id==='sand')remainingTurns++;else if(id==='reroll'){const value=1+Math.floor(Math.random()*3),next={...trainingSession,tools,lastRoll:value,rollPending:false,message:`${value}に振り直しました。自動で進みます`,eventLog:logTraining(trainingSession,`振り直し ${value}`)};Audio_.se.trainingTool();setTrainingSession(next);advanceTraining(next,value);return;}else effects[id]=true;Audio_.se.trainingTool();patchTraining({tools,effects,remainingTurns,eventLog:logTraining(trainingSession,`${t.name}使用`)});};
