...
const GIFT_REWARD_LABELS = { diamond:'ダイヤ', breederPoint:'ブリーダーポイント', dyeMock:'染色もどき', bondPointReset:'絆ポイントリセットアイテム' };
...
const buildGiftClaim = (gift, balances, now=Date.now()) => {
  ...
  rewards.forEach(({type,amount})=>{ if(type==='diamond') next.gold+=amount; else if(type==='breederPoint') next.breederPoints+=amount; else { const id=type==='dyeMock'?'dye_mock':'bond_reset_scroll'; next.ownedItems[id]=(next.ownedItems[id]||0)+amount; } });
  ...
  return (
    <div onPointerDown={(e)=>{const rect=e.currentTarget.getBoundingClientRect(); spawnRipple(e.clientX-rect.left, e.clientY-rect.top);}} className="h-full w-full bg-slate-950 text-white overflow-hidden relative select-none font-sans" style={{height:'100%'}}>
      ...
      <div className="relative z-10 h-full flex flex-col" style={screenShake?{animation:bigShake?'mooQuake 750ms ease-in-out':'screenShake 450ms ease-in-out'}:undefined}>
        ...
        {gameState==='GIFT_BOX'&&(()=>{const now=Date.now();const unclaimed=gifts.filter(g=>!g?.claimedAt);const history=gifts.filter(g=>g?.claimedAt);const shown=giftTab==='unclaimed'?unclaimed:history;const claimable=unclaimed.filter(g=>!giftIsExpired(g,now)&&normalizeGiftRewards(g));return <div className="flex-1 flex flex-col h-full min-h-0 p-4" style={{paddingTop:'calc(1rem + env(safe-area-inset-top))',paddingBottom:'calc(1rem + env(safe-area-inset-bottom))'}}>
          <div className="flex items-center justify-between gap-2 mb-3 shrink-0"><button onClick={returnToHome} className="p-3 text-slate-400 active:scale-90"><ArrowLeft size={20}/></button><h2 className="text-xl font-black text-cyan-200 flex items-center gap-2"><Package size={22}/>ギフトボックス</h2><div className="w-11"></div></div>
          <div className="grid grid-cols-2 gap-2 mb-3 shrink-0"><button onClick={()=>setGiftTab('unclaimed')} className={`min-h-[44px] rounded-xl font-black text-sm ${giftTab==='unclaimed'?'bg-cyan-600 text-white':'bg-slate-900 text-slate-400'}`}>未受取 ({unclaimed.filter(g=>!giftIsExpired(g,now)).length})</button><button onClick={()=>setGiftTab('history')} className={`min-h-[44px] rounded-xl font-black text-sm ${giftTab==='history'?'bg-indigo-600 text-white':'bg-slate-900 text-slate-400'}`}>受取済み ({history.length})</button></div>
          {giftTab==='unclaimed'&&<button disabled={!claimable.length} onClick={()=>claimGiftIds(claimable.map(g=>g.id))} className="shrink-0 mb-3 min-h-[44px] rounded-xl bg-gradient-to-r from-cyan-600 to-indigo-600 text-white font-black disabled:opacity-40">すべて受け取る</button>}
          <div className="flex-1 min-h-0 overflow-y-auto mh-scroll space-y-2 pb-2">{shown.length===0?<div className="mt-16 text-center text-slate-500 font-bold">{giftTab==='unclaimed'?'未受取のギフトはありません':'受取済みのギフトはありません'}</div>:shown.map(g=>{const expired=giftIsExpired(g,now);const valid=!!normalizeGiftRewards(g);return <article key={g.id} className={`rounded-2xl border p-3 ${g.claimedAt?'bg-slate-900/70 border-slate-700':expired?'bg-red-950/30 border-red-800/60':'bg-cyan-950/30 border-cyan-500/50'}`}>
            <div className="flex items-start justify-between gap-1.5"><div className="min-w-0"><h3 className="font-black text-white break-words leading-tight">{g.title||'名称なしギフト'}</h3><p className="text-[11px] leading-snug text-slate-400 mt-0.5 break-words">{g.description||''}</p></div><span className={`shrink-0 rounded-full px-2 py-0.5 text-[9px] font-black ${g.claimedAt?'bg-slate-700 text-slate-300':expired?'bg-red-900 text-red-200':valid?'bg-cyan-700 text-white':'bg-amber-900 text-amber-200'}`}>{g.claimedAt?'受取済み':expired?'期限切れ':valid?'受取可能':'確認が必要'}</span></div>
            <div className="my-2 space-y-1">{Array.isArray(g.rewards)&&g.rewards.map((r,i)=><div key={i} className="rounded-lg bg-black/30 px-3 py-1.5 text-sm leading-snug font-black text-amber-200 break-words">{giftRewardText(r)}</div>)}</div>
            <div className="flex items-end justify-between gap-1.5"><div className="min-w-0 pb-0.5 text-[9px] leading-tight text-slate-500 break-words">{g.claimedAt?`受取日時: ${new Date(g.claimedAt).toLocaleString('ja-JP')}`:`受取期限: ${g.expiresAt?new Date(g.expiresAt).toLocaleString('ja-JP'):'期限情報なし'}`}</div>{!g.claimedAt&&<button disabled={expired||!valid} onClick={()=>claimGiftIds([g.id])} className="shrink-0 min-h-[40px] px-4 rounded-xl bg-cyan-600 text-white text-sm font-black disabled:bg-slate-700 disabled:text-slate-500">受け取る</button>}</div>
          </article>})}</div>
        </div>})()}
