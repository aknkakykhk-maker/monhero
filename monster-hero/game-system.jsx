...
const BUILD_DATE = "2026-07-26 09:40"; // 更新のたびに手動で書き換える(日付+時刻、JST) ※version.jsonのbuildも同じ値に合わせること
...
const MASU_COLOR_REGION_HUES = {
  ...
  // 2026年に新規イラスト(悪魔っ子)へ差し替え。地肌(彩度0.05〜0.12)は染色対象外にする。
  // 髪はグラデーションで根元(hue321前後)〜毛先(hue350前後)まで色相が連続的に変化しており、
  // 衣装(hue347前後)とbboxが無ければ毛先側が衣装の色相に奪われてしまう。互いにbboxで
  // 頭上/胴体以下の範囲を分離することで、範囲内では色相の遠近だけで確実に髪側へ倒れるようにしている。
  // 翼は両者と色相が近すぎるため、色を問わず位置で強制するposBboxにして別部位として独立させた
  Pixie: [{ hue: 321, sMin: 0.15, bbox: [0.25, 0.0, 0.75, 0.30] }, { posBbox: [[0.02, 0.28, 0.30, 0.55], [0.68, 0.28, 0.98, 0.55]] }, { hue: 347, sMin: 0.15, bbox: [0.0, 0.28, 1.0, 1.0] }],
