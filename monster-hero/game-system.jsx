...
const BUILD_DATE = "2026-07-26 16:08"; // 更新のたびに手動で書き換える(日付+時刻、JST) ※version.jsonのbuildも同じ値に合わせること
...
const MASU_COLOR_REGION_HUES = {
  // 口ばし(染色③)は体との色相の距離が近い場面があり、色相判定だけだと輪郭がガビガビに
  // なっていたため、実測した口ばしの輪郭に沿って段状の矩形を積み重ねるposBboxに変更している
  Mocchi: [{ hue: 350, sMin: 0.08 }, { hue: 92, sMin: 0.2 }, { posBbox: [[0.448, 0.237, 0.545, 0.244], [0.423, 0.244, 0.564, 0.250], [0.404, 0.250, 0.577, 0.256], [0.380, 0.256, 0.583, 0.263], [0.354, 0.263, 0.583, 0.269], [0.317, 0.269, 0.677, 0.275], [0.311, 0.275, 0.683, 0.281], [0.311, 0.281, 0.677, 0.287], [0.317, 0.287, 0.677, 0.294], [0.323, 0.294, 0.670, 0.300], [0.336, 0.300, 0.658, 0.306], [0.348, 0.306, 0.645, 0.313], [0.361, 0.313, 0.633, 0.319], [0.392, 0.319, 0.602, 0.325]], noAAGuard: true, noEdgeGuard: true }],
  // 2026年に新規イラストへ差し替え。体(明るい黄)と瞳(暗い黄褐色)は同じ色相のため、
  // 明度で明暗を分けて別部位にしている(白目・彩度の低い部分は染色対象外のまま)。
  // 画像はダウンスケールせず元の解像度に近い状態のまま実装している。口(染色③)は
  // 瞳の下に三日月形で見える部位で、色相判定だけだと輪郭がガビガビになっていたため、
  // ダウンスケール前の高解像度な元イラストで輪郭を実測し、行ごとに矩形を積み重ねている
  Suezo: [{ hue: 45, sMin: 0.3, vMin: 0.55 }, { hue: 45, sMin: 0.3, vMax: 0.55 }, { posBbox: [[0.2954,0.5211,0.3137,0.5246],[0.2954,0.5246,0.696,0.528],[0.298,0.528,0.696,0.5315],[0.2997,0.5315,0.696,0.5349],[0.3032,0.5349,0.6943,0.5384],[0.3057,0.5384,0.6917,0.5418],[0.3075,0.5418,0.6891,0.5453],[0.3092,0.5453,0.6873,0.5487],[0.3118,0.5487,0.6856,0.5522],[0.3144,0.5522,0.6839,0.5557],[0.3161,0.5557,0.6813,0.5591],[0.3187,0.5591,0.6796,0.5626],[0.3213,0.5626,0.677,0.566],[0.3239,0.566,0.6744,0.5695],[0.3265,0.5695,0.6718,0.5729],[0.3299,0.5729,0.6684,0.5764],[0.3325,0.5764,0.6658,0.5798],[0.3359,0.5798,0.6632,0.5833],[0.3394,0.5833,0.6597,0.5867],[0.3403,0.5867,0.6563,0.5902],[0.3429,0.5902,0.6528,0.5936],[0.3454,0.5936,0.6494,0.5971],[0.3498,0.5971,0.6451,0.6005],[0.3541,0.6005,0.6425,0.604],[0.3575,0.604,0.6408,0.6074],[0.3627,0.6074,0.6356,0.6109],[0.367,0.6109,0.6304,0.6143],[0.3722,0.6143,0.627,0.6178],[0.3774,0.6178,0.6209,0.6212],[0.3834,0.6212,0.6157,0.6247],[0.3886,0.6247,0.6106,0.6281],[0.3946,0.6281,0.6045,0.6316],[0.4015,0.6316,0.5976,0.635],[0.4084,0.635,0.5916,0.6385],[0.4153,0.6385,0.5838,0.6419],[0.4231,0.6419,0.576,0.6454],[0.4309,0.6454,0.5683,0.6488],[0.4403,0.6488,0.5588,0.6523],[0.4507,0.6523,0.5476,0.6557],[0.4654,0.6557,0.5329,0.6592]], noAAGuard: true, noEdgeGuard: true }],
