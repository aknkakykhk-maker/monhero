...
const MASU_COLOR_REGION_HUES = {
  ...
  // 角(オレンジ)と尻尾の先端のみをまとめて染色③とした(角は色相、尻尾の先端は位置で判定)。
  // 尻尾の根元〜中央は体の毛の続きとして染色①側で塗る(先端だけ染色③に切り替わる)
  // 尻尾は根元〜中央にかけて彩度が非常に低く(毛色の陰影のグラデーション)、体の青毛(sMin:0.28)にも
  // 白バケツ(顔用bboxの範囲外)にも該当せず無染色のまま残っていたため、根元〜中央側もposBboxで
  // 位置判定にし、体と同じ染色①に含めた(彩度に関係なく必ず染まるようにする)
  // 頭上の帽子状の毛(耳と耳の間、顔の上)は地の色が青系で染色①(体)側の毛のはずだが、
  ...
  // 染色③(耳のオレンジ+尻尾の先端)はnoAAGuardで無理に染めようとすると、耳のような小さく
  // 輪郭の多い部位ではアンチエイリアス済みの縁ピクセルまで塗ってしまいギザギザした汚い輪郭になる
  // (実機で確認して撤回)。ここは元のガード仕様のまま(耳は無染色で残る)にしておく。
  // 染色①(頭上の帯、posBboxで強制指定した部分)は逆に、透明ピクセルに近い(耳と耳の間の谷になっている)
  // せいでAAガードにかかって塗り残っていたため、posBboxで位置指定済み(=誤って輪郭を拾う心配がない)の
  // 安全な範囲だけnoAAGuardを付けて塗り残りを解消する
  Tiger: [{ hue: 233, sMin: 0.28, noAAGuard: true, posBbox: [[0.43, 0.11, 0.49, 0.24], [0.665, 0.30, 0.84, 0.65]] }, { white: true, sMax: 0.28, vMin: 0.4, bbox: [0.26, 0.27, 0.66, 1.0] }, { hue: 38, posBbox: [[0.84, 0.28, 1.0, 0.50]] }],
...
// 尻尾は根元側(体に近い方)を染色①(体)、先端側のみ染色③(耳と同じ)に分けている
// (0.665〜0.84が根元=①、0.84〜1.0が先端=③。根元側は彩度が低く単体では判定できないためposBboxで強制している)
const MASU_COLOR_REGION_SIZE_OVERRIDES = {
  Tiger: {
    128: { 0: { posBbox: [[0.383, 0.117, 0.469, 0.141], [0.352, 0.141, 0.477, 0.164], [0.344, 0.164, 0.484, 0.188], [0.328, 0.188, 0.520, 0.211], [0.665, 0.30, 0.84, 0.65]] },
           2: { posBbox: [[0.84, 0.28, 1.0, 0.50]] } },
    // 額の帯(耳と耳の間)のposBboxが実際の耳の輪郭よりわずかに狭く、両端に幅1〜2pxの隙間ができていた。
    // その隙間はどの染色部位の色相にも遠く、以前は色相フォールバックで無理やり(遠い色相のまま)
    // 染色②などへ押し込まれて偶然目立たなかったが、遠すぎるマッチを弾く仕組みを入れたことで
    // 無染色のまま浮いて見えるようになったため、実測した耳の輪郭ぎりぎりまでposBboxを広げた。
    // さらに額の帯の下端(y0.269〜0.29)には、白い顔の毛(染色②)より彩度の高い青みがかった
    // 陰影ストライプがあり、染色②のsMax(0.28)にちょうど収まってしまうため染色②の白バケツに
    // 奪われて無染色の白顔の毛の隣に浮いて見えていた。ここも染色①側に含めて自然に繋げる
    160: { 0: { posBbox: [[0.420, 0.163, 0.555, 0.181], [0.425, 0.181, 0.565, 0.200], [0.415, 0.200, 0.572, 0.219], [0.405, 0.219, 0.578, 0.269], [0.385, 0.269, 0.60, 0.29], [0.665, 0.30, 0.84, 0.65]] },
           2: { posBbox: [[0.84, 0.28, 1.0, 0.50]] } },
  },
};
