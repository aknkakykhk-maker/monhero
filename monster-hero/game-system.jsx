...
const BUILD_DATE = "2026-07-26 13:21"; // 更新のたびに手動で書き換える(日付+時刻、JST) ※version.jsonのbuildも同じ値に合わせること
...
const MASU_COLOR_REGION_HUES = {
  Mocchi: [{ hue: 350, sMin: 0.08 }, { hue: 92, sMin: 0.2 }, { hue: 45, sMin: 0.15 }],
  // 2026年に新規イラストへ差し替え。体(明るい黄)と瞳(暗い黄褐色)は同じ色相のため、
  // 明度で明暗を分けて別部位にしている(白目・彩度の低い部分は染色対象外のまま)
  Suezo: [{ hue: 45, sMin: 0.3, vMin: 0.55 }, { hue: 45, sMin: 0.3, vMax: 0.55 }, { hue: 340, sMin: 0.3 }],
  ...
  // 2026年に新規イラストへ差し替え。体(赤、染色①)・お腹/頭上クレスト/口/翼の金色(染色②)の2部位が
  // メイン。差し替え前の旧イラストでもクレストは色相的にお腹と同じ金色に分類され、染色③は
  // クレスト最上部のごく小さな縁取りのみだった(染色②が大半、染色③はほぼ目立たない程度)。
  // 差し替え直後に一度クレスト全体をposBboxで染色③として独立させたが、旧イラストと比べて
  // 染色③の面積が大きくなりすぎ「差し替え前と印象が変わった」ため、旧イラストと同じ比率に戻し、
  // クレスト最上部のごく小さな範囲だけ染色③として残している
  Mitarashi: [{ hue: 0, sMin: 0.3 }, { hue: 35, sMin: 0.3 }, { posBbox: [[0.42, 0.05, 0.57, 0.09]], noAAGuard: true, noEdgeGuard: true }],
