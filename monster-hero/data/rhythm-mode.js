...
const rhythmMatchInputBatch=(notes,inputs,nowMs,offsetMs=0)=>{
  ...
  return (Array.isArray(inputs)?inputs:[]).map(input=>{
    ...
    //
    // ここは「叩いた時刻に近いほう」ではなく **判定が良くなるほう** で決める。
    // 近さで決めると、2つのちょうど中間で切り替わってしまい、
    // 16分(0.088秒)なら0.044秒遅れただけで次のノーツへ移る。それは
    //   「近くに次のノーツがあるときに判定がそっちにいってる」(2026-09-05・実機の指摘)
    // そのものなので、遅れて叩いたぶんは前のノーツのままにしたい。
    //
    // 一方で、次のノーツをほぼぴったり叩いたときは、そちらを取ってほしい。
    //   「タップ判定の巻き込みもまだある」(2026-09-05・実機の指摘)
    //   ノーツA=1.000秒 / B=1.100秒 で1.099秒に叩くと、以前は0.099秒前のAが取られていた。
    //
    // 判定の段(MARVELOUS/EXCELLENT/…)で比べると、この2つが同時に立つ。
    //   ・16分で0.048秒遅れ … 前=MARVELOUS / 次=0.040秒前でMARVELOUS → 同じ段なので前を取る
    //   ・A/Bの例          … 前=EXCELLENT(0.099秒) / 次=MARVELOUS(0.001秒) → 次のほうが良い段
    // 同じ段のときは、実際に叩いたつもりである「過ぎている側」を取る。
    const judgeRank=deltaMs=>{
      const distance=Math.abs(Number(deltaMs));
      for(let i=0;i<RHYTHM_JUDGMENTS.length;i++){
        const windowMs=RHYTHM_JUDGMENTS[i].windowMs;
        if(!Number.isFinite(windowMs)||distance<=windowMs)return i;
      }
      return RHYTHM_JUDGMENTS.length;
    };
    let chosen=null;
    if(passedBest&&upcomingBest){
      chosen=judgeRank(upcomingBest.noteTime-now)<judgeRank(now-passedBest.noteTime)?upcomingBest:passedBest;
    }
    else chosen=passedBest||upcomingBest;
