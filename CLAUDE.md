...
返答を終える直前に `tools/hooks/japanese-only-check.js`(`.claude/settings.json` の Stop フック)が
英語の文を見つけると終われなくなる。止められたら言い訳せず日本語で書き直す(2026-09-24)。
...
- **更新履歴はプレイヤー向けの文にする。作業報告にしない**(2026-09-18の指示)。
  ゲームの外の数え方(「全◯場面」「◯本から◯本」)・開発の語(「検査を追加」)・作業の都合
  (「さきほど」「今回はそのまま」)を書かない。タイトルは「何が良くなったか」で書く。
  `node tools/changelog/player-words-check.js` が見張る
...
ノーツ数が動く強化は**譜面の作り方の版**(`chartRevision`・`tools/mode/rhythm-chart-v3-revision.js`)を
1つ上げてその中へ入れる。版を書いていない既存曲は版1のまま、新しく解析した曲だけが最新版になる。
